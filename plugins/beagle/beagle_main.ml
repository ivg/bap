...
module Hunter (Machine : Primus.Machine.S) = struct
  ...
  let process_byte char =
    Eval.pos >>| Primus.Pos.tid
    >>= fun curr ->
    Machine.Local.get beagle
    >>= fun (Beagle d) ->
  ...
  let process_variable (_, r) =
    ...
    | Bil.Imm w ->
        ...
        Machine.Global.get hunters_club
        >>= fun {checked} ->
    ...
    Machine.sequence
      Primus.Interpreter.
        ...
        ; (* variable_read    >>> process_variable; *)
          (* variable_written >>> process_variable; *) stored >>> process_memory
        ; Primus.Machine.kill >>> gohome; Beagle_prey.detected >>> print_prey
        ; Beagle_prey.detected >>> hunt; Beagle_prey.caught >>> print_result ]
end

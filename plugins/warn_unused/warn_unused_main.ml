...
let mark unchecked proj =
  Project.program proj |> (marker unchecked)#run |> Project.with_program proj

let taint proj = Project.program proj |> taint |> Project.with_program proj
...
module Cmdline = struct
  let man =
    ...
    ; `Pre
        ...
        \    $(b,--)$(mname)$(b,-taint)\n\
        \    $(b,--)$(mname)$(b,-print)\n\
        \    $(b,--)$(mname)$(b,-mark)"; `S "DESCRIPTION"
    ; `P
        "If a subroutine has GNU attribute $(b,warn_unused_result) and\n\
        \  its result is not used, then print a warning message."; `S "PASSES"
    ; `I
        ( "$(b,--)$(mname)$(b,-taint)"

...
module Abi32 = struct
  ...
  let arg = function Ret_0 -> reg 3 | Ret_1 -> reg 4 | Arg n -> reg (n + 3)
end
    ...
    | Some width -> (
      ...
      | Some sz ->
          ...
          if width > word && width <= word * 2 then
            Some (data, Bil.(Abi.arg Ret_0 ^ Abi.arg Ret_1))
          else if width <= word then Some (data, Abi.arg Ret_0)
          else (
            warning "size of return object doesn't fit into double word\n" ;
            raise Unsupported ) ) in
  let params =
    List.mapi args ~f:(fun i (n, t) ->
        ...
        | Some size -> (
          ...
          | Some sz when size <= word -> (C.Abi.data Abi.size t, Abi.arg (Arg i))
          | _ ->
...
let main proj =
  ...
  | `ppc ->
      ...
      let abi =
        C.Abi.
          {insert_args= dispatch (module Abi32); apply_attrs= (fun _ -> ident)}
      in

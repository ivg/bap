...
let addic cpu ops =
  ...
  RTL.
    [ tm := ra; rt := ra + im; cpu.ca := rt < low cpu.word_width tm
...
let addc cpu ops =
  ...
  let tm = unsigned var cpu.word_width in
  RTL.
    [ tm := ra; rt := ra + rb; cpu.ca := rt < low cpu.word_width tm
    ; cpu.ca32 := low word rt < low word tm ]
...
let adde cpu ops =
  ...
  let tm = unsigned var cpu.word_width in
  RTL.
    [ tm := ra; rt := ra + rb + cpu.ca; cpu.ca := rt < low cpu.word_width tm
    ; cpu.ca32 := low word rt < low word tm ]
...
let addme cpu ops =
  ...
  let tm = unsigned var cpu.word_width in
  RTL.
    [ tm := ra; rt := ra + cpu.ca - one; cpu.ca := rt < low cpu.word_width tm
    ; cpu.ca32 := low word rt < low word tm ]
...
let addze cpu ops =
  ...
  let tm = unsigned var cpu.word_width in
  RTL.
    [ tm := ra; rt := ra + cpu.ca; cpu.ca := rt < low cpu.word_width tm
    ; cpu.ca32 := low word rt < low word tm ]

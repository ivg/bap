...
module Std = struct
  ...
  let concat f g cpu ops = f cpu ops @ g cpu ops
  let ( ^ ) = concat
  ...
  let dot fc cpu ops =
    ...
    fc cpu ops
    @ RTL.
        [ x := low cpu.word_width res; nth bit cpu.cr 0 := x < zero
        ; nth bit cpu.cr 1 := x > zero; nth bit cpu.cr 2 := x = zero ]
  ...
  let lift addr_size endian mem insn =
    ...
    let lift lifter =
      ...
      with Failure str -> Error (Error.of_string str) in

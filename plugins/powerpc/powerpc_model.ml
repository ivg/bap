...
let make_regs_i typ prefix range =
  List.fold ~init:Int.Map.empty
    ~f:(fun regs i -> Map.set regs i (make_var_i typ prefix i))
    range
...
module Exps (B : Bitwidth) = struct
  ...
  let crn =
    Int.Map.fold Vars.cri ~init:String.Map.empty ~f:(fun ~key:_ ~data:var acc ->
...
module Make_ppc (S : Spec) : PowerPC = struct
  ...
  let flags =
    Var.Set.of_list
      [ so; ca; ca32; ov; ov32; Map.find_exn cri 0; Map.find_exn cri 1
      ; Map.find_exn cri 2 ]
end

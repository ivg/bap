...
let bc cpu ops =
  ...
  let tm = signed var cpu.word_width in
  RTL.
    [ x := last bo 5; when_ (nth bit x 2 = zero) [cpu.ctr := cpu.ctr - one]
...
let bclrl cpu ops =
  ...
  RTL.
    ...
    ; ctr_ok := nth bit x 2 lor ((cpu.ctr <> zero) lxor nth bit x 3)
    ; cond_ok := nth bit x 0 lor (bi lxor lnot (nth bit x 1))
    ; tm := cpu.lr land (ones cpu.word_width << sh); cpu.lr := cpu.pc + step
...
let bdnzlr cpu _ops =
  ...
  RTL.
    [ cpu.ctr := cpu.ctr - one
    ; when_ (cpu.ctr <> zero) [cpu.jmp (cpu.lr land (ones cpu.word_width << sh))]
    ]

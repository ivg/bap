...
let rldicr cpu ops =
  ...
  RTL.
    [ mask := zero; mask := lnot mask << width - me - one
    ; ra := nth doubleword (rs ^ rs ^ rs << sh) 0 land mask ]
...
let rldic cpu ops =
  ...
  RTL.
    [ mask := ones; mask1 := mask >> mb; mask2 := mask << sh
...
let rldimi cpu ops =
  ...
  RTL.
    [ mask := ones; mask1 := mask >> mb; mask2 := mask << sh
    ; mask := mask1 land mask2; tmp := nth doubleword (rs ^ rs ^ rs << sh) 0

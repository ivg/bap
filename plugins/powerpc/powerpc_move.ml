...
let mtspr cpu ops =
  ...
  RTL.
    [ tmp := sr; num := extract tmp 5 9 ^ extract tmp 0 4
    ; switch num
        [ case lr_num [cpu.lr := rs]; case ctr_num [cpu.ctr := rs]
...
let mfspr cpu ops =
  ...
  RTL.
    [ tmp := sr; num := extract tmp 5 9 ^ extract tmp 0 4
    ; switch num
        [ case lr_num [rt := cpu.lr]; case ctr_num [rt := cpu.ctr]
        ; default [message "Unknown cpu.register number"] ] ]

...
let cmpwi cpu ops =
  ...
  RTL.
    [ nth bit bf 0 := low word ra < si; nth bit bf 1 := low word ra > si
    ; nth bit bf 2 := low word ra = si; nth bit bf 3 := cpu.so ]
...
let cmpdi cpu ops =
  ...
  let si = signed imm ops.(2) in
  RTL.
    [ nth bit bf 0 := ra < si; nth bit bf 1 := ra > si; nth bit bf 2 := ra = si
    ; nth bit bf 3 := cpu.so ]
...
let cmpw cpu ops =
  ...
  RTL.
    [ nth bit bf 0 := low word ra < rb; nth bit bf 1 := low word ra > rb
    ; nth bit bf 2 := low word ra = rb; nth bit bf 3 := cpu.so ]
...
let cmpd cpu ops =
  ...
  let rb = signed cpu.reg ops.(2) in
  RTL.
    [ nth bit bf 0 := ra < rb; nth bit bf 1 := ra > rb; nth bit bf 2 := ra = rb
    ; nth bit bf 3 := cpu.so ]
...
let cmplwi cpu ops =
  ...
  RTL.
    [ nth bit bf 0 := low word ra < ui; nth bit bf 1 := low word ra > ui
    ; nth bit bf 2 := low word ra = ui; nth bit bf 3 := cpu.so ]
...
let cmpldi cpu ops =
  ...
  let ui = unsigned imm ops.(2) in
  RTL.
    [ nth bit bf 0 := ra < ui; nth bit bf 1 := ra > ui; nth bit bf 2 := ra = ui
    ; nth bit bf 3 := cpu.so ]

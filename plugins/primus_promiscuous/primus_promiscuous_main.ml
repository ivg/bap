...
module Forker (Machine : Primus.Machine.S) = struct
    ...
    Machine.List.map (assumptions blk)
      ~f:
        (Machine.List.filter ~f:(fun {var; res} ->
             Env.get var >>| Primus.Value.to_word
             >>| fun r -> Word.(r <> of_bool res)))
  ...
  let do_fork blk ~child =
    ...
    if Set.mem t.visited (Term.tid blk) then Machine.return pid
      ...
      else child () >>= fun () -> Machine.switch pid >>| fun () -> id
  ...
  let assume_returns blk call =
    ...
    | Some (Direct dst) ->
        Machine.current ()
        >>= fun pid ->
        do_fork blk ~child:Machine.return
        >>= fun id ->
...
let legacy_promiscous_mode_components =
  [ "var-randomizer"; "mem-randomizer"; "arg-randomizer"
  ; "promiscuous-path-explorer"; "division-by-zero-handler"; "limit" ]

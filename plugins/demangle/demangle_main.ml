...
let () =
  let () =
    Config.manpage
      [ `S "DESCRIPTION"; `P "Apply specified demangler to all subroutine names"
      ; `S "SEE ALSO"; `P "$(b,bap-plugin-cxxfilt)(1)" ] in
  ...
  Config.when_ready (fun {Config.get= ( ! )} ->
      Future.upon Plugins.loaded (fun () ->
          Option.iter !demangler (fun demangler ->
              ...
              | None ->
                  let names =
                    Demanglers.available () |> List.map ~f:Demangler.name
                    |> String.concat ~sep:", " in
                  invalid_argf
                    "Didn't find demangler %s, should be one of: %s\n" demangler
                    names ())))

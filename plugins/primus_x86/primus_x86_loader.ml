...
module SetupPLT (Machine : Primus.Machine.S) = struct
  ...
  let correct_sp sp addend _ =
    Env.get sp >>= fun x -> Interpreter.binop Bil.plus x addend >>= Env.set sp

  let correct_sp sp addend =
    match Var.typ sp with
    | Unk | Mem _ -> Machine.return ()
    | Imm width ->
        Value.of_int ~width addend
        >>= fun addend ->
  ...
  let run () =
    ...
    >>= function
    | `x86 -> Machine.sequence [correct_sp IA32.sp 4; unresolve]
    | `x86_64 -> Machine.sequence [correct_sp AMD64.sp 8; unresolve]
    | _ -> Machine.return ()

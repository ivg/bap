...
module BilParser = struct
  ...
  let is_big e =
    Bil.int @@ if [%compare.equal: endian] e BigEndian then Word.b1 else Word.b0
  ...
  let stmt : type t r. (t, exp, r, stmt) stmt_parser =
   fun (module S) ->
    let set v x =
      ...
      | Mem (ks, vs) -> S.set_mem n (Size.in_bits ks) (Size.in_bits vs) x in
    fun s ->
      match Call.dst s with
      | Some dst ->
...
module Optimizer = Theory.Parser.Make (Bil_semantics.Core) [@@inlined]

let provide_bir () =
  Knowledge.promise Theory.Program.Semantics.slot
  @@ fun obj ->
  KB.collect Theory.Program.Semantics.slot obj
  >>| fun sema ->
  let bir = Bil_ir.reify @@ KB.Value.get Bil_ir.slot sema in
  KB.Value.put Term.slot sema bir

module Relocations = struct
  type t = {rels: addr Addr.Map.t; exts: string Addr.Map.t}
  ...
  module Request = struct
    ...
    let arch =
      ...
      Fact.Seq.reduce
        ~f:(fun a1 a2 ->
          ...
          else Fact.failf "arch is ambiguous: %a <> %a" Arch.pp a1 Arch.pp a2 ())
        (Seq.filter_map ~f:Arch.of_string s)
      >>= fun a ->
      match a with
      | Some a -> Fact.return a
      | None -> Fact.failf "unknown/unsupported architecture" ()

    let arch_width = arch >>| fun arch -> Arch.addr_size arch |> Size.in_bits
    ...
    let external_symbols =
      ...
      Seq.fold s ~init:Addr.Map.empty ~f:(fun addrs (addr, name) ->
          Map.set addrs ~key:(Addr.of_int64 ~width addr) ~data:name)
  end
  ...
  let find_external {exts} mem = Seq.find_map ~f:(Map.find exts) (span mem)
  let find_internal {rels} mem = Seq.find_map ~f:(Map.find rels) (span mem)

  let subscribe () =
    let open Future.Syntax in
    Stream.hd Project.Info.spec >>| of_spec
  ...
  let fixup info mem bil =
    ...
    | Some info -> (
      ...
      | Some dst -> override_internal dst bil
      | None -> (
        match find_external info mem with
        | Some name -> override_external name bil
        | None -> bil ) )
end
...
module Brancher = struct
  ...
  let jmp _ =
    ...
    >>= fun dst -> ret Theory.Effect.Sort.jump dst
  ...
  let blk _ data ctrl =
    ...
    >>= fun ctrl ->
    let k =
      Theory.Effect.Sort.join [Theory.Effect.sort data] [Theory.Effect.sort ctrl]
    in
...
let create_intrinsic arch mem insn =
  ...
  @ Bil.
      ...
      ; Call.create (sprintf "%s:%s" (Insn.encoding insn) (Insn.name insn)) ]
...
type enable_intrinsic =
  {for_all: bool; for_unk: bool; for_special: bool; predicates: predicate list}
...
let lift ~enable_intrinsics:{for_all; for_unk; for_special; predicates} arch mem
    insn =
  if for_all || matches_spec predicates insn then
    ...
    | Error _ as err ->
        if for_unk then Ok (create_intrinsic arch mem insn) else err
    | Ok bil ->
...
let provide_lifter ~enable_intrinsics ~with_fp () =
  ...
  in
  let ( >>? ) x f = x >>= function None -> KB.return unknown | Some x -> f x in
  let enable_intrinsics = split_specs enable_intrinsics in
  let lifter obj =
    Knowledge.collect Arch.slot obj
    >>= fun arch ->
    Theory.instance ~context:(context arch) ()
    >>= Theory.require
    >>= fun (module Core) ->
    Knowledge.collect Memory.slot obj
    >>? fun mem ->
    Knowledge.collect Disasm_expert.Basic.Insn.slot obj
    >>? fun insn ->
    match lift ~enable_intrinsics arch mem insn with
    | Error _ -> Knowledge.return (Insn.of_basic insn)
    | Ok bil ->
        Bil_semantics.context
        >>= fun ctxt ->
        Knowledge.provide Bil_semantics.arch ctxt arch
        >>= fun () ->
        let module Lifter = Theory.Parser.Make (Core) in
        Optimizer.run BilParser.t bil
        >>= fun sema ->
        ...
        >>| fun sema ->
        let bil = Insn.bil sema in
        KB.Value.merge ~on_conflict:`drop_left sema (Insn.of_basic ~bil insn)
  in

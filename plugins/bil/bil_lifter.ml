...
let lift ~enable_intrinsics:{ for_all; for_unk; for_special; predicates } arch
    mem insn =
...
let provide_lifter ~enable_intrinsics ~with_fp () =
  ...
  in
  let ( >>? ) x f =
    x >>= function None -> KB.return unknown | Some x -> f x
  in
  let enable_intrinsics = split_specs enable_intrinsics in
  let lifter obj =
    Knowledge.collect Arch.slot obj >>= fun arch ->
    Theory.instance ~context:(context arch) () >>= Theory.require
    >>= fun (module Core) ->

...
module Make (B : Theory.Core) = struct
  ...
  let pack fsort sign expn coef =
    ...
    ite is_subnormal (pred expn) expn
    >>>= fun expn ->
    if Caml.(p = t) then pack_raw fsort sign expn coef
    else
      B.low (Bits.define t) coef
      >>>= fun coef -> pack_raw fsort sign expn coef >>= fun r -> !!r
  ...
  let unpack fsort x f =
    ...
    fsign x
    >>>= fun sign ->
    finite_significand fsort expn x
    >>>= fun coef ->
  ...
  let fzero fsort sign =
    ...
    ite sign
      ( ones bits
      >>>= fun ones -> (not (ones lsr one bits)) >>>= fun one -> one lor bitv )
      bitv
  ...
  let is_inf fsort x : Theory.bool =
    unpack fsort x
    @@ fun _ expn coef -> B.(and_ (is_zero coef) (is_all_ones expn))
  ...
  let is_qnan fsort x =
    let open B in
    unpack_raw fsort x
    @@ fun _sign expn coef -> is_all_ones expn && non_zero coef && msb coef
  ...
  let is_nan fsort x =
    let open B in
    unpack_raw fsort x
    @@ fun _sign expn coef -> is_all_ones expn && non_zero coef
  ...
  let qnan fsort =
    ...
    let spec = Sort.spec fsort in
    let sigs = Bits.define spec.t in
    (not (ones sigs lsr one sigs))
    >>>= fun coef -> pack_raw fsort B.b1 (ones exps) coef
  ...
  let with_special fsort x f =
    ...
    is_all_ones expn
    >>>= fun is_special ->
    (is_special && is_zero coef)
    >>>= fun is_inf ->
    (is_special && non_zero coef && inv (msb coef))
    >>>= fun is_snan ->
  ...
  let is_finite_nonzero fsort x =
    ...
    unpack_raw fsort x
    @@ fun _ expn coef ->
    inv (is_all_ones expn)
    >>>= fun ok_expn -> ok_expn && (non_zero expn || non_zero coef)
  ...
  let fsucc fsort x =
    ...
    succ coef
    >>>= fun coef ->
    ite (is_zero coef) (one exps) (zero exps)
    >>>= fun de ->
    ite (is_zero coef) (leading_one sigs) coef
    >>>= fun coef -> expn + de >>>= fun expn -> pack fsort sign expn coef
  ...
  let fpred fsort x =
    ...
    pred coef
    >>>= fun coef ->
    ite (is_all_ones coef) (one exps) (zero exps)
    >>>= fun de -> expn - de >>>= fun expn -> pack fsort sign expn coef
  ...
  let guardbits loss lost_bits f =
    ...
    !!lost_bits > one sort
    >>>= fun has_rnd ->
    !!lost_bits > of_int sort 2
    >>>= fun has_stk ->
    ite has_grd (pred !!lost_bits) (zero sort)
    >>>= fun grd_pos ->
    ite has_rnd (pred grd_pos) (zero sort)
    >>>= fun rnd_pos ->
    ite has_grd (testbit loss grd_pos) b0
    >>>= fun grd ->
    ite has_rnd (testbit loss rnd_pos) b0
    >>>= fun rnd ->
    ite has_stk (non_zero (extract_last loss rnd_pos)) b0
    >>>= fun stk -> f grd rnd stk

  let round rm sign coef loss lost_bits f =
    let open B in
    guardbits loss lost_bits
    @@ fun grd rnd stk ->
    ite (is_round_up rm sign (lsb coef) grd rnd stk) (succ coef) coef
    >>>= fun coef' ->
    and_ (non_zero coef) (is_zero coef')
    >>>= fun is_overflow -> f coef' is_overflow
  ...
  let nearest_pow2 num =
    let rec find pow n =
      let n' = 2 * n in
      if n' >= num then (pow + 1, n') else find (pow + 1) n' in
    find 0 1
  ...
  let clz x =
    ...
    let rec loop lets x = function
      ...
      | (total, shf) :: shifts ->
          ones sort' lsl of_int sort total
          >>>= fun mask ->
          ite (is_zero (x land mask)) (x lsl of_int sort shf) x
          >>>= fun nextx ->
          ite (is_zero (x land mask)) (of_int sort shf) (zero sort)
          >>>= fun nextn -> loop (nextn :: lets) nextx shifts in
    loop [] (B.unsigned sort' !!x) shifts
    >>>= fun n ->
    of_int sort Caml.(num - size)
    >>>= fun dif -> ite (is_zero !!x) (of_int sort size) (n - dif)
  ...
  let possible_lshift expn coef =
    ...
    min_exponent exps
    >>>= fun mine ->
    ite (!!expn < mine) (zero exps) (!!expn - mine)
    >>>= fun diff ->
  ...
  let norm_finite expn coef f =
    ...
    coef
    >>-> fun _sigs coef ->
    possible_lshift !!expn !!coef
    >>>= fun shift ->
    unsigned exps shift
    >>>= fun dexpn ->
    !!coef lsl shift
    >>>= fun coef ->
    ite (is_zero coef) (min_exponent exps) (!!expn - dexpn)
    >>>= fun expn -> f expn coef
  ...
  let guardbits' overflow last loss lost_bits f =
    ...
    ite overflow last guard'
    >>>= fun guard ->
    ite overflow guard' round'
    >>>= fun round ->
    ite overflow (round' || sticky') sticky'
    >>>= fun sticky -> f guard round sticky

  let fadd_finite fsort rm x y =
    let open B in
    let exps, sigs = floats fsort in
    unpack fsort x
    @@ fun xsign xexpn xcoef ->
    unpack fsort y
    @@ fun _ yexpn ycoef ->
    ite (xexpn > yexpn) (xexpn - yexpn) (yexpn - xexpn)
    >>>= fun lost_bits ->
    match_
    ...
    >>>= fun loss ->
    ite (xexpn > yexpn) xcoef (xcoef lsr lost_bits)
    >>>= fun xcoef ->
    ite (yexpn > xexpn) ycoef (ycoef lsr lost_bits)
    >>>= fun ycoef ->
    xcoef + ycoef
    >>>= fun sum ->
    max xexpn yexpn
    >>>= fun expn ->
    ite (sum >= xcoef) expn (succ expn)
    >>>= fun expn ->
    guardbits' (sum < xcoef) (lsb sum) loss lost_bits
    @@ fun guard round sticky ->
    ite (sum < xcoef) (sum lsr one sigs) sum
    >>>= fun coef ->
    ite (sum < xcoef) (coef lor leading_one sigs) coef
    >>>= fun coef ->
    is_round_up rm xsign (lsb coef) guard round sticky
    >>>= fun up ->
    ite up (succ coef) coef
    >>>= fun coef' ->
    (is_zero coef' && non_zero coef)
    >>>= fun rnd_overflow ->
    ite rnd_overflow (leading_one sigs) coef'
    >>>= fun coef ->
    ite rnd_overflow (succ expn) expn
    >>>= fun _expn' ->
    ite (expn > max_exponent exps) (zero sigs) coef
    >>>= fun coef ->
  ...
  let common_ground xexpn xcoef yexpn ycoef f =
    ...
    ite (xexpn > yexpn) (xexpn - yexpn) (yexpn - xexpn)
    >>>= fun diff ->
    ite (is_zero diff) diff (diff - one exps)
    >>>= fun lost_bits ->
    match_
      [ (xexpn > yexpn)
  ...
  let fsub_finite fsort rm x y =
    ...
    @@ fun xsign xexpn xcoef ->
    unpack fsort y
    @@ fun _ysign yexpn ycoef ->
    bitv_extend xcoef ~addend:1
    @@ fun xcoef sigs' ->
    bitv_extend ycoef ~addend:1
    @@ fun ycoef _ ->
    or_ (xexpn < yexpn) (and_ (xexpn = yexpn) (xcoef < ycoef))
    >>>= fun swap ->
    ite swap (inv xsign) xsign
    >>>= fun sign ->
    ite (xexpn = yexpn && xcoef = ycoef) b0 sign
    >>>= fun sign ->
    common_ground xexpn xcoef yexpn ycoef
    @@ fun loss lost_bits xcoef ycoef ->
    ite (is_zero loss) (zero sigs') (one sigs')
    >>>= fun borrow ->
    ite swap (ycoef - xcoef - borrow) (xcoef - ycoef - borrow)
    >>>= fun coef ->
    msb coef
    >>>= fun msbc ->
    max xexpn yexpn
    >>>= fun expn ->
    ite (xexpn = yexpn) expn (expn - one exps)
    >>>= fun expn ->
    ite (is_zero coef) (min_exponent exps) (ite msbc (succ expn) expn)
    >>>= fun expn ->
    guardbits loss lost_bits
    @@ fun guard' round' sticky' ->
    ite (round' || sticky') (inv guard') guard'
    >>>= fun guard' ->
    ite msbc (lsb coef) guard'
    >>>= fun guard ->
    ite msbc guard' round'
    >>>= fun round ->
    ite msbc (round' || sticky') sticky'
    >>>= fun sticky ->
    ite msbc (coef lsr one sigs') coef
    >>>= fun coef ->
    is_round_up rm sign (lsb coef) guard round sticky
    >>>= fun up ->
    unsigned sigs coef
    >>>= fun coef ->
    ite up (succ coef) coef
  ...
  let fsum_special fsort is_sub x y =
    ...
    let s3 = fsign y in
    let is_sub = s1 lxor (s2 lxor s3) in
    with_special fsort x
    @@ fun ~is_inf:xinf ~is_snan:xsnan ~is_qnan:xqnan ->
    with_special fsort y
    @@ fun ~is_inf:yinf ~is_snan:ysnan ~is_qnan:yqnan ->
    (xsnan || xqnan)
    >>>= fun xnan ->
    (ysnan || yqnan)
    >>>= fun ynan ->
    ...
    >>>= fun is_inf ->
    match_
      [ (is_sub && is_inf) --> qnan fsort; (xinf && yinf) --> x
  ...
  let normalize_coef coef f =
    ...
    coef
    >>-> fun _sort coef ->
    clz !!coef >>>= fun clz -> !!coef lsl clz >>>= fun coef -> f coef clz
  ...
  let fmul_finite fsort rm x y =
    let open B in
    let double e c f =
      double e @@ fun e es -> double c @@ fun c cs -> f e es c cs in
    ...
    @@ fun xsign xexpn xcoef ->
    unpack fsort y
    @@ fun ysign yexpn ycoef ->
    xor xsign ysign
    >>>= fun sign ->
    normalize_coef xcoef
    @@ fun xcoef dx ->
    normalize_coef ycoef
    @@ fun ycoef dy ->
    dx + dy
    >>>= fun dnorm ->
    double xexpn xcoef
    @@ fun xexpn exps' xcoef sigs' ->
    double yexpn ycoef
    @@ fun yexpn _ ycoef _ ->
    of_int sigs' precision
    >>>= fun prec ->
    xexpn + yexpn
    >>>= fun expn ->
    xcoef * ycoef
    ...
    msb coef
    >>>= fun coef_overflowed ->
    ite coef_overflowed (succ expn) expn
    >>>= fun expn ->
    of_int exps' (bias fsort)
    >>>= fun bias ->
    bias + unsigned exps' dnorm
    >>>= fun dexpn ->
    ite (dexpn >=$ expn) (dexpn - expn + of_int exps' mine) (zero exps')
    >>>= fun underflow ->
    underflow > of_int exps' precision
    >>>= fun is_underflow ->
    expn - dexpn + underflow
    >>>= fun expn ->
    expn > of_int exps' maxe
    >>>= fun is_overflow ->
    ite coef_overflowed (one sigs') (zero sigs')
    >>>= fun from_overflow ->
    coef lsr (from_overflow + unsigned sigs' underflow)
    >>>= fun coef ->
    coef lsl one sigs'
    >>>= fun coef ->
    low sigs coef
    ...
    low exps expn
    >>>= fun expn ->
    round rm sign coef loss prec
    @@ fun coef rnd_overflow ->
    ite rnd_overflow (leading_one sigs) coef
    >>>= fun coef ->
    ite rnd_overflow (succ expn) expn
    >>>= fun expn ->
    (expn > of_int exps maxe || is_overflow)
    >>>= fun is_overflow ->
    norm expn coef
    @@ fun expn coef ->
    ite is_underflow (zero exps) expn
    >>>= fun expn ->
    ite is_overflow (ones exps) expn
    >>>= fun expn ->
    ite (is_overflow || is_underflow) (zero sigs) coef
    >>>= fun coef -> pack fsort sign expn coef
  ...
  let fmul_special fsort x y =
    ...
    @@ fun ~is_inf:xinf ~is_snan:xsnan ~is_qnan:xqnan ->
    with_special fsort y
    @@ fun ~is_inf:yinf ~is_snan:_ysnan ~is_qnan:_yqnan ->
    fsign x
    >>>= fun xsign ->
    fsign y
    >>>= fun ysign ->
    ...
    >>>= fun is_inf ->
    match_
      [ (is_zero x && yinf) --> qnan fsort; (is_zero y && xinf) --> qnan fsort
  ...
  let long_division prec nomin denom f =
    ...
    let rec loop i bits nomin =
      if Caml.(i < 0) then
        List.fold bits ~f:( lor ) ~init:(zero sort)
        >>>= fun coef ->
        match_
        ...
        ite (nomin > denom) (mask_bit sort i) (zero sort)
        >>>= fun bit ->
        ite (nomin > denom) (nomin - denom) nomin
        >>>= fun next_nomin ->
        next_nomin lsl one sort
        >>>= fun next_nomin ->
  ...
  let fdiv_finite fsort rm x y =
    ...
    let norm_nominator exps sigs nomin denom f =
      ite (nomin < denom) (nomin lsl one sigs) nomin
      >>>= fun nomin' ->
      ite (nomin < denom) (one exps) (zero exps)
      >>>= fun dexpn -> f nomin' dexpn in
    let exps, sigs = floats fsort in
    ...
    dy - dx
    >>>= fun de ->
    bitv_extend nomin ~addend:1
    @@ fun nomin sigs' ->
    bitv_extend denom ~addend:1
    @@ fun denom _ ->
    norm_nominator exps sigs' nomin denom
    @@ fun nomin dexpn' ->
    long_division prec nomin denom
    @@ fun coef (guard', round', sticky') ->
    unsigned sigs coef
    >>>= fun coef ->
    of_int exps (bias fsort)
    >>>= fun bias ->
    unsigned exps de - dexpn'
    >>>= fun dexpn ->
    xexpn - yexpn
    >>>= fun expn ->
    ...
    >>>= fun is_underflow ->
    (xexpn > yexpn && expn > maxe - dexpn - bias)
    >>>= fun is_overflow ->
    expn + dexpn + bias
    >>>= fun expn ->
    if_ underflowed
      ~then_:
        ( abs expn + mine
        >>>= fun fix_underflow ->
        extract_last coef fix_underflow
        >>>= fun loss ->
        guardbits loss fix_underflow
        @@ fun guard round sticky ->
        (sticky || round' || sticky')
        >>>= fun sticky ->
        coef lsr fix_underflow
        >>>= fun coef ->
        is_round_up rm sign (lsb coef) guard round sticky
        >>>= fun up ->
        ite up (succ coef) coef
  ...
  let fdiv_special fsort x y =
    ...
    @@ fun ~is_inf:xinf ~is_snan:xsnan ~is_qnan:xqnan ->
    with_special fsort y
    @@ fun ~is_inf:yinf ~is_snan:ysnan ~is_qnan:_yqnan ->
    fsign x
    >>>= fun xsign ->
    fsign y
    >>>= fun ysign ->
    (xinf && yinf)
    >>>= fun is_inf ->
    inv (xinf || xsnan || xsnan)
    >>>= fun is_finx ->
    inv (yinf || ysnan || ysnan)
    >>>= fun is_finy ->
    xor xsign ysign
    >>>= fun sign ->
    match_
      ...
      ; (is_zero y && is_finx) --> inf fsort sign; (xinf && yinf) --> qnan fsort
      ; is_inf --> with_sign (xor xsign ysign) x
      ; (xsnan || xqnan) --> transform_to_quite fsort x
      ; anything_else --> transform_to_quite fsort y ]
      ...
      ~else_:
        ...
        if_
          ...
          ~else_:
            ( low (exps fsort') expn
            >>>= fun expn ->
            coef lsl of_int (sigs fsort) sigs_sh
            >>>= fun truncated ->
            high (sigs fsort') truncated
            >>>= fun truncated ->
            high (sigs fsort') coef
            >>>= fun coef ->
            lsb coef
            >>>= fun last ->
            msb truncated
            >>>= fun guard ->
            truncated lsl one (sigs fsort')
            >>>= fun truncated ->
            msb truncated
            >>>= fun round ->
            truncated lsl one (sigs fsort')
            ...
            non_zero truncated
            >>>= fun sticky ->
            is_round_up rm sign last guard round sticky
            >>>= fun up ->
            ite (is_all_ones coef && up) (succ expn) expn
            >>>= fun expn ->
            ite up (succ coef) coef
            >>>= fun coef ->
  ...
  let extend fsort x fsort' =
    ...
    >>>= fun expn ->
    unsigned (sigs fsort') coef
    >>>= fun coef ->
    coef lsl of_int (sigs fsort') d_sigs
    >>>= fun coef -> pack fsort' sign expn coef
  ...
  let gen_cast_float fsort _rmode sign bitv =
    ...
    unsigned sigs clz
    >>>= fun clz ->
    of_int sigs (Bits.size inps) - clz
    >>>= fun msbn ->
    if_ (msbn > prec)
      ~then_:
        ( msbn - prec
        ...
        hi - prec + one sigs
        >>>= fun lo ->
        extract sigs hi lo !!bitv
        >>>= fun coef ->
        extract sigs (pred lo) (zero sigs) !!bitv
        >>>= fun loss ->
        round rne sign coef loss lo
        @@ fun coef rnd_overflow ->
        ite rnd_overflow (not (ones sigs lsr one sigs)) coef
        >>>= fun coef ->
        ite rnd_overflow (succ expn) expn
        >>>= fun expn ->
        expn + unsigned exps de
        >>>= fun expn ->
        norm expn coef @@ fun expn coef -> pack fsort sign expn coef )
      ~else_:
        ( unsigned sigs !!bitv
  ...
  let cast_int (fsort : _ fsort) outs bitv =
    ...
    unpack fsort bitv
    @@ fun sign expn coef ->
    expn - of_int exps bias + one exps
    >>>= fun bits ->
    of_int sigs p - unsigned sigs bits
    >>>= fun bits ->
    coef lsr unsigned sigs bits
    >>>= fun coef ->
  ...
  let range_reduction fsort x f =
    ...
    expn >= low
    >>>= fun increase ->
    ite (expn >= low) (expn - d_expn) (expn + d_expn)
    >>>= fun expn ->
  ...
  let range_reconstruction fsort x d_expn clz increase =
    ...
    if_ (is_zero clz)
      ~then_:
        ( lsb d_expn
        >>>= fun is_odd ->
        ite (is_odd && increase) (succ d_expn) (ite is_odd (pred d_expn) d_expn)
        >>>= fun d_expn ->
        ite increase
        ...
        >>>= fun r -> ite is_odd (idiv fsort rne r sqrt2) r )
      ~else_:
        ( (* for subnormals  *)
        ...
        lsb clz
        >>>= fun is_odd ->
        ite is_odd (pred clz) clz
        >>>= fun clz ->
        clz / of_int (exps fsort) 2
        >>>= fun clz ->
        expn - (d_expn / of_int (exps fsort) 2) - clz
        >>>= fun expn ->
        pack_raw fsort sign expn coef
        >>>= fun r -> ite is_odd (idiv fsort rne r sqrt2) r )
  ...
  let horner fsort x cfs =
    ...
    let rec sum y = function
      | [] -> y
      | c :: cs -> x * y >>>= fun y -> y + c >>>= fun y -> sum y cs in
  ...
  let normalize_subnormal fsort x f =
    ...
    >>>= fun clz ->
    B.(coef lsl clz)
    >>>= fun coef -> pack fsort sign expn coef >>>= fun r -> f r clz
  ...
  let fsqrt fsort rm x =
    ...
    B.ite (is_zero x) (fzero fsort B.b0)
      ( normalize_subnormal fsort x
      @@ fun x clz ->
      range_reduction fsort x
      @@ fun y d_expn increase ->
      fone fsort B.b0
      >>>= fun one ->
      fsub fsort rm y one
      >>>= fun y ->
      extend fsort y fsort'
      >>>= fun y ->
      horner fsort' y (coefs (bits fsort'))
      >>>= fun y ->
      B.unsigned (exps fsort') d_expn
      >>>= fun d_expn ->
      range_reconstruction fsort' y d_expn clz increase
      >>>= fun r -> truncate fsort' r rm fsort )
  ...
  let test fsort x =
    ...
    >>>= fun d_expn ->
    range_reconstruction fsort' y d_expn clz increase
    >>>= fun r -> truncate fsort' r B.rne fsort
end

...
let () =
  ...
  let optim =
    let doc =
      ...
      \      propagation, and elimination of dead temporary (aka virtual) \
       variables." in
    Configuration.(
      parameter Type.(enum optimizations =? o1) ~doc "optimization") in
  let list_passes =
  ...
  let enable_fp_emu =
    Configuration.flag "enable-fp-emulation"
      ~doc:
        "Enable the floating point emulation mode.\n\
    ...
    Configuration.parameters
      ...
      ~doc:
        "Translate the specified instructions into calls to intrinsic \
         ...
         $(b,:unknown,:special,asm:addsd,llvm:trap)." in
  declare ~provides:["bil"; "core-theory"; "lifter"]
  @@ fun ctxt ->
  let open Syntax in
  if ctxt --> list_passes then print_passes ()
    ...
    Theory.declare
      ...
      ~package:"bap.std" ~name:"bil" ~desc:"semantics in BIL"
      ~provides:["bil"; "lifter"] ;
    Theory.declare
      !!(module Bil_semantics.Core_with_fp_emulation : Theory.Core)
      ~package:"bap.std" ~name:"bil-fp-emu" ~extends:["bap.std:bil"]

...
type blk = {name: Theory.Label.t; defs: def term list; jmps: jmp term list}
[@@deriving bin_io]
...
let pp_cfg ppf ir = fprintf ppf "%a" (pp_print_list Blk.pp) (BIR.reify ir)
let inspect cfg = Sexp.Atom (asprintf "%a" pp_cfg cfg)
let null = KB.Symbol.intern "null" Theory.Program.cls
let is_null x = null >>| fun null -> Theory.Label.equal null x

let domain =
  KB.Domain.optional ~inspect "graph" ~equal:(fun x y ->
      Theory.Label.equal x.entry y.entry)

let graph =
  KB.Class.property Theory.Program.Semantics.cls "ir-graph" domain
    ~persistent:
      (KB.Persistent.of_binable
         (module struct type t = cfg option [@@deriving bin_io] end))
    ~package:"bap.std" ~public:true
...
module IR = struct
  ...
  let push fld elt cfg : cfg =
    ...
    | {blks= blk :: blks} -> {cfg with blks= push_to_blk fld blk elt :: blks}

  let fresh = KB.Object.create Theory.Program.cls
  let ( ++ ) b j = push_to_blk jmp b j

  let reify x : cfg knowledge =
    match KB.Value.get graph x with
    | Some g -> KB.return g
    | None -> null >>| fun entry -> {blks= []; entry}
  ...
  let set v x =
    ...
    fresh
    >>= fun tid ->
    data {entry; blks= [{name= entry; jmps= []; defs= [Def.reify ~tid v x]}]}
  ...
  *)
  let repeat cnd body =
    ...
    | {blks= []} ->
        ...
        data
          { entry= head
          ; blks= [{name= head; defs= []; jmps= [goto ~cnd ~tid head]}] }
    | {entry= loop; blks= b :: blks} ->
        ...
        data
          { entry= head
          ; blks=
              (blk tail ++ goto ~tid:jmp1 ~cnd loop)
  ...
  let branch cnd yes nay =
    ...
    | {entry= lhs; blks= b :: blks}, {blks= []} ->
        ...
        ret
          ...
          ; blks=
              blk tail
              :: (blk head ++ jump ~tid:jmp1 lhs ++ goto ~tid:jmp2 tail)
              :: (b ++ goto ~tid:jmp3 tail)
              :: blks }
    | {blks= []}, {entry= rhs; blks= b :: blks} ->
        ...
        ret
          ...
          ; blks=
              blk tail
              :: (blk head ++ jump ~tid:jmp1 tail ++ goto ~tid:jmp2 rhs)
              :: (b ++ goto ~tid:jmp3 tail)
              :: blks }
    | {entry= lhs; blks= yes :: ayes}, {entry= rhs; blks= nay :: nays} ->
        ...
        ret
          ...
          ; blks=
              blk tail
              :: (blk head ++ jump ~tid:jmp1 lhs ++ goto ~tid:jmp2 rhs)
    ...
    | {blks= []}, {blks= []} ->
        ...
        ret
          { entry= head
          ; blks=
              [blk tail; blk head ++ jump ~tid:jmp1 tail ++ goto ~tid:jmp2 tail]
          }
  ...
  let jmp dst =
    ...
    fresh
    >>= fun tid ->
    ctrl {entry; blks= [blk entry ++ Jmp.reify ~tid ~dst:(Jmp.indirect dst) ()]}

  let appgraphs fst snd =
    match (fst, snd) with
    | {entry; blks}, {blks= []} | {blks= []}, {entry; blks} -> ret {entry; blks}
    | {entry; blks= ({jmps= []} as x) :: xs}, {blks= [y]} ->
        ret {entry; blks= {x with defs= y.defs @ x.defs; jmps= y.jmps} :: xs}
    | {entry; blks= x :: xs}, {entry= snd; blks= y :: ys} ->
        fresh
        >>= fun tid ->
        ret {entry; blks= y :: (x ++ goto ~tid snd) :: List.rev_append xs ys}

  let ( >>-> ) x f = x >>= reify >>= f
  let seq fst snd = fst >>-> fun fst -> snd >>-> fun snd -> appgraphs fst snd
  ...
  let do_goto dst =
    ...
    fresh
    >>= fun tid ->
    KB.collect Theory.Label.is_subroutine dst
    >>= fun is_call -> ctrl {entry; blks= [blk entry ++ goto ?is_call ~tid dst]}

  let blk _entry defs jmps =
    defs >>-> fun defs -> jmps >>-> fun jmps -> appgraphs defs jmps

  let goto = do_goto
end

let reify = function None -> [] | Some g -> BIR.reify g

let init () =
  Theory.declare
    !!(module IR : Theory.Core)
    ~package:"bap.std" ~name:"bir"
    ~desc:"Builds the graphical representation of a program." ~provides:["cfg"]

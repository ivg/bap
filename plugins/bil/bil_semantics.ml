...
module Call : sig
  ...
  let prefix = "bil-call:"
  let create name = Bil.special @@ sprintf "%s%s" prefix name
  let dst = function Bil.Special p -> String.chop_prefix ~prefix p | _ -> None
end
...
let inherits slot =
  ...
  KB.Class.property cls ~package name (KB.Slot.domain slot)
...
module Simpl = struct
  ...
  let exp width =
    ...
    and cast t s x =
      ...
      | Cast (t', s', x) when [%compare.equal: cast] t t' && s' > s ->
          Cast (t, s, x)
    ...
    let run : exp -> exp = function
      ...
      | (Let _ | Var _ | Int _ | Unknown (_, _) | Load _ | Store _) as x -> x
    in
    run
end
...
module Basic : Theory.Basic = struct
  ...
  module Base = struct
    ...
    let lift2 mk s f x y =
      x >>-> fun sx x -> y >>-> fun sy y -> mk (s sx sy) (f x y)

    let lift3 mk s f x y z =
      x
      >>-> fun sx x ->
      y >>-> fun sy y -> z >>-> fun sz z -> mk (s sx sy sz) (f x y z)
    ...
    let ite cnd yes nay =
      ...
      yes
      >>-> fun s yes ->
      nay >>-> fun _ nay -> gen s (Bil.ite (bool_exp cnd) yes nay)
    ...
    let branch cnd yes nay =
      ...
      yes
      >>= fun yes ->
      nay >>:= fun nay -> eff Bil.[If (bool_exp cnd, effect yes, nay)]
    ...
    let cast res b x =
      ...
      | 1, Bil.Int b ->
          if Word.(b = b0) then cast UNSIGNED
            ...
            ite (msb !!x) (cast SIGNED)
              (logor (cast UNSIGNED) (mask_high res diff))
      | 1, _ ->
          ite !!b (logor (cast UNSIGNED) (mask_high res diff)) (cast UNSIGNED)
    ...
    let append s ex ey =
      ...
      | _ ->
          if size s < size sx then vec s (Cast (LOW, size s, x))
    ...
    let load mem key =
      ...
      | Some sz -> exp vals Bil.(load mem key BigEndian sz)
      | None -> unk vals
    ...
    let pass = data []
    let skip = ctrl []
    let seq x y = x >>= fun x -> y >>= fun y -> eff (effect x @ effect y)
    let blk _ x y = x >>:= fun x -> y >>:= fun y -> eff (x @ y)
    ...
    let repeat cnd body =
      cnd
      >>= fun cnd -> body >>:= fun body -> data [Bil.While (bool_exp cnd, body)]

    let jmp dst = dst >>= fun dst -> ctrl [Bil.Jmp (bitv_exp dst)]
    let goto lbl = ctrl [Bil.special @@ Format.asprintf "(goto %a)" Tid.pp lbl]
  end
  ...
  let arch lbl =
    KB.collect Arch.slot lbl
    >>= function `unknown -> context >>= KB.collect arch | r -> !!r

  let goto lbl =
    KB.collect Theory.Label.addr lbl
    >>= fun dst ->
    arch lbl
    >>= fun arch ->
    match dst with
    ...
    | _ -> (
        KB.collect Theory.Label.ivec lbl
        >>= function
        | Some ivec -> ctrl Bil.[CpuExn ivec]
        | None -> (
            KB.collect Theory.Label.name lbl
            >>= fun name ->
            ctrl
            @@
            match name with
...
module Core : Theory.Core = struct include Theory.Empty include Basic end
...
module FPEmulator = struct
  ...
  let tmp x f =
    x >>= fun x -> Theory.Var.scoped (sort x) @@ fun v -> let_ v !!x (f (var v))
  ...
  let forder x y =
    ...
    | Some {Theory.IEEE754.k; w; t} ->
        ...
        List.reduce_exn ~f:and_
          [ not_nan !!x; not_nan !!y; inv (and_ (is_zero mx) (is_zero my))
          ; inv (and_ x_is_pos y_is_neg)
          ; or_ (and_ x_is_neg y_is_pos) (ite x_is_neg (ult my mx) (ult mx my))
          ]
end

module Core_with_fp_emulation = struct include Core include FPEmulator end

...
let eval ?(name = "") ~expected test _ctxt =
  ...
  let float64_bits w =
    ...
    float64_bits (Int64.float_of_bits x)
  in
  match exp test with
  | None -> assert false
  | Some e -> (
...
let sqrt_ x ctxt =
  let name =
    sprintf "sqrt %g %Lx %s" x (Int64.bits_of_float x) (float64_bits x)
  in
  let expected = Float.sqrt x |> word_of_float in
...
let random_float () =
  ...
  let make () =
    let expn = expn () in
    let frac = frac () in
    make_float (sign ()) expn frac
  in
  let small () =
    let x = Random.int 42 in
    let y = Int64.of_int x in
    Random.float (Int64.float_of_bits y)
  in
...
let convert_128_to_64 x expected _ctxt =
  ...
  | Some e -> (
      let check =
        ...
        let r = Primus.Value.to_word r in
        let equal = Word.(r = expected) in
        assert_bool "convert_64_to_32" equal
      in
      match Main.run proj check with
      | Primus.Normal, _ -> ()
      | _ -> raise (Failure "Something went wrong") )

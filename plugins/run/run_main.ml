...
let callgraph start prog =
  ...
  let connect_unreachable_scc g =
    Graphlib.depth_first_search
      (module Callgraph)
      g ~start ~init:g ~start_tree:mark_as_root in
...
let all_subroutines ?(marked = false) prog =
  ...
  let marked_non_root_to_entry t =
    if Set.mem roots t || Set.mem unmarked t then None else Some (`tid t) in
...
let is_visited = function
  | `tid tid ->
      Machine.Global.get visited
      >>= fun subs -> Machine.return (Set.mem subs tid)
...
let main {Config.get= ( ! )} proj =
  ...
  let enqueue_jobs =
    if !in_isolation then enqueue_separate_jobs else enqueue_super_job in
  ...
  let rec run proj state =
    ...
    if run_until_visited then
      ...
      | Some sub -> enqueue_term sub ; run proj state
      | None -> (
        match find_first_unvisited_blk prog with
        | Some blk -> enqueue_term blk ; run proj state
        | None -> proj )
    else proj in
  run proj state

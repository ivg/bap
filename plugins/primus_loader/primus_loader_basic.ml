...
module Make (Param : Param) (Machine : Primus.Machine.S) = struct
  ...
  let make_word addr =
    Machine.arch >>| Arch.addr_size
    >>| fun size -> Addr.of_int64 ~width:(Size.in_bits size) addr
  ...
  let setup_stack () =
    target
    >>= fun (module Target) ->
    make_word stack_base
    >>= fun bottom ->
    let top = Addr.(bottom -- stack_size) in
    Val.of_word bottom
    >>= fun bottom ->
    Env.set Target.CPU.sp bottom
    >>= fun () -> Mem.allocate ~readonly:false ~executable:false top stack_size
  ...
  let segmentations =
    ...
    Ogre.foreach Ogre.Query.(select (from segment)) ~f:ident
    ...
    |> Machine.Seq.fold ~init:null ~f:(fun endp (mem, tag) ->
           ...
           | Some seg ->
               ...
               let update_ends =
                 ...
                 | _ -> Machine.return () in
               alloc mem
               >>= fun () -> update_ends >>| fun () -> Addr.max endp maddr)
  ...
  let save_args array ptr =
    ...
    |> Machine.Seq.fold ~init:(ptr, []) ~f:(fun (ptr', ptrs) str ->
           save_string str ptr' >>= save_string "\x00"
           >>| fun ptr -> (ptr, ptr' :: ptrs))
  ...
  let save_word endian word ptr =
    ...
    |> Machine.Seq.fold ~init:ptr ~f:(fun ptr byte ->
           Mem.store ptr byte >>| fun () -> Word.succ ptr)
  ...
  let setup_main_frame () =
    ...
    let argc =
      Array.length argv |> Word.of_int ~width:(Size.in_bits addr_size) in
    ...
    let total_size =
      bytes_in_addr
      + (* argc *)
      table_size argv + table_size envp + frame_size argv + table_size envp
    in
    let argv_frame_ptr =
      bytes_in_addr + table_size argv + table_size envp |> Addr.nsucc sp in
    let argv_table_ptr = Addr.nsucc sp bytes_in_addr in
    Mem.allocate ~readonly:false ~executable:false sp total_size
    >>= fun () ->
    ...
    save_word endian argc sp >>= fun _ -> set_word "environ" envp_table_ptr
  ...
  let init () =
    ...
    >>= fun () ->
    set_word "brk" endp
    >>= fun () -> setup_registers () >>= fun () -> init_names ()
end

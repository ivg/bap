...
module IsConst (Machine : Primus.Machine.S) = struct
  ...
  let run inputs =
    Machine.Local.get state >>| all_consts inputs
    >>= function true -> Value.b1 | false -> Value.b0
end
...
module Tracker (Machine : Primus.Machine.S) = struct
  ...
  let init () =
    Machine.sequence
      Primus.Interpreter.
        [ const >>> intro; binop >>> prop_binop; unop >>> prop_unop
        ; cast >>> prop_cast; extract >>> prop_extract; concat >>> prop_concat
        ]
end

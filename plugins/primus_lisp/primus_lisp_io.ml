...
type state = {redirections: string String.Map.t; channels: channel Int.Map.t}

let default_channels =
  Int.Map.of_alist_exn
    [ (0, {input= Some In_channel.stdin; output= None})
...
let init_channels =
  List.fold ~init:default_channels ~f:(fun chans (chan, name) ->
      ...
      | Some fd ->
          ...
          let chan =
            if fd > 0 then {input= None; output= Some (Out_channel.create name)}
            else {output= None; input= Some (In_channel.create name)} in
...
let init redirections =
  let module Lib (Machine : Primus.Machine.S) = struct
    ...
    let value_to_fd fd =
      Value.to_word fd |> Word.to_int
      |> function Error _ -> None | Ok n -> Some n

    let string_of_charp ptr =
      let rec loop chars ptr =
        Eval.load ptr LittleEndian `r8
        >>= fun c ->
        let c =
          (* [load p e `r8] must return a byte *)
          Char.of_int_exn @@ Word.to_int_exn @@ Value.to_word c in
        if Char.(c = '\000') then
    ...
    let run [path] =
      ...
      | Some path -> (
        ...
        | Ok channel ->
            ...
            >>= fun () -> addr_width >>= fun width -> Value.of_int ~width fd )
  end in
    ...
    let run [fd] =
      ...
      | Some fd ->
          ...
          if Map.mem s.channels fd then
            Machine.Local.put state {s with channels= Map.remove s.channels fd}
            >>= fun () -> ok
          else error
  end in
  ...
  let module Primitives (Machine : Primus.Machine.S) = struct
    ...
    let setup_standard_channels =
      ...
      Machine.sequence
        [ set "*standard-input*" 0; set "*standard-output*" 1
        ; set "*error-output*" 2; (* CL name *) set "*standard-error*" 2
          (* conventional name *) ]
    ...
    let init () =
      ...
      Machine.sequence
        [ setup_standard_channels; setup_redirections
        ; def "channel-open"
            ((one int // all int) @-> int)
            (module Open)

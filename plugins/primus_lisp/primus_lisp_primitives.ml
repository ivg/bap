...
let registers =
  ...
  |> Seq.fold ~init:Int.Map.empty ~f:(fun regs (_, insn) ->
         Insn.ops insn
         |> Array.fold ~init:regs ~f:(fun regs ->
              function
              | Op.Reg r -> Map.set regs (Reg.code r) (Reg.name r) | _ -> regs))
...
module Closure (Machine : Primus.Machine.S) = struct
  ...
  let exec_addr addr =
    Linker.exec (`addr (Primus.Value.to_word addr))
  ...
  let is_zero =
    all @@ fun x -> Value.zero (Value.bitwidth x) >>= fun z -> Eval.binop EQ x z
  ...
  let extract hi lo x =
    to_int hi >>= fun hi -> to_int lo >>= fun lo -> Eval.extract ~hi ~lo x

  let symbol_concat syms =
    Machine.List.map syms ~f:Value.Symbol.of_value
    >>= fun strs -> Value.Symbol.to_value (String.concat strs)

  let set_value reg x =
    Value.Symbol.of_value reg
    >>= fun reg ->

...
let reglist_to_map regs =
  List.fold ~init:String.Map.empty
    ~f:(fun regs var -> String.Map.set regs (Var.name var) var)
    regs
  ...
  List.fold ~init:String.Map.empty
    ~f:(fun regs i ->
      ...
      | Some a ->
          ...
          String.Map.set regs name var)
    range

let make_regs_i typ prefix range =
  List.fold ~init:Int.Map.empty
    ~f:(fun regs i -> Int.Map.set regs i (make_var_i typ prefix i))
    range
...
module Make_MIPS (S : Spec) : MIPS = struct
  ...
  let gprs =
    [ (make_gpr "ZERO", "R0"); (make_gpr "AT", "R1"); (make_gpr "V0", "R2")
    ; (make_gpr "V1", "R3"); (make_gpr "A0", "R4"); (make_gpr "A1", "R5")
    ; (make_gpr "A2", "R6"); (make_gpr "A3", "R7"); (make_gpr "T0", "R8")
    ; (make_gpr "T1", "R9"); (make_gpr "T2", "R10"); (make_gpr "T3", "R11")
    ; (make_gpr "T4", "R12"); (make_gpr "T5", "R13"); (make_gpr "T6", "R14")
    ; (make_gpr "T7", "R15"); (make_gpr "S0", "R16"); (make_gpr "S1", "R17")
    ; (make_gpr "S2", "R18"); (make_gpr "S3", "R19"); (make_gpr "S4", "R20")
    ; (make_gpr "S5", "R21"); (make_gpr "S6", "R22"); (make_gpr "S7", "R23")
    ; (make_gpr "T8", "R24"); (make_gpr "T9", "R25"); (make_gpr "K0", "R26")
    ; (make_gpr "K1", "R27"); (make_gpr "GP", "R28"); (make_gpr "SP", "R29")
    ; (make_gpr "FP", "R30"); (make_gpr "RA", "R31") ]
  ...
  let gpr =
    List.fold gprs ~init:String.Map.empty ~f:(fun init (reg, alias) ->
        ...
        let names =
          if gpr_bitwidth = 64 then (name ^ "_64") :: names else names in
        List.fold names ~init ~f:(fun regs name -> Map.set regs name reg))

  let gpri =
    List.foldi ~init:Int.Map.empty
      ~f:(fun n regs (reg, _) -> Map.set regs n reg)
      gprs
...
module Spec32 = struct let gpr_bitwidth = 32 let addr_size = `r32 end
module Spec64 = struct let gpr_bitwidth = 64 let addr_size = `r64 end
...
module Make_cpu (M : MIPS) : CPU = struct
  ...
  let gpr =
    ...
    List.fold data ~init:Var.Set.empty ~f:(fun regs v -> Var.Set.add regs v)

  let sp =
    Var.Set.find_exn gpr ~f:(fun v ->
        String.is_prefix ~prefix:"SP" (Var.name v))

  let fp =
    Var.Set.find_exn gpr ~f:(fun v ->
        String.is_prefix ~prefix:"FP" (Var.name v))

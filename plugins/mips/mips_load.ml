...
let ld cpu ops =
  ...
  let base = signed cpu.reg ops.(1) in
  let off = signed imm ops.(2) in
  RTL.[rt := cpu.load (base + off) doubleword]
...
let ldpc cpu ops =
  ...
  RTL.[rt := cpu.load (cpu.cia + (off lsl unsigned const byte 3)) doubleword]
...
let lh cpu ops =
  ...
  let base = signed cpu.reg ops.(1) in
  let off = signed imm ops.(2) in
  RTL.[rt := cpu.load (base + off) halfword]
...
let lwpc cpu ops =
  let rt = signed cpu.reg ops.(0) in
  let off = signed imm ops.(1) in
  RTL.[rt := cpu.load (cpu.cia + (off lsl unsigned const byte 2)) word]

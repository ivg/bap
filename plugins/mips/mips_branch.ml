...
let bal cpu ops =
  ...
  RTL.[rs := cpu.cia + step; cpu.jmp (cpu.cia + off)]
...
let beq cpu ops =
  ...
  RTL.[when_ (rs = rt) [cpu.jmp (cpu.cia + off)]]
...
let bgez cpu ops =
  let rs = signed cpu.reg ops.(0) in
  let off = signed imm ops.(1) in
  RTL.[when_ (rs >= zero) [cpu.jmp (cpu.cia + off)]]
...
let bgezal cpu ops =
  ...
  let step = unsigned const byte 8 in
  RTL.
    [when_ (rs >= zero) [cpu.gpr 31 := cpu.cia + step; cpu.jmp (cpu.cia + off)]]

(* BLTZALC rt, offset *)
let bltzalc cpu ops =
  let rt = signed cpu.reg ops.(0) in
  let off = signed imm ops.(1) in
  RTL.[when_ (rt < zero) [cpu.jmp (cpu.cia + off)]]
...
let jal cpu ops =
  let target = signed imm ops.(0) in
  let step = unsigned const byte 8 in
  RTL.[cpu.gpr 31 := cpu.cia + step; cpu.jmp target]
...
let jalr cpu ops =
  ...
  let step = unsigned const byte 8 in
  RTL.[rd := cpu.cia + step; cpu.jmp rs]
...
let jialc cpu ops =
  ...
  let off = signed imm ops.(1) in
  let step = unsigned const byte 8 in
  RTL.[cpu.gpr 31 := cpu.cia + step; cpu.jmp (rt + off)]
...
let bne cpu ops =
  ...
  let off = signed imm ops.(2) in
  RTL.[when_ (lnot (ra = rb)) [cpu.jmp (cpu.cia + off)]]

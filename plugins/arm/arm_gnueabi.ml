...
let align ncrn t =
  if Size.equal (size#alignment t) `r64 then
    match ncrn with
    | [ _; _; _; _ ] -> ncrn
    | [ _; r2; r3 ] -> [ r2; r3 ]
    | _ -> []
...
let retregs = function
  ...
  | t -> (
      match size#bits (t :> C.Type.t) with
...
let args _ _ { C.Type.Proto.return; args = ps } =
  ...
  let _, _, params =
    List.fold ps ~init:(regs, mems, []) ~f:(fun (regs, mems, args) (n, t) ->
        ...
        (regs, mems, (C.Abi.data size t, concat (exps @ rest)) :: args))
  in
  Some C.Abi.{ return; hidden; params = List.rev params }

let abi = C.Abi.{ insert_args = args; apply_attrs = (fun _ -> ident) }

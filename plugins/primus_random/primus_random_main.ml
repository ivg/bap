...
module Generator = struct
  ...
  type generator =
    {predicate: predicate; distribution: name; parameters: string list}
  ...
  let predicate_of_sexp : Sexp.t -> predicate = function
    ...
    | List [Atom "mem"; Atom lower; Atom upper] -> Mem (Interval (lower, upper))
    | other ->
  ...
  let of_sexp : Sexp.t -> generator = function
    ...
    | List ((Atom ("static" | "random") as dis) :: ps) ->
        {predicate= Any; distribution= name_of_sexp dis; parameters= atoms ps}
    | List (pred :: dis :: pars) ->
    ...
    | xs ->
        ...
        Primus.Generator.of_iterator
          ( module struct
            ...
            let next i = i
            let value i = xs.(i mod Array.length xs) end )
          0 ~to_bitvec:ident ?width
...
let main ctxt =
  ...
  let module RandomizeMemory (Machine : Primus.Machine.S) = struct
    ...
    let allocate ?upper ~width lower generator =
      ...
      | Some upper ->
          eval upper
          >>= fun upper -> Memory.add_region ~generator ~lower ~upper ()
    ...
    *)
    ...
    let map_page ?generator already_mapped addr =
      let rec map len =
        ...
        already_mapped last
        >>= function
        | true -> map (len / 2) | false -> Memory.allocate ?generator addr len
      in
  ...
  let module PertubeOutputs (Machine : Primus.Machine.S) = struct
    ...
    let pertube gen : exp -> unit Machine.t = function
      ...
      | Load (_, ptr, endian, size) ->
          ...
          >>= Val.of_word
          >>= fun value -> Eval.store ptr value endian size
      | exp ->
          Exp.free_vars exp |> Set.to_sequence
          |> Machine.Seq.iter ~f:(pertube_var gen)

    let exec =
      Debug.msg "the unresolved handler is called"
      >>= fun () ->
      Eval.pos
      >>= function
      | Primus.Pos.Jmp {up= {me= blk; up= {me= sub}}} when is_entry sub blk ->
          Machine.Local.get args
          >>= fun {args} ->
          Term.enum arg_t sub
          |> Machine.Seq.iter ~f:(fun arg ->
                 match Map.find args (Arg.lhs arg) with
                 | Some gen ->
                     Debug.msg "pertubing argument %a" Arg.pp arg
                     >>= fun () -> pertube gen (Arg.rhs arg)
                 | None ->

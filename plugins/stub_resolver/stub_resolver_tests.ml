...
let block_of_bytes addr b =
  ...
  let dis =
    Or_error.ok_exn @@ Dis.create ~backend:"llvm" (Arch.to_string `x86_64) in
...
let provide_stubs syms =
  ...
  KB.promise (Value.Tag.slot Sub.stub)
  @@ fun label ->
  KB.collect Theory.Label.name label
  >>| function Some name when Set.mem stubs name -> Some () | _ -> None

let provide_aliases syms =
  KB.promise Theory.Label.aliases
  @@ fun label ->
  KB.collect Theory.Label.name label
  >>| function
...
let create_program syms =
  ...
  provide_stubs syms ; provide_aliases syms ; prog

let tid_for_name_exn prog name =
  Term.to_sequence sub_t prog
  |> Seq.find_map ~f:(fun s ->
         if Sub.name s = name then Some (Term.tid s) else None)
  |> Option.value_exn
...
let run name symbols expected should_fail _ctxt =
  let syms =
    List.fold symbols
      ...
      ~f:(fun syms (name, data) -> Map.add_exn syms name data) in
...
let suite =
  ...
  >::: [ test "simple case: we have pairs"
       ...
       ; test "simple case: no pairs" [real "b0" []; stub "b1" []] ~expected:[]
       ...
       ; test "crossreference"
           ...
           ~expected:[("j1", "j0")]
       ; test "many pairs"
           [ real "k0" []; real "k1" []; real "k2" []; stub "k3" ["k0"]
           ; stub "k4" ["k1"]; stub "k5" ["k2"] ]
           ~expected:[("k3", "k0"); ("k4", "k1"); ("k5", "k2")]
       ; test "several intersections 1"
           [ stub "m0" ["m2"; "m3"; "m4"]; real "m1" ["m0"]
           ; stub "m5" ["m6"; "m7"; "m8"]; real "m6" ["m5"; "m9"]
           ; real "m9" ["m10"]; stub "m10" [] ]
           ~expected:[("m0", "m1")]
       ; test "several intersections 2"
           [ stub "n0" ["n1"; "n2"; "n3"]; real "n1" []; real "n4" ["n5"]
           ; stub "n5" []; real "n6" []; stub "n7" ["n6"]
           ; real "n8" ["n1"; "n5"] ]
           ~expected:[("n7", "n6")]
       ; test "several intersections 3"
           [ stub "p0" ["p1"; "p2"; "p3"]; stub "p4" ["p5"; "p6"; "p7"]
           ; real "p5" []; stub "p6" ["p8"; "p9"; "p10"; "p4"]
           ; real "p11" ["p12"; "p13"; "p1"] ]
           ~expected:[("p0", "p11")] ]

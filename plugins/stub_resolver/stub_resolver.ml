...
type t = {groups: groups; names: names; next: int; stubs: Tid.Set.t}

let tids =
  Knowledge.Domain.mapping
    (module Tid)
    "tids" ~equal:Tid.equal ~inspect:sexp_of_tid

let slot =
  Knowledge.Class.property Theory.Program.cls "stubs" tids
    ~persistent:
      (Knowledge.Persistent.of_binable
         (module struct type t = tid Tid.Map.t [@@deriving bin_io] end))
    ~public:true ~desc:"The mapping from stubs to real symbols"
...
let is_stub sub =
  KB.collect (Value.Tag.slot Sub.stub) (Term.tid sub)
  >>= function None -> KB.return false | Some () -> KB.return true
...
let update_stubs t sub =
  is_stub sub
  >>| fun is_stub ->
  if is_stub then {t with stubs= Set.add t.stubs (Term.tid sub)} else t

let find_groups names aliases =
  Map.fold names ~init:[] ~f:(fun ~key:group ~data:aliases' groups ->
      if Set.(is_empty @@ inter aliases aliases') then groups
      else group :: groups)

let unite_names t groups =
  List.fold groups
    ~init:(Set.empty (module String))
    ~f:(fun als id -> Set.union als (Map.find_exn t.names id))

let pick_representative = function
  | [] -> assert false
  | groups -> List.min_elt groups ~compare:Int.compare |> Option.value_exn

let redirect t ~from ~to_ =
  Map.map t.groups ~f:(fun id ->
      if List.mem from id ~equal:Int.equal then to_ else id)
...
let add t sub =
  ...
  | [id] ->
      ...
      let names =
        Map.update t.names id ~f:(function
          | None -> assert false
...
let collect_by_group_id groups =
  Map.fold groups ~init:Int.Map.empty ~f:(fun ~key:tid ~data:id xs ->
      Map.update xs id ~f:(function None -> [tid] | Some tids -> tid :: tids))
...
let find_pairs t = collect_by_group_id t.groups |> unambiguous_pairs t.stubs

let resolve prog =
  Knowledge.Seq.fold ~init:empty (Term.to_sequence sub_t prog) ~f:add
  >>| find_pairs

let provide prog =
  Knowledge.Object.create cls
  >>= fun obj ->
  resolve prog
  >>= fun links -> KB.provide slot obj links >>= fun () -> KB.return obj

...
module Plt = struct
  ...
  let section_memory mem sec_name =
    let collect_addresses addrs (mem, _) =
      Memory.foldi mem ~word_size:`r8 ~init:addrs ~f:(fun addr _ acc ->
          Set.add acc (Word.to_bitvec addr)) in
  ...
  let provide mem =
    ...
    >>| function Some addr when Set.mem stubs addr -> Some () | _ -> None
end

let update prog = relink prog (Stub_resolver.run prog)
let main proj = Project.with_program proj (update @@ Project.program proj)

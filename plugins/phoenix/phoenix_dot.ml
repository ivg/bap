...
struct
  ...
  module Dottable = struct
    ...
    module V = struct type t = block * string end
    ...
    let default_vertex_attributes gr =
      [ `Shape `Box; `Fontsize 14; `Fontcolor 0x666699; `Fontname "Monospace"
      ; `Width 1.0 ]
    ...
    let edge_attributes (_, kind, _, arity) =
      ...
      [ `Color color; `Arrowsize 0.5; `Headport `N; `Tailport `S
      ; `Labelfloat true; `Weight weight ]
  end

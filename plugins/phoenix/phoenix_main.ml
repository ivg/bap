...
let labels_with : _ Config.param =
  ...
  let possibilities =
    [("name", `with_name); ("asm", `with_asm); ("bil", `with_bil)] in
  Config.(
...
let man =
  ...
  ; `P
      ...
      \      target file will be used as a directory name."; `S "SEE ALSO"
  ...
  Config.when_ready (fun {Config.get= ( ! )} ->
      let cfg_format =
        let deprecated_options =
          [ (`with_name, !with_name); (`with_asm, !with_asm)
          ; (`with_bil, !with_bil) ]
          |> List.filter ~f:snd |> List.map ~f:fst in
        deprecated_options @ !labels_with in
      let options =
        create !output_folder cfg_format !no_resolve !keep_alive !no_inline
          !keep_consts !no_optimizations in

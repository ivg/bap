...
struct
  ...
  let store_index () =
      ...
      Ezjsonm.(
        dict
          ...
          ; ( "id"
            ...
            ); ("expanded", bool false); ("function", bool is_func)
    ...
    let children =
      `O
        Ezjsonm.
          [ ("text", string filename); ("id", string (path root))
          ; ("expanded", bool true); ("root", bool true); ("children", `A funcs)
          ] in
    let document =
      `O Ezjsonm.[("text", string "."); ("children", `A [children])] in
  ...
  let store_functions () =
    ...
    let func min max name =
      Ezjsonm.(
        dict
          [ ("name", string name); ("startAddress", address Memory.min_addr min)
          ; ("endAddress", address Memory.max_addr max)
          ; ("hash", string "0000000000000000000") ]) in
    let functions =
      Symtab.to_sequence symbols
    ...
    let document =
      Ezjsonm.(
        `O
          [ ( "binary"
            , `O
                [ ("name", string filename); ("functionNumber", int amount)
                ; ("functions", `A functions) ] ) ]) in

...
let parse_func_start =
  ...
  let parse_addr input ~start ~stop =
    Z.of_substring_base 16 input ~pos:start ~len:(stop - start)
  in
  fun input ~accept ->
    try
...
let provide_objdump demangler file =
  ...
  let symbolizer =
    ...
    Hashtbl.find funcs @@ Bitvec.to_bigint (Word.to_bitvec addr)
  in
...
let () =
  let demangler =
    ...
    in
    Config.(param ~doc (some string) "demangler")
  in
  Config.manpage
    [
      `S "DESCRIPTION";

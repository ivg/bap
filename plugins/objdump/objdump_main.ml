...
let objdump_cmds demangler =
  objdump :: List.map targets ~f:(fun p -> p ^ "-objdump")
...
let parse_func_start =
  ...
  fun input ~accept ->
    try
      ...
      let addr =
        parse_addr input ~start:(Re.Group.start groups 1)
...
let provide_objdump demangler file =
  let funcs =
    Hashtbl.create
      ( module struct
        ...
        let sexp_of_t x = Sexp.Atom (Z.to_string x) end ) in
  ...
  let symbolizer =
    Symbolizer.create
    @@ fun addr -> Hashtbl.find funcs @@ Bitvec.to_bigint (Word.to_bitvec addr)
  in
  Symbolizer.provide agent symbolizer ;
  provide_roots funcs

let main demangler =
  Stream.observe Project.Info.file @@ provide_objdump demangler
...
let () =
  ...
  Config.manpage
    [ `S "DESCRIPTION"
    ; `P
        "This plugin provides a symbolizer based on objdump. Note that we \
         parse objdump output, thus this symbolizer is potentially fragile to \
         changes in objdumps output."; `S "EXAMPLES"
    ; `P "To view the symbols after running the plugin:"
    ; `P "$(b, bap) $(i,executable) --dump-symbols "
    ; `P "To view symbols without this plugin:"
    ; `P "$(b, bap) $(i,executable) --no-objdump --dump-symbols"; `S "SEE ALSO"

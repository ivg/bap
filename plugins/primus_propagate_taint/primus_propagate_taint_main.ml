...
let mapper =
  Primus.Machine.State.declare ~name:"primus-taint-mapper"
    ~uuid:"12511d75-e626-4b0d-8dcd-2a3b597992cf" (fun _ ->
      {regs= Tid.Map.empty; ptrs= Tid.Map.empty; tids= Taint.Object.Map.empty})
...
module Marker (Machine : Primus.Machine.S) = struct
  ...
  let mark _ =
    ...
    Machine.update (fun proj ->
        Project.program proj |> mark_terms s |> Project.with_program proj)

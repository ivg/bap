...
module Computation (Machine : Primus.Machine.S) = struct
  open Taint.Rel
  module Id = struct let name = "propagate-by-computation" end
  ...
  let init () =
    Machine.sequence
      Primus.
        [ Interpreter.loaded >>> loaded; Interpreter.stored >>> stored
        ; Interpreter.binop >>> binop; Interpreter.unop >>> unop
        ; Interpreter.extract >>> extract; Interpreter.concat >>> concat
        ; Interpreter.cast >>> cast ]
end

module Exact (Machine : Primus.Machine.S) = struct
  open Taint.Rel
  module Id = struct let name = "propagate-exactly" end
  module Eval = Primus.Interpreter.Make (Machine)
  module Taint = Taint.Tracker.Make (Machine)
  module Value = Primus.Value.Make (Machine)
  module Support = Support (Id) (Machine)

...
module Get (Machine : Primus.Machine.S) = struct
  ...
  let run rel [k; v] =
    Kind.of_value k
    >>= fun k -> Tracker.lookup v rel >>= optional taint first_of_kind k
end
...
module Sanitize (Machine : Primus.Machine.S) = struct
  ...
  let run rel [k; v] =
    Kind.of_value k >>= fun k -> Tracker.sanitize v rel k >>| fun () -> v
end
...
module PolicySelect (Machine : Primus.Machine.S) = struct
  ...
  let run [k; p] =
    Kind.of_value k
    >>= fun k ->
    Policy.of_value p >>= fun p -> Policy.select p k >>= fun () -> nil
end
...
module Signals (Machine : Primus.Machine.S) = struct
  ...
  let init () =
    Lisp.signal ~params ~doc Taint.Gc.taint_finalize
    @@ fun (t, live) -> Machine.List.all [Object.to_value t; Value.of_bool live]
end
...
let set_default_policy name =
  let module Init (Machine : Primus.Machine.S) = struct
    ...
    let init () =
      ...
      >>= fun v -> Policy.of_value v >>= Policy.set_default
  end in
...
;;
manpage
  [ `S "DESCRIPTION"; `P "The Primus Taint Analysis Framework Control Module"
  ; `P
      ...
      \    provides several useful components."; `S "Taint Propagation Policies"
  ; `P
...
let () =
  when_ready (fun {get= ( !! )} ->
      ...
      Primus.Components.register_generic "taint-primitives"
        ...
        ~package:"bap"
        ~desc:
          "Exposes Primus Lisp primitives that controls the Primus Taint \
           Engine." ;

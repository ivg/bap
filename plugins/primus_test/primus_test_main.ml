...
module Location = struct
  ...
  *)
  ...
  type trace =
    {trace: point list; switches: (int * id) list; initial: id; length: int}
  ...
  let trace =
    Primus.Machine.State.declare ~uuid:"3663b645-87a6-4561-b75f-c447cdc221bc"
      ~name:"program-trace"
    @@ fun _ -> {trace= []; switches= []; initial= {id= "unknown"}; length= 0}

  let state =
    Primus.Machine.State.declare ~uuid:"22d66ae9-82ad-4185-a1f8-5eb99f5569df"
      ~name:"incident-locations"
    @@ fun _ -> {incidents= String.Map.empty; locations= Primus.Value.Map.empty}

  let push pc machine t =
    let pc = Word.to_bitvec pc and machine = Id.to_string machine in
    match t.trace with
    | [] -> {trace= [pc]; initial= {id= machine}; switches= []; length= 1}
    | _ ->
        let current =
          match t.switches with [] -> t.initial.id | (_, {id}) :: _ -> id in
        let t =
          if Bitvec.equal (List.hd_exn t.trace) pc then t
          else {t with trace= pc :: t.trace; length= t.length + 1} in
  ...
  let classify {incidents; locations} name traces ~new_representative
    ...
    | Some classes -> (
        ...
        |> function None -> new_class cls' | Some other -> other )
...
module Incident = struct
  ...
  module Report (Machine : Primus.Machine.S) = struct
    ...
    let run (name :: locations) =
      ...
      >>= fun name ->
      !!report (name, locations)
      >>= fun () ->
      Machine.Global.get Location.state
      >>= fun s ->
      Location.classify s name locations
        ~new_representative:(fun id cls ->
          !!report_new_instance (name, id, locations)
          >>= fun () ->
          !!report_new_representative (name, id, locations)
          >>= fun () ->
          Machine.Global.update Location.state (Location.set_cls name id cls))
        ~new_instance:(fun id -> !!report_new_instance (name, id, locations))
        ~new_class:(fun cls ->
          next name s.incidents
          >>= fun id ->
          !!report_new_class (name, id, locations)
          >>= fun () ->
          !!report_new_instance (name, id, locations)
          >>= fun () ->
          Machine.Global.update Location.state (Location.set_cls name id cls))
...
;;
manpage [`S "DESCRIPTION"; `P "Primus Testing and Program Verification module."]
;;

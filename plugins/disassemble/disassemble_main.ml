...
let features_used =
  [ "disassembler"; "lifter"; "symbolizer"; "rooter"; "reconstructor"; "brancher"
  ; "loader" ]
...
let run_passes base proj =
  Err.List.fold ~init:(0, proj) ~f:(fun (step, proj) pass ->
      ...
      Project.Pass.run pass proj |> pass_error
      >>= fun proj -> Ok (step + 1, proj))
...
let project_state_cache () =
  let module State = struct type t = Project.state [@@deriving bin_io] end in
...
let process passes outputs project =
  ...
  run_passes 0 project autoruns
  >>= fun (step, proj) ->
  run_passes step proj passes
  >>| fun (_, proj) ->
...
let pp_guesses ppf badname =
  let guess =
    String.map badname ~f:(function '_' -> '-' | c -> Char.lowercase c) in
  let suffix = "-" ^ name in
  let good_guess name =

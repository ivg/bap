...
let print_bir patterns sema ppf proj =
  let pp =
    match sema with None -> Program.pp | Some cs -> Program.pp_slots cs in
  ...
  let pp_memmap ppf memmap =
    let pp_region ppf mem =
      pr ppf "Region(%a,%a)" pp_word (Memory.min_addr mem) pp_word
        (Memory.max_addr mem) in
    let pp_binding ppf (mem, attr) =
      pr ppf "Annotation(%a,@ %a)" pp_region mem pp_attr attr in
...
let pp_bir_cfg ~labeled name ppf g =
  ...
  let nodes_of_edge e = Cfg.(Node.label (Edge.src e), Node.label (Edge.dst e)) in
  let node_label, edge_label =
    if labeled then (Some node_label, Some edge_label) else (None, None) in
  ...
  let entries =
    ...
    |> Seq.fold
         ...
         ~f:(fun entries sub ->
           ...
           | None -> entries
           | Some entry -> Map.add_exn entries (Term.tid sub) (Term.tid entry))
  in
  fprintf ppf "digraph {@\nnode[shape=box];@\n" ;
  Term.enum sub_t prog
  |> Seq.iter ~f:(fun sub ->
...
let setup_tabs ppf = pp_print_as ppf 50 "" ; pp_set_tab ppf ()
...
let main attrs ansi_colors demangle symbol_fmts subs secs patterns doms =
  ...
  let pp_cfg =
    Data.Write.create ~pp:(print_bir_graph ~labeled:true patterns) () in
  let pp_calls =
    Data.Write.create ~pp:(print_bir_graph ~labeled:false patterns) () in
...
let () =
  let () =
    Config.manpage
      ...
      ; `P
          ...
          \         $(b,bap-plugin-dump-symbols)(1)" ] in
  ...
  let print_symbols : _ list Config.param =
    let opts =
      [("name", `with_name); ("addr", `with_addr); ("size", `with_size)] in
    let doc =
      sprintf
  ...
  let semantics : string list option Config.param =
    ...
    Config.(
      param
        (some (list string))
        ~as_flag:(Some []) ~doc ~docv:"SEMANTICS-LIST" "semantics") in

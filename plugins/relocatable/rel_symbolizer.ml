...
module Rel = struct
  ...
  let external_symbols =
    ...
    Fact.return
      ( of_aseq
      @@ Seq.map s ~f:(fun (addr, data) -> (Addr.of_int64 ~width addr, data)) )
end

let agent =
  Knowledge.Agent.register ~package:"bap.std" "rel-symbolizer"
    ~desc:"extracts symbols from external relocations"

let init () =
  Stream.observe Project.Info.spec
  @@ fun spec ->
  let name =
  ...
  Config.manpage
    ...
    ; `P
        "The relocation symbolizer leverages the relocation information stored \
         in files\n\
        \        to extract symbol names. Since a relocation references an \
         external symbol which\n\
        \        doesn't have an address we use an address of a callsite."
    ; `S "SEE ALSO"; `P "$(b,bap-plugin-llvm)(1) code" ] ;

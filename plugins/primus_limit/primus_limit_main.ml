...
module Main (Machine : Primus.Machine.S) = struct
  ...
  let on_blk blk =
    ...
    Machine.Local.get state
    >>= fun s ->
    check_max_visits name s.visited
    >>= fun () ->
  ...
  let init () =
    Machine.sequence [Primus.Interpreter.enter_blk >>> on_blk; register_counter]
end

let () =
  Config.when_ready (fun {get} ->
      if
        Option.is_some (get Cfg.max_length)
      ...
      Primus.Components.register_generic "limit"
        ...
        ~package:"bap"
        ~desc:
          "Enables program termination by limiting the maximum life time of \
           each Primus Machine.")

...
type output = [`insn | `bil | `bir | `sema | `kinds | `size | `invalid]
[@@deriving compare]

let outputs = [`insn; `bil; `bir; `sema; `kinds; `size; `invalid]
...
module Spec = struct
  ...
  let outputs =
    ...
    Extension.Command.dictionary ~doc ~as_flag outputs Type.(list string) name
...
let to_binary ?(map = ident) s =
  ...
  let bytes =
    if separated then String.split_on_chars ~on:seps s
      ...
      List.init
        (String.length s / 2)
        ~f:(fun n -> String.slice s (n * 2) ((n * 2) + 2)) in
...
let create_memory arch data base =
  ...
  Memory.create endian base @@ Bigstring.of_string data
  |> function Ok r -> Ok r | Error e -> fail (Create_mem e)
...
let parse_base arch base =
  ...
  @@ Result.try_with
  @@ fun () ->
  Word.create (Bitvec.of_string base) (Size.in_bits (Arch.addr_size arch))
...
let validate_formats formats =
  ...
  @@ List.map formats ~f:(function
       ...
       | (`kinds | `size | `invalid), [opt] when String.equal enabled opt ->
           Ok ()
...
let () =
  Extension.Command.(
    declare ~doc:mc_man "mc"
      Spec.(
        args $ arch $ base $ backend $ only_one $ stop_on_error $ input
        $ outputs))
  @@ fun arch base backend only_one stop_on_error input outputs _ctxt ->
  validate_formats outputs
  >>= fun () ->
  parse_base arch base
  >>= fun base ->
  read_input input
  >>= fun data ->
  create_memory arch data base
  >>= fun mem ->
  create_disassembler ?backend arch
  >>= fun dis ->
  let formats = formats outputs in
  run ~only_one ~stop_on_error dis arch mem formats
  >>= fun bytes ->
  Dis.close dis ;
...
let () =
  ...
  @@ fun backend loader stop_on_error input outputs _ctxt ->
  validate_formats outputs
  >>= fun () ->
  let formats = formats outputs in
  ...
  | Ok (img, _warns) ->
      ...
      |> Seq.map ~f:(fun mem ->
             run ~stop_on_error dis arch mem formats >>= fun _bytes -> Ok ())
...
let () =
  Extension.Error.register_printer
  @@ function Fail err -> Some (string_of_failure err) | _ -> None

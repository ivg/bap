...
let () =
  Config.manpage
    ...
    ; `S "SEE ALSO"; `P "$(b,bap-api)(3), $(b,bap-c)(3), $(b,bap-plugin-api)(1)"
    ]
...
let array size t : C.Type.t = `Array (spec cvr {C.Type.Array.element= t; size})
...
let rec gnu_attr = function
  | GNU_NONE -> None
  | GNU_CALL (name, args) -> Some C.Type.Attr.{name; args= gnu_attrs_args args}
  | GNU_ID s -> Some C.Type.Attr.{name= s; args= []}
  | GNU_CST _ | GNU_EXTENSION | GNU_INLINE -> None
...
type qualifier =
  { apply:
      'a 'b.    ('a C.Type.qualifier, 'b) C.Type.spec
...
let const =
  { apply=
      (fun t ->
        C.Type.Spec.
          {t with qualifier= C.Type.Qualifier.{t.qualifier with const= true}})
  }
...
let volatile =
  { apply=
      (fun t ->
        C.Type.Spec.
          {t with qualifier= C.Type.Qualifier.{t.qualifier with volatile= true}})
  }
...
type tag = {lookup: 'a. (string -> 'a list -> C.Type.t) -> string -> C.Type.t}
...
let parse (size : C.Size.base) parse lexbuf =
  ...
  let lookup what name =
    match Hashtbl.find tags name with Some t -> t | None -> what name [] in
  ...
  List.iter [`r8; `r16; `r32; `r64] ~f:(fun sz ->
      ...
      def C.Type.all_of_unsigned ; def C.Type.all_of_signed) ;

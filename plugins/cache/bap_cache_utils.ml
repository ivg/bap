...
let write_to_file ?temp_dir writer file data =
  ...
  protect
    ...
    ~finally:(fun () ->
      Out_channel.close ch ; Unix.chmod tmp 0o444 ; Sys.rename tmp file)
...
let binable_to_file :
 ...
 fun ?temp_dir b file data ->
  ...
  protect
    ~f:(fun () ->
      let buf =
        Mmap.V1.map_file fd Bigarray.char Bigarray.c_layout true [|size|] in
      ignore @@ T.bin_write_t (Bigarray.array1_of_genarray buf) ~pos:0 data)
    ~finally:(fun () ->
      Unix.close fd ; Unix.chmod tmp 0o444 ; Sys.rename tmp file)

let binable_from_file :
    type t. (module Binable.S with type t = t) -> string -> t =
 fun b file ->
  let module T = (val b) in
  let fd = Unix.(openfile file [O_RDONLY] 0o400) in
  let data = Mmap.V1.map_file fd Bigarray.char Bigarray.c_layout false [|-1|] in

...
let update_config sz ov no_gc gc =
  ...
  if is_set sz || is_set ov || is_set no_gc || is_set gc then (
    Cache.write_config cfg' ; exit 0 )

let init dir = set_dir dir ; Cache.init () ; run_gc_with_threshold ()
...
let add_description ?arg name descr =
  let name =
    match arg with None -> name | Some arg -> sprintf "%s=%s" name arg in
  let name = "--" ^ name in
  opts := Map.add_exn !opts name descr

let flag doc name = add_description name doc ; Command.flag ~doc name

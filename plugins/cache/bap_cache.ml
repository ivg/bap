...
module Cfg = struct
  module T = struct type t = config [@@deriving bin_io, compare, sexp] end

  let default = {capacity= 4 * 1024; (* 4 Gb  *)
                                     overhead= 25; gc_enabled= true}
...
let mkdtemp ?(mode = 0o0700) ?tmp_dir ?(prefix = "") ?(suffix = "") () =
  ...
  let rec create name =
    let tmp =
      ...
      | Some tmp -> tmp in
    let path = String.concat ~sep:Filename.dir_sep [tmp; prefix; name; suffix] in
...
module Upgrade = struct
  ...
  let upgrade_from_index_v2 index dst =
    ...
    let rename from to_ = Sys.rename from to_ ; Unix.chmod to_ 0o444 in
    try
...
let init () = init_cache_dir () ; Upgrade.run ()

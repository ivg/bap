...
let main path min_length max_length threshold comp =
  let finder arch =
    create_finder path threshold arch comp ~min_length ~max_length in
  let find finder mem =
  ...
  let find_roots arch mem =
    match finder arch with
    | Error _ as err ->
        warning "will not provide roots" ;
        err
    | Ok finder -> (
      match find finder mem with
...
|}
    ...
    ; `P
        ...
        \        links to other plugins, that provide function identification \
         services."
    ; `P
        "[1]: Bao, Tiffany, et al. \"Byteweight: Learning to recognize\n\
        \    functions in binary code.\" 23rd USENIX Security Symposium (USENIX\n\
        \    Security 14). 2014."; `S "SEE ALSO"
    ; `P
        "$(b,bap-byteweight)(1), $(b,bap-plugin-ida)(1), \
         $(b,bap-plugin-read-symbols)(1)" ] ;
  let open Config in
  let min_length =
    param int ~default:8 "min-length"
      ~doc:
        "The minimum length of a word, that could identify a function start. \
         ...
         affect prior probabilities, etc." in
  let max_length =
    param int ~default:16 "max-length" ~synonyms:["length"]
      ~doc:
        "The maximum length of a word, that could identify a function start. \
         ...
         considered, affect prior probabilities, etc." in
  let threshold =
    param float ~default:10. "threshold"
      ~doc:
        "If greater than 1.0 then it is the Bayes factor, otherwise it is a \
         probability." in
  let sigsfile =
    param non_dir_file ~default:Sigs.default_path "sigs"
      ~doc:"Path to the signature file" in
  let compiler =
    param (some string) "comp"

...
let table =
  Hashtbl.of_alist_exn
    ...
    [ ( "sin"
      , floats
          [| -2.1872506537704514e-10; 1.0000000200128274; -3.0302439009453449e-7
           ; -1.6666487069033565e-1; -5.4940340462839474e-6
           ; 8.3432342039387503e-3; -1.1282260464920005e-5
           ; -1.8998652204112551e-4; -4.1639895115337191e-6
           ; 4.0918788476303817e-6; -2.6049442541122577e-7 |] ) ]
...
module Range_Reduction = struct
  ...
  let fmod r x y =
    ...
    >>>= fun d ->
    floor r (var d)
    >>>= fun c -> fmul r y (var c) >>>= fun z -> fsub r x (var z)

  let to_pos_angle rm x =
    x
    >>-> fun sort x ->
    Reduction_Constant.pi_mul_2 sort
    >>>= fun pi_2 -> ite (is_fneg !!x) (fsub rm (var pi_2) !!x) !!x

  (* Sine is an odd function. *)
  let odd_function rm x return =
    x
    >>-> fun sort x ->
    fsub rm !!x (Reduction_Constant.pi sort)
    >>>= fun x_m_pi ->
    is_fpos (var x_m_pi)
    >>>= fun is_pos ->
    ite (var is_pos) (var x_m_pi) !!x
    >>>= fun reduced -> return (var reduced) (var is_pos)
end
...
module Sin = struct
  ...
  let range_reduce rm x return =
    x
    >>-> fun sort x ->
    Reduction_Constant.one_over_2pi sort
    >>>= fun one_over_2pi ->
    Range_Reduction.fmod rm !!x (var one_over_2pi)
    >>>= fun n ->
    Range_Reduction.to_pos_angle rm (var n)
    >>>= fun pn -> Range_Reduction.odd_function rm (var pn) return
...
module Horner : sig
  ...
  let build func var coefs =
    ...
    let polynomial_evaluaton c n = approximate c n in
    let formula =
      match func with
      | Sin -> Sin.build CT.(var v) c polynomial_evaluaton
      | Cos ->
          let shifted_var =
            CT.(fadd rne (var v) (Reduction_Constant.pi_div_2 fsort)) in
...
module Approximate (Machine : Primus.Machine.S) = struct
  ...
  let run [name; size; x] =
    ...
    Value.Symbol.of_value name
    >>= fun name ->
    let elementry_function =
      match parse_op name with Some op -> op | _ -> assert false in
    let coefficients =
    ...
    match exp with
    | None -> assert false
    | Some e -> Eval.set vx x >>= fun () -> printf "%a\n" Exp.ppo e ; Eval.exp e
end

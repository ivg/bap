...
let rec load = function
  | [] -> Ok `Done
  | uri :: uris -> Trace.load uri >>= fun trace -> Traces.add trace ; load uris
  ...
  let () =
    Config.manpage
      ...
      ; `P
          ...
          \       $(b,Traces) of the traces library."; `S "SEE ALSO"
      ; `P "$(b,bap-trace)(3)" ]
    ...
    Config.when_ready (fun {Config.get= ( ! )} ->
        Future.upon Plugins.loaded (fun () ->
            ...
            | Error e -> (
              ...
              | `Ambiguous_uri -> exitf "More than one provider for a given URI"
              | exception Incompatibe_args ->

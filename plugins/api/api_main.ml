...
let try_with f =
  match try_with f with Ok r -> Ok r | Error exn -> Error (`Fail exn)

let try_with_default ~default f =
  match try_with f with Ok r -> r | Error _ -> default

let getenv v = try Some (Sys.getenv v) with Caml.Not_found -> None
...
module Api_path = struct
  ...
  let expand_home path =
    Option.(
      value_map ~default:path ~f:ident
        ( String.chop_prefix ~prefix:"~" (to_string path)
        >>= fun p ->
        getenv "HOME"
        >>= fun h -> Some Path.(filename_of_string h / filename_of_string p) ))
  ...
  let is_apiable_path p =
    let r =
      exists p
      >>= fun x ->
      try_with_access (FileUtil.test is_apiable) (to_string p)
      >>= fun y -> Ok (x && y) in
  ...
  let api_of_path path =
    if is_apiable_path path then Some {desc= desc_of_path path; path} else None
  ...
  let find_apis ?lang path =
    let path' =
      match lang with Some lang -> with_lang path lang | None -> path in
...
module Api = struct
  ...
  let cp api dest =
    ok_if_true2 Api_path.exists dest ~error:`No_dst
    >>= fun () ->
    ok_if_true2 Api_path.is_dir dest ~error:`No_dst
    >>= fun () ->
  ...
  let rm path desc =
    ...
    ok_if_true2 Api_path.exists path ~error:`No_dst
    >>= fun () -> try_with (fun () -> FileUtil.rm [Api_path.to_string path])
end
      ...
      let rec add = function
        ...
        | p :: paths' -> (
          ...
          | Error `No_dst | Error `No_access ->
              info "Api.cp: skipping a non-writable destination %a" Api_path.pp
                p ;
              add paths'
          | Error (`Fail exn) ->
...
let files paths lang =
  List.(
    paths >>| Api_path.find_apis ~lang |> join
    >>| fun api -> Api_path.to_string api.path)
...
let show_all_apis paths =
  let open Format in
  let pp_api api =
    printf "   %s : %s@." api.desc.name (Api_path.to_string api.path) in
  let print_lang apis =
    match apis with
    | [] -> ()
    | hd :: _ ->
...
let sanity_check paths =
  let is_same api api' =
    equal_api_descr api.desc api'.desc && Path.compare api.path api'.path <> 0
  in
...
module Cmdline = struct
  let man =
    [ `S "DESCRIPTION"; `P "Automatically apply API bundled in the plugin. "
    ; `S "SEE ALSO"; `P "$(b,bap-api)(3), $(b,bap-plugin-frontc-parser)(1)" ]

...
module Spec = struct
  ...
  let arch =
    Configuration.parameter ~doc:"Specifies the ISA of raw bytes" arch_t "arch"
...
let register_loader ctxt =
  Image.register_loader ~name:"raw"
    ( module struct
      ...
      let from_data data =
        Or_error.try_with @@ fun () -> Some (generate length_of_data data) end
    )

let () =
  Extension.declare ~provides:["loader"] ~doc
  @@ fun ctxt -> Ok (register_loader ctxt)

...
type memory = {bank: Primus.Memory.Descriptor.t; lower: Addr.t; upper: Addr.t}
...
module Input : sig
  ...
  type t = Var of var | Ptr of {bank: Primus.Memory.Descriptor.t; addr: addr}
  ...
  module Make (Machine : Primus.Machine.S) = struct
    ...
    let allocate_if_not bank addr =
      Mems.is_writable addr
      >>= function
      | true -> Machine.return ()
      | false -> (
          let name = Primus.Memory.Descriptor.name bank in
          Machine.Global.get memories
          >>= fun memories ->
    ...
    let set input value =
      ...
      | Ptr {bank; addr} ->
          ...
          Mems.switch bank
          >>= fun () ->
          allocate_if_not bank addr
          >>= fun () -> Mems.set addr value >>= fun () -> Mems.switch current
...
type executor = {inputs: Set.M(Input).t; values: SMT.expr Primus.Value.Id.Map.t}
...
module Executor (Machine : Primus.Machine.S) : sig
  ...
  let lift1 x r f =
    Machine.Local.get executor
    >>= fun s ->
    match value s x with
    | None -> Machine.return ()
    | Some x ->
        Machine.Local.put executor
          {s with values= Map.add_exn s.values (id r) (f x)}
  ...
  let lift2 x y r f =
    ...
    | fx, fy ->
        ...
        Machine.Local.put executor
          {s with values= Map.add_exn s.values (id r) (f x y)}
  ...
  let lift3 x y z r f =
    ...
    | fx, fy, fz ->
        ...
        Machine.Local.put executor
          {s with values= Map.add_exn s.values (id r) (f x y z)}
  ...
  let set_input origin x =
    ...
    Machine.Local.update executor ~f:(fun s ->
        {inputs= Set.add s.inputs origin; values= Map.set s.values id x})
  ...
  let init () =
    Machine.sequence
      Primus.Interpreter.
        [ binop >>> on_binop; unop >>> on_unop; extract >>> on_extract
        ; concat >>> on_concat; ite >>> on_ite; cast >>> on_cast
...
let forker ctxt : Primus.component =
  ...
  let module Forker (Machine : Primus.Machine.S) = struct
    ...
    let branch_point = function
      | Primus.Pos.Jmp {up= {me= blk}; me= jmp} when Term.length jmp_t blk > 1
        ->
          Some (blk, jmp)
      | _ -> None
    ...
    let on_cond cond =
      ...
      | None -> Machine.return ()
      | Some constr -> (
          Eval.pos >>| compute_edge
          >>= function
          | Some edge ->
              require (Value.is_zero cond)
              @@ fun () ->
              Visited.all
              >>= fun visited ->
              require (not (is_edge_dst_visited visited edge))
              @@ fun () -> new_task ~refute:false ~edge constr
          | None ->
              Machine.Local.get worker
              >>= fun self ->
              Machine.Global.get master
              >>= fun s ->
              require (count s.tries self.tid < cutoff)
              @@ fun () -> new_task ~refute:(Value.is_one cond) constr )
    ...
    let exec_task model task =
      Debug.msg "SAT"
      >>= fun () ->
      Machine.Local.update worker ~f:(fun t -> {t with task})
      >>= fun () ->
      ...
      Machine.Seq.iter inputs ~f:(fun input ->
          ...
          | Some value ->
              ...
              >>= fun () ->
              Value.of_word (SMT.Value.to_word value)
              >>= fun value ->
    ...
    let rec run_master system =
      ...
      | Some master_pid -> (
          ...
          >>= function
          | None ->
              Debug.msg "Worklist is empty, finishing"
              >>= fun () -> Eval.halt >>| never_returns
          | Some (m, t) ->
              Debug.msg "We have some tasks"
              >>= fun () ->
              Machine.fork ()
              >>= fun () ->
              Machine.current ()
              ...
              if Id.equal master_pid client_pid then
                Debug.msg "Client is finished, master is resumed"
                >>= fun () -> run_master system
              else Debug.msg "Forked a new machine" >>= fun () -> exec_task m t
          )

    let update_id t = Machine.Local.update worker ~f:(fun s -> {s with tid= t})

    let init () =
      Machine.sequence
        Primus.Interpreter.
          [ eval_cond >>> on_cond; enter_term >>> update_id
...
module SymbolicPrimitives (Machine : Primus.Machine.S) = struct
  ...
  let with_memory {bank} perform =
    ...
    >>= fun current ->
    Mems.switch bank
    >>= fun () -> perform >>= fun x -> Mems.switch current >>| fun () -> x

  let open_bank id =
    Machine.Global.get memories
    >>= fun mems ->
    Val.Symbol.of_value id
    >>= fun name ->
  ...
  let arch_addr_size =
    Machine.gets Project.arch >>| Arch.addr_size >>| Size.in_bits

  let create_value var rest =
    arch_addr_size
    >>= fun default_width ->
    let width = width default_width rest in
    Val.Symbol.of_value var
    >>= fun name ->
  ...
  let assert_ name assertions =
    Machine.Observation.post failed_assertion ~f:(fun report ->
        Constraints.get Context.Scope.user
        >>= fun user ->
        Constraints.get Context.Scope.path
        >>= fun path ->
        ...
        | None -> Machine.return ()
        | Some model ->
            Val.Symbol.of_value name
            >>= fun name ->
            Executor.inputs >>| Seq.to_list
            >>= fun inputs -> report (name, model, inputs))
    >>= fun () -> Val.b1

...
type pos = Ret_0 | Ret_1 | Arg of int
...
module MS_64 = struct
  ...
  let size =
    object
      ...
      method! alignment =
        function
        | `Basic {C.Type.Spec.t= #C.Type.short} -> `r32 | t -> super#alignment t
    end
...
module WATCOM_STACK = struct
  ...
  let demangle s =
    match String.chop_suffix s ~suffix:"_" with None -> s | Some s -> s
end
...
let supported_api (module Abi : abi) {C.Type.Proto.return; args} =
  ...
  let return =
    match Abi.size#bits return with
    | None -> None
    | Some width -> (
      match Size.of_int_opt width with
      | None ->
          warning "size of return object doesn't fit into word sizes" ;
          raise Unsupported
      | Some _ ->
          let data = C.Abi.data Abi.size return in
          if width > word && width <= word * 2 then
            Some
              ( data
              , Bil.(Abi.arg return width Ret_0 ^ Abi.arg return width Ret_1) )
          else if width <= word then Some (data, Abi.arg return width Ret_0)
          else (
            warning "size of return object doesn't fit into double word\n" ;
            raise Unsupported ) ) in
  let params =
    List.mapi args ~f:(fun i (_, t) ->
        match Abi.size#bits t with
        | None ->
            warning "size of %a parameter is unknown" C.Type.pp t ;
            raise Unsupported
        | Some size -> (
          match Size.of_int_opt size with
...
let supported () : (module abi) list =
  [ (module SysV); (module CDECL); (module STDCALL); (module MS_32)
  ; (module MS_64); (module FASTCALL); (module WATCOM_STACK)
  ; (module WATCOM_REGS) ]

let is_named_after abi name =
  let module Abi = (val abi : abi) in
  String.equal Abi.name name || String.equal (Abi.name ^ "_abi") name

let name (module Abi : abi) = Abi.name
let arch (module Abi : abi) = Abi.arch

let find name =
  supported () |> List.find ~f:(fun abi -> is_named_after abi name)

let auto proj =
  supported () |> List.find ~f:(fun (module Abi) -> Abi.autodetect proj)

let api abi proto =
  try Some (supported_api abi proto)
  with Unsupported ->
    warning "skipped function due to unsupported abi" ;
    None

let default_abi arch : (module abi) =
  match arch with `x86 -> (module CDECL) | `x86_64 -> (module SysV)
  ...
  let abi =
    ...
    |> List.find ~f:(fun abi ->
           List.exists attrs ~f:(fun {C.Type.Attr.name} ->
               is_named_after abi name))
    |> function None -> default | Some abi -> abi in
  ...
  let main proj =
    ...
    | #Arch.x86 as arch ->
        let abi =
          ...
          | Some abi -> abi
          | None -> (
            match auto proj with
            | Some abi -> info "autodetected ABI" ; abi
            | None ->
        ...
        let abi =
          C.Abi.{insert_args= dispatch abi; apply_attrs= (fun _ -> ident)} in

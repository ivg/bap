  ...
  let rec to_ir mode addr next ss pref has_rex has_vex =
    ...
    | Bswap (t, op) ->
        let e =
          ...
          | Type.Imm 32 | Type.Imm 64 ->
              ...
              reverse_bytes op' t'
          | _ -> disfailwith "bswap: Expected 32 or 64 bit type" in
        [assn t op e]
    | Retn (op, far_ret)
      when List.is_empty pref || is_repz pref || is_repnz pref ->
        ...
        let load_stmt =
          if far_ret then
            (* TODO Mess with segment selectors here *)
            unimplemented "long retn not supported"
          else Bil.Move (temp, load_s mode seg_ss (size_of_typ mt) rsp_e) in
        let rsp_stmts =
          Bil.Move (rsp, Bil.(rsp_e + Int (mi (bytes_of_width mt))))
          ::
          ( match op with
          | None -> []
          | Some (t, src) -> [Bil.Move (rsp, Bil.(rsp_e + op2e t src))] ) in
        (load_stmt :: rsp_stmts) @ [Bil.Jmp (Bil.Var temp)]
    | Mov (t, dst, src, condition) ->
        let c_src =
          match condition with
          | None -> op2e t src
          | Some c -> Bil.Ite (c, op2e t src, op2e t dst) in
        ...
        let base_e e =
          ...
          let table_entry =
            load_s mode None (size_of_typ entry_size) Bil.(base + index) in
          (* Extract the base *)
          concat_explist
            ( ( match mode with
              | X86 -> []
            ...
            @ [ Bil.Extract (63, 56, table_entry)
              ...
              ; Bil.Extract (31, 16, table_entry) ] ) in
        let bs =
    ...
    | Movs (Type.Imm _bits as t) ->
        let stmts =
          [ store_s mode seg_es t rdi_e
              (load_s mode seg_es (size_of_typ t) rsi_e); string_incr mode t rsi
          ; string_incr mode t rdi ] in
    ...
    | Movoffset ((tdst, dst), offsets) ->
        ...
        let padding hi lo =
          ...
          else [Bil.Extract (hi, lo, op2e tdst dst)] in
        let offsets =
          List.sort
          ...
          Bil.
            ...
            ; foreach_size (fun (i, left_bit, right_bit) ->
                  ...
                  ; tmp_dst :=
                      var tmp_dst
                      lor cast unsigned t_width (var elt)
                          lsl (var iv * int bits) ]); [assn t dst (var tmp_dst)]
            ]
    ...
    | Pmovmskb (t, dst, src) ->
        ...
        let src =
          match src with
          | Ovec _ -> op2e t src
          | _ -> disfailwith "invalid operand" in
        ...
        let or_together_bits =
          List.fold_left
            ...
            ~init:(int_exp 0 !!padt) all_bits in
    ...
    | Palignr (t, dst, src, vsrc, imm) -> (
        ...
        let high, low =
          ...
          | _ ->
              disfailwith
                "impossible: used non 64/128/256-bit operand in palignr" in
    ...
    | Pcmpstr (t, xmm1, xmm2m128, _imm, imm8cb, pcmpinfo) ->
        ...
        let concat elt_width exps =
          ...
          | first :: exps' ->
              ...
              let exp =
                List.fold exps' ~init:first ~f:(fun e e' -> Bil.(e ^ e')) in
        ...
        let implicit_check is_valid_xmm_i get_xmm_i =
          let f acc i =
            let previous_valid =
              if i = 0 then exp_true else Bil.var (is_valid_xmm_i (i - 1)) in
          ...
          List.fold ~f ~init:[] nelem_range in
        (* Build expressions that assigns the correct values to the
        ...
        let explicit_check is_valid_xmm_i sizee =
          ...
          let init =
            Bil.
              [if_ (int nelem_e < sizee) [sizev := int nelem_e] [sizev := sizee]]
          in
          let f acc i =
        ...
        let get_bit index =
          ...
          | EqualAny ->
              ...
              let check_char acc j =
                ...
                Bil.(is_eql land is_valid lor acc) in
          ...
          | Ranges ->
              ...
              let check_char acc j =
                ...
                let ind1 = ind0 + 1 in
                let rangevalid =
                  Bil.(is_valid_xmm1_e ind0 land is_valid_xmm1_e ind1) in
                let ( <= ) =
                ...
                let inrange =
                  ...
                  land (get_xmm2 index <= get_xmm1 ind1) in
          ...
          List.fold_left
            ...
            | LSB ->
                List.range ~stride:(-1) ~start:`exclusive ~stop:`inclusive nelem
                  0
            | MSB -> List.init ~f:(fun x -> x) nelem ) in
        (* For pcmpistrm/pcmpestrm *)
        let mask e =
          match imm8cb.outselectmask with
          | Bitmask -> Bil.(cast unsigned 128 e)
          | Bytemask ->
              let get_element i = Bil.(cast unsigned !!elemt (extract i i e)) in
              let range =
                List.range ~stride:(-1) ~start:`exclusive ~stop:`inclusive nelem
                  0 in
        ...
        List.concat
          [ xmm1_checks; xmm2_checks; res_bil
          ; Bil.
              [ int_res_1 := cast unsigned 16 (var res); res_of_cb
              ; res_of_pcmpinfo; cf := var int_res_2 <> int_exp 0 16
              ; zf := contains_null xmm2m128_e; sf := contains_null xmm1_e
              ; oF := extract 0 0 (var int_res_2); af := int_exp 0 1
              ; pf := int_exp 0 1 ] ]
    | Pshufd (t, dst, src, vsrc, imm) -> (
        ...
        let get_dword ndword =
          ...
          let index =
            Bil.(Cast (UNSIGNED, !!t, Extract (high_b, low_b, imm_e))) in
          let t' = !!t in
        ...
        let dwords =
          concat_explist
            (List.map ~f:get_dword
               (List.range ~stride:(-1) ~stop:`inclusive topdword 0)) in
        match vsrc with
    ...
    | Pshufb (exp_type, dst_op, src_op, vsrc) ->
        ...
        let check_mem_alignment =
          ...
          | _ -> [] in
        List.concat
          [ check_mem_alignment; [Bil.move tmp_dst zero]
          ; foreach_byte (fun i ->
                Bil.
                  ...
                  ; if_
                      ...
                      [ ind :=
                          cast unsigned 8
                            (extract index_bits 0 (var mask_byte_i))
                      ; tmp_byte :=
                          cast unsigned op_size
                            (extract 7 0 (dst lsr (var ind * byte))) ]
                  ; (tmp_dst :=
    ...
    | Call (o1, ra) -> (
        ...
        | Oimm _ ->
            [ Bil.Move (rsp, Bil.(rsp_e - Int (mi (bytes_of_width mt))))
            ; store_s mode None mt rsp_e (Bil.Int ra); Bil.Jmp target ]
        | _ ->
            let t = tmp mt in
            [ Bil.Move (t, target)
            ; Bil.Move (rsp, Bil.(rsp_e - Int (mi (bytes_of_width mt))))
            ; store_s mode None mt rsp_e (Bil.Int ra); Bil.Jmp (Bil.Var t) ] )
    | Jump o -> [Bil.Jmp (jump_target mode ss has_rex o)]
    | Jcc (o, c) -> [Bil.If (c, [Bil.Jmp (jump_target mode ss has_rex o)], [])]
    ...
    | Shift (st, s, dst, shift) ->
        ...
        Bil.
          [ old := dste; assn s dst (s_f dste count)
          ; if_
              (count <> int_exp 0 s')
              [ cf := new_cf; sf := compute_sf dste; zf := compute_zf s' dste
              ; pf := compute_pf s dste; af := unknown "after-shift" bool_t
              ; if_
                  (count = int_exp 1 s')
                  [oF := new_of]
    ...
    | Shiftd (st, s, dst, fill, count) ->
        ...
        (* SWXXX If shift is greater than the operand size, dst and
           flags are undefined *)
        Bil.
          [ was := e_dst; assn s dst result
          ; if_
              (e_count <> int_exp 0 s')
              [ cf := new_cf; sf := compute_sf e_dst; zf := compute_zf s' e_dst
    ...
    | Bt (t, bitoffset, bitbase) ->
        ...
        let value, shift =
          ...
          | Oaddr a ->
              ...
              (byte, shift)
          | Ovec _ | Oseg _ | Oimm _ -> disfailwith "Invalid bt operand" in
    ...
    | Bs (t, dst, src, dir) ->
        ...
        let check_bit bitindex next_value =
          Bil.(
            Ite
              ( Extract (bitindex, bitindex, src_e) = int_exp 1 1
              , int_exp bitindex t'
              , next_value )) in
        ...
        let bitlist =
          ...
          | Backward -> (* most significant *) List.rev bitlist in
        let first_one =
          List.fold_right ~f:check_bit bitlist
            ~init:
              (Bil.Unknown ("bs: destination undefined when source is zero", t))
        in
        ...
        let undef r =
          Bil.Move
            (r, Bil.Unknown (Var.name r ^ " undefined after bsf", Var.typ r))
        in
    ...
    | Stmxcsr dst ->
        let dst =
          match dst with
          | Oaddr addr -> addr
          | _ -> disfailwith "stmxcsr argument cannot be non-memory" in
        [store reg32_t dst (Bil.Var mxcsr) (*(Unknown ("stmxcsr", reg32_t));*)]
    | Ldmxcsr src ->
        let src =
          match src with
          | Oaddr addr -> addr
          | _ -> disfailwith "ldmxcsr argument cannot be non-memory" in
        [Bil.Move (mxcsr, load (size_of_typ reg32_t) src)]
    | Fnstcw dst ->
        let dst =
          match dst with
          | Oaddr addr -> addr
          | _ -> disfailwith "fnstcw argument cannot be non-memory" in
        [store reg16_t dst (Bil.Var fpu_ctrl)]
    | Fldcw src ->
        let src =
          match src with
          | Oaddr addr -> addr
          | _ -> disfailwith "fldcw argument cannot be non-memory" in
        ...
        let bitvector =
          Array.to_list
            (Array.init width ~f:(fun i ->
                 Bil.(
                   Ite (Extract (i, i, bits), int_exp 1 width, int_exp 0 width))))
        in
        let count = List.reduce_exn ~f:Bil.( + ) bitvector in
        set_zf width bits :: assn t d count
    ...
    | Sahf ->
        ...
        let extractlist =
          List.map
            ...
            (List.range ~stride:(-1) ~stop:`inclusive 7 0) in
        Bil.Move (tah, ah_e)
        :: List.concat (List.map2_exn ~f:(fun f e -> f e) assnsf extractlist)
    | Lahf ->
        let o_ah = Oreg 4 in
    ...
    | Sub (t, o1, o2) (* o1 = o1 - o2 *) ->
        ...
        let op2 =
          if is_small_imm o2 t then sign_extend_imm o2 t else op2e t o2 in
        Bil.(
    ...
    | Sbb (t, o1, o2) ->
        ...
        let d = op2e t o1 in
        let s1 = if is_small_imm o2 t then sign_extend_imm o2 t else op2e t o2 in
    ...
    | Cmpxchg8b o ->
        ...
        [ Bil.Move (equal, Bil.(accumulator = dst_e)); Bil.Move (zf, equal_v)
    ...
    | Xadd (t, dst_op, src_op) ->
        ...
        Bil.
          [ tmp_src := src; tmp_dst := dst; tmp_res := src + dst
          ; assn t src_op dst; assn t dst_op (var tmp_res) ]
        @ set_flags_add !!t (Bil.var tmp_dst) (Bil.var tmp_src) dst
    | Xchg (t, src, dst) ->
        let tmp = tmp t in
        [ Bil.Move (tmp, op2e t src); assn t src (op2e t dst)
    ...
    | Or (t, o1, o2) ->
        let o2 = if is_small_imm o2 t then sign_extend_imm o2 t else op2e t o2 in
    ...
    | Ptest (t, o1, o2) ->
        ...
        ; Move (af, exp_false); Move (oF, exp_false); Move (pf, exp_false)
        ; Move (sf, exp_false)
        ...
        let assnstmts, assne =
          Bil.(
            assn_dbl t
              ...
              * Cast (UNSIGNED, !!new_t, op2e t src) )) in
        let flag =
          let highbit = !!new_t - 1 in
          let lowbit = !!new_t / 2 in
          Bil.(Extract (highbit, lowbit, assne) <> int_exp 0 !!t) in
        assnstmts
        @ [ Bil.Move (oF, flag); Bil.Move (cf, flag)
    ...
    | Imul (t, (oneopform, dst), src1, src2) ->
        ...
        let mul_stmts =
              ...
              let assnstmts, assne =
                assn_dbl t
                  Bil.(
                    ...
                    * Cast (SIGNED, !!new_t, op2e t src2)) in
              let flag =
          ...
          | false ->
              ...
              ; assn t dst Bil.(Cast (LOW, !!t, Var tmp)); Bil.Move (oF, flag)
              ; Bil.Move (cf, flag) ] in
        mul_stmts
    ...
    | Div (t, src) ->
        ...
        Bil.
          [ divisor := cast unsigned !!dt (op2e t src); dividend := op2e_dbl t
          ; if_
              (var divisor = zero)
    ...
    | Idiv (t, src) ->
        ...
        Bil.
          [ divisor := cast signed !!dt (op2e t src); dividend := op2e_dbl t
          ; if_
              (var divisor = zero)
              [Cpu_exceptions.divide_by_zero]
              (* else *)
...
let insn arch mem insn =
  ...
  Or_error.try_with (fun () -> disasm_instr mode mem addr)
  |> function
  | Error err ->
      warning "the legacy lifter failed at %a - %a" pp_insn (mem, insn) Error.pp
        err ;
      Error err
  | Ok bil -> (
    ...
    | Ok () -> Ok bil
    | Error err ->
        warning "BIL is not well-type in the legacy lifter at %a - %a" pp_insn
          (mem, insn) Type.Error.pp err ;

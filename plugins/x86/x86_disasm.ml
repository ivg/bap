...
let cc_to_exp i =
  let cc =
    ...
    | _ -> disfailwith X86 "impossible condition code" in
...
let parse_instr mode mem addr =
  ...
  let g a =
    ...
    Memory.get ~addr:a mem >>= Word.to_int |> ok_exn |> Char.of_int_exn in
  ...
  let get_prefixes a =
    let rex a =
      ...
      | X8664 ->
          ...
          if i >= 0x40 && i <= 0x4f then (Some i, s a) else (None, a) in
    let rec f l a =
  ...
  let parse_disp =
    ...
    | Imm 32 -> parse_disp32
    | Imm 64 -> parse_disp64
    | _ -> disfailwith "unsupported displacement size" in
  let parse_imm8cb b =
    let open Pcmpstr in
  ...
  let parse_sib rex modb a =
    ...
    let bits2rege =
      match mode with X86 -> bits2reg32e | X8664 -> bits2reg64e in
    ...
    if idx = 4 then (base, na)
      ...
      else Bil.(base + (idx lsl Int (im ss)), na) in
  ...
  let parse_modrm16ext rex _iaoffset a =
    ...
    parse_modrm16int a b r modb rm na in
  ...
  let parse_modrm3264int rex at _ia _a _b r modb rm naoffset na =
    ...
    let bits2rege =
      ...
      | _ -> failwith "parse_modrm3264int: invalid address type" in
    match modb land 7 with
    (* MOD *)
    | 0 -> (
      match rm land 7 with
  ...
  let parse_modrm3264ext rex vex at ia immoff a =
    ...
    let b, r, modb, rm, na = parse_modrmbits64 rex a in
    parse_modrm3264int rex at ia a b r modb rm immoff na in
  ...
  let parse_modrm3264seg rex vex at ia iaoffset a =
    ...
    (Oseg (r land 7), rm, na) in
  (* Get the extra vvvv operand from the VEX prefix *)
  let get_vex_opr vex =
    match vex with
    | None -> None
    | Some {vex_v; _} -> Some (Ovec (lnot vex_v land 0xf)) in
  ...
  let parse_modrm3264_vec rex vex at ia iaoffset a =
    ...
    (tovec r, tovec rm, get_vex_opr vex, na) in
  ...
  let parse_immz t a =
    ...
    | _ -> disfailwith "parse_immz unsupported size" in
  let parse_simm t a =
    let open Type in
    match t with
    | Imm 8 -> parse_simm8 a
    ...
    | _ -> disfailwith "parse_simm unsupported size" in
  ...
  let oimm_resize op size =
    ...
    | _ -> disfailwith "oimm_resize only handles Oimm" in
  (* This is for opcodes like e9 *)
  let expanded_jump_type opsize =
    match mode with X86 -> opsize | X8664 -> reg32_t in
  let get_opcode _pref ({rex; vex; rm_extend; addrsize; _} as prefix) a =
    let ( parse_disp_addr
      ...
      | Imm 64 ->
          ...
          , parse_modrm3264seg rex vex addrsize a
          , parse_modrm3264ext rex vex addrsize a )
      | _ -> failwith "Bad address type" in
    ...
    let b1, na =
      if Option.is_some vex then (0x0f, a) else (Char.to_int (g a), s a) in
    match b1 with
    (* Table A-2 *)
    ...
    | 0x68 | 0x6a ->
        let o, na =
          ...
          if b1 = 0x68 then parse_immz prefix.opsize na else parse_immb na in
    ...
    | 0x8c ->
        ...
        let extend =
          if Type.equal prefix.opsize reg64_t then reg64_t else reg16_t in
    ...
    | 0x98 ->
        let srct =
          ...
          | Imm 32 -> reg16_t
          | Imm 64 -> reg32_t
          | _ -> disfailwith "invalid opsize for CBW/CWDE/CWQE" in
    ...
    | 0xff -> (
        ...
        | 6 ->
            let size =
              match mode with X86 -> prefix.opsize | X8664 -> reg64_t in
            (Push (size, rm), na)
        | _ -> disfailwith (Printf.sprintf "impossible opcode: %02x/%d" b1 r) )
    (*** 00 to 3e ***)
    | b1 when b1 < 0x3e && b1 land 7 < 6 ->
        let ins a =
          ...
          | 6 -> Xor a
          | 7 -> Cmp a
          | _ -> disfailwith (Printf.sprintf "impossible opcode: %02x" b1) in
    ...
    | 0x0f -> (
            ...
            let tdst, dst, telt, tsrc1, src1, off_src1, off_dst1, src2 =
              ...
              | (0x12 | 0x16) when Option.is_some rv ->
                  ...
                  let rv =
                    match rv with
                    | Some r -> r
                    | None -> disfailwith "impossible" in
                  let src2 =
                    [ { offlen= reg64_t
                      ; offtyp= reg128_t
                      ; offop= rm
                      ; offsrcoffset= offs2
                      ; offdstoffset= offd2 } ] in
        ...
        | 0x10 | 0x11 | 0x28 | 0x29 | 0x6e | 0x7e | 0x6f | 0x7f | 0xd6 ->
            ...
            let src, dst, tsrc, tdst, align =
              ...
              | _ ->
                  unimplemented
                    (Printf.sprintf "mov opcode case missing: %02x" b2) in
        ...
        | 0x57 ->
            ...
            in
            ( Ppackedbinop (t, prefix.opsize, Bil.( lxor ), "xorp", r, rm, rv)
            , na )
        ...
        | (0x64 | 0x65 | 0x66 | 0x74 | 0x75 | 0x76) as o ->
            let r, rm, rv, na = parse_modrm_vec None na in
            let elet =
              match o land 0x6 with
              | 0x4 -> reg8_t
              | 0x5 -> reg16_t
              | 0x6 -> reg32_t
              | _ -> disfailwith "impossible" in
            let bop, bstr =
              match o land 0x70 with
              | 0x70 -> (Bil.EQ, "pcmpeq")
              | 0x60 -> (Bil.SLT, "pcmpgt")
              | _ -> disfailwith "impossible" in
  ...
  let opsize, bopsize, mopsize =
    ...
    else (reg32_t, modesize, reg64_t) in
  let opsize =
    match rex with
    | Some {rex_w= true; _} ->
        reg64_t
    ...
    | Some {rex_w= false; _} | None -> opsize in
  let opsize =
    match vex with Some {vex_we= true; _} -> reg64_t | _ -> opsize in
  let mopsize =
    match vex with
  ...
  let addrsize =
    ...
    | X8664 -> if ints_mem pref pref_addrsize then reg32_t else reg64_t in

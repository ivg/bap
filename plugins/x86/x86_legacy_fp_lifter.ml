...
module Fp_lifter = struct
  ...
  let var_of_reg_t mode = function
    ...
    *)
    ...
    | reg -> Error.failwiths "no var_of_reg_t translation yet"
               reg sexp_of_reg_t
  ...
  let st_index_of_reg_t = function
    ...
    | `ST6 -> 6
    | `ST7 -> 7
    | reg -> Error.failwiths "Input is not an ST register." reg sexp_of_reg_t
  ...
  let lift arch mem insn =
    ...
    let ld_fprr mem src =
      let index = decode src |> Option.value_exn |> st_index_of_reg_t in
      load_fp ~index |> ToIR.push_x87_stack |> Result.Ok
    in

    (** FSTP: Copy ST(0) to ST(i) and pop register stack. *)
    let st_fprr mem dst =
      let dst_index = decode dst |> Option.value_exn |> st_index_of_reg_t in
    ...
    let cvtsi2sd_rr ~typ arch mem dst src =
      let dst = decode dst |> Option.value_exn |> var_of_reg_t arch in
      let src = decode src |> Option.value_exn |> exp_of_reg_t arch in
      assert (get_type src = typ);
      cvtsi2sd dst src
    in
    ...
    let cvtsi2ss_rr ~typ arch mem dst src =
      let dst = decode dst |> Option.value_exn |> var_of_reg_t arch in
      let src = decode src |> Option.value_exn |> var_of_reg_t arch in
    ...
    let sse_binopsd_rrr ~typ ~binop arch mem dst src1 src2 =
      let dst = decode dst |> Option.value_exn |> var_of_reg_t arch in
      let src1 = decode src1 |> Option.value_exn |> exp_of_reg_t arch in
      let src2 = decode src2 |> Option.value_exn |> exp_of_reg_t arch in
    ...
    let x87_binop style ~binop ?(reversed=false) mem sti =
      ...
      let sti_index = decode sti |> Option.value_exn |> st_index_of_reg_t in
      let st0 = load_fp ~index:0 in
    ...
    let unpcklps_rr arch mem dest src1 src2 =
      let src1 = decode src1 |> Option.value_exn |> exp_of_reg_t arch in
      let src2 = decode src2 |> Option.value_exn |> exp_of_reg_t arch in
      let dest = decode dest |> Option.value_exn |> var_of_reg_t arch in
    ...
    let xch_f mem sti =
      let i = decode sti |> Option.value_exn |> st_index_of_reg_t in
    ...
    let vinserti128_rrri arch mem dst src1 src2 imm =
      ...
      let src1 = decode src1 |> Option.value_exn |> exp_of_reg_t arch in
      let src2 = decode src2 |> Option.value_exn |> exp_of_reg_t arch in
      let imm = Bap.Std.Imm.to_int imm |> Option.value_exn in
    ...
    let packuswb_rr arch mem dst src1 src2 =
      let dst_size = decode dst |> Option.value_exn |> size_of_reg_t in
      let dst = decode dst |> Option.value_exn |> var_of_reg_t arch in
      let src1 = decode src1 |> Option.value_exn |> var_of_reg_t arch in
      let src2 = decode src2 |> Option.value_exn |> var_of_reg_t arch in
      ...
      *)
      ...
      let low_byte, remaining_bytes = Sequence.next bytes |> Option.value_exn in
      let final_value =

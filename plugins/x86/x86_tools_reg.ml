...
module Make (CPU : X86CPU) : RR = struct
  ...
  let of_mc reg =
    let open Option in
    X86_asm.Reg.decode reg
    >>= function #X86_asm.reg as t -> of_asm t | _ -> None
  ...
  let var t =
    ...
    else Error.failwiths "invalid reg variable" t X86_asm.sexp_of_reg
  ...
  let set r e =
    ...
    let rhs =
      ...
      | #Reg.r32, `x86
       ...
       |#Reg.r256, _ ->
          e
      | #Reg.r64, `x86 -> Error.failwiths "invalid reg" r X86_asm.sexp_of_reg
    in
    Bil.(lhs := rhs)
end

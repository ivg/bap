...
open Bap_elf.Std
open Bap_dwarf.Std
open Backend [@@warning "-D"]
open Elf
include Self ()

let () =
  Config.manpage
    [ `S "DESCRIPTION"; `P "An ELF image loader written in pure OCaml."
    ; `S "SEE ALSO"; `P "$(b,bap-elf)(3), $(b,bap-plugin-llvm)(1)" ]
...
let section_data data s : string Or_error.t =
  int_of_int64 s.sh_size
  >>= fun size ->
  int_of_int64 s.sh_offset
  >>= fun offset ->
  try
...
let create_symtab data endian elf =
  ...
  Dwarf.Data.create endian sections
  >>= fun data ->
  Dwarf.Fbi.create data
  >>= fun dff ->
  let seq =
    Sequence.mapi (Dwarf.Fbi.functions dff) ~f:(fun i (name, fn) ->
        ...
        let size =
          ...
          | None -> return None
          | Some pc_hi ->
              Addr.Int_err.(!$pc_hi - !$pc_lo)
              >>= Addr.to_int
              >>| fun size -> Some size in
        size
        >>= fun size ->
        let location =
          Location.{addr= pc_lo; len= Option.value size ~default:1} in
        return
          { Symbol.name
          ; is_function= true
          ; is_debug= true
...
let create_segment make_addr i es : Segment.t Or_error.t option =
  if not ([%compare.equal: p_type] es.p_type PT_LOAD) then None
    ...
    let segment =
      ...
      int_of_int64 es.p_filesz
      >>= fun len ->
      int_of_int64 es.p_offset
      >>= fun off ->
      perm_of_flags es.p_flags
      >>= fun perm ->
      let addr = make_addr es.p_vaddr in
      let location = Location.Fields.create ~len ~addr in
      return {Segment.name; Segment.perm; Segment.off; Segment.location} in
    match segment with
...
let img_of_elf data elf : Img.t Or_error.t =
  ...
  let addr_size =
    match elf.e_class with ELFCLASS32 -> `r32 | ELFCLASS64 -> `r64 in
  ...
  let segments, errors =
    ...
    |> Seq.to_list
    |> List.partition_map ~f:(function Ok s -> `Fst s | Error e -> `Snd e) in
  let symbols, errors =
    match create_symtab data endian elf with
    | Ok syms -> (syms, errors)
    | Error err -> ([], Error.tag err "failed to read symbols" :: errors) in
  arch
  >>= fun arch ->
  let sections =
    Seq.filter_map elf.e_sections ~f:(fun s ->

...
module Result = struct
  type t = {visited: int Tid.Map.t; tainted_regs: taints; tainted_ptrs: taints}
  ...
  let union_maps x y ~f =
    Map.merge x y ~f:(fun ~key ->
      function `Left s | `Right s -> Some s | `Both (s1, s2) -> Some (f s1 s2))
...
let memory_lookup proj addr =
  ...
  Memmap.lookup memory addr |> Seq.hd
  |> function
  | None -> None
  | Some (mem, _) -> (
      ...
      if is_seeded def then
        foreach
          ...
          ~f:(fun var ->
            ...
            >>= fun r ->
            self#update var r
            >>= fun () -> SM.update (fun ctxt -> taint_result def ctxt r))
      else SM.return ()
  end

    ...
    method! map_term cls t =
      ...
      let taint taint taints =
        ...
        map_if
          ...
          ~f:(fun t -> Term.set_attr t taint taints) in
...
let seeded callgraph subs =
  let callers sub =
    Graphlib.fold_reachable
      ...
      callgraph ~rev:true ~init:Tid.Set.empty ~f:Set.add (Term.tid sub) in
...
let main args proj =
  ...
  let state =
    ...
    | None ->
        ...
        State.Cache.save digest s ; s in

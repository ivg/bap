...
module Check = struct
  ...
  let run {path; ida} =
    ...
    Result.all_unit
      [ require_ida path; require_ida_python path; require_kind ida.graphical
      ; require_kind ida.graphical64; require_kind ida.headless
      ; require_kind ida.headless64 ]

  let check_integrity ida =
    let files = [ida.graphical; ida.headless; ida.graphical64; ida.headless64] in
...
let create_ida path =
  ...
  Result.(
    find_kind ~is_headless:true `m32
    >>= fun headless ->
    find_kind ~is_headless:false `m32
    >>= fun graphical ->
    find_kind ~is_headless:true `m64
    >>= fun headless64 ->
    find_kind ~is_headless:false `m64
    >>= fun graphical64 ->
...
let create' path is_headless =
  ...
  Result.(
    ...
    >>= fun () ->
    check_headless is_headless
    >>= fun () ->
    create_ida path
    >>= fun ida -> check_integrity ida >>= fun () -> Ok {ida; path; is_headless})

...
let ida_symbolizer =
  ...
  Knowledge.Agent.register ~reliability ~package:"bap.std" "ida-symbolizer"
...
let mapfile path : Bigstring.t =
  ...
  let data =
    Mmap.V1.map_file fd Bigarray.char Bigarray.c_layout false [|size|] in
  Unix.close fd ;
...
module IdaBrancher : sig
  ...
  let of_brancher_info arch bf : t =
    ...
    let helper tab (addr, _normal_flow, other_flows) =
      Addr.Table.add_exn tab ~key:!addr ~data:(other_flows_to_dests other_flows) ;
      tab in
    List.fold bf ~init:(Addr.Table.create ()) ~f:helper

  let resolve t mem _ =
    match Addr.Table.find t (Memory.min_addr mem) with
    | Some dests -> dests
    | None -> []
end
...
let main () =
  register_source
    ( module struct
      ...
      let provide _ data = provide data end ) ;

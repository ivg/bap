...
let entities : (string, entity) List.Assoc.t =
  [ ("entities", `Entities); ("plugins", `Plugins); ("commands", `Commands)
  ; ("recipes", `Recipes); ("tags", `Tags); ("features", `Tags)
  ; ("passes", `Passes); ("formats", `Formats); ("classes", `Classes)
  ; ("theories", `Theories); ("agents", `Agents) ]
...
let entity_desc : (entity * string) list =
  [ (`Entities, "prints this message"); (`Plugins, "installed BAP plugins")
  ; (`Commands, "bap utility commands"); (`Recipes, "installed recipes")
  ; (`Tags, "plugin capabilities"); (`Passes, "disassembler passes")
  ; (`Formats, "data output formats")
  ; (`Classes, "knowledge representation classes")
  ; (`Theories, "installed theories"); (`Agents, "knowledge providers") ]
...
let () =
  ...
  let push x xs = match xs with None -> Some [x] | Some xs -> Some (x :: xs) in
  let parse_filter =

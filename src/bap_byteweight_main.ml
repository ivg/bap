...
let train_on_file loader comp operation max_length db path =
  create_image loader path
  >>= fun img ->
  let arch = Image.arch img in
  let oracle = oracle_of_image img in
  load_or_create_signatures ?comp ~path:db operation arch
  >>= fun bw ->
...
let find loader threshold min_length max_length comp path input _ctxt =
  create_image loader input
  >>= fun img ->
  let arch = Image.arch img in
  load_or_create_signatures ?comp ?path `load arch
  >>| fun bw ->
  let find =
    if Float.(threshold > 1.) then
      BW.find_using_bayes_factor bw threshold ~min_length ~max_length
    else BW.find_using_threshold bw threshold ~min_length ~max_length in
  let scan_memory mem =
...
let symbols loader print_name print_size input _ctxt =
  create_image loader input >>| Image.symbols
  >>| fun syms ->
...
module SymIO = struct
  let read arch ic : (string * addr * addr) list =
    ...
    List.(
      Sexp.input_sexps ic >>| sym_of_sexp
      >>| fun (s, es, ef) -> (s, addr_of_int64 es, addr_of_int64 ef))
    ...
    try
      let syms =
        List.map syms ~f:(fun (s, es, ef) ->
            (s, Addr.to_int64 es |> ok_exn, Addr.to_int64 ef |> ok_exn)) in
...
let () =
  ...
  @@ function Byteweight_failed err -> Some (string_of_error err) | _ -> None

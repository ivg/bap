...
let stx name opt_opcode size arch ctxt =
  ...
  let init =
    Bil.
      [ r9 := int data; r14 := int @@ Word.of_int ~width x
      ; r31 := int @@ Word.of_int ~width y ] in
  let bits = Size.in_bits size in
  let expected = Word.extract_exn ~hi:(bits - 1) ~lo:0 data in
  check_mem init bytes mem ~addr:ea ~size expected arch ctxt
...
let stux name opt_opcode size arch ctxt =
  ...
  let init =
    Bil.
      [ r31 := int (Word.of_int ~width x); r25 := int (Word.of_int ~width y)
      ; r9 := int data ] in
  let bits = Size.in_bits size in
  let expected = Word.extract_exn ~hi:(bits - 1) ~lo:0 data in
  check_mem init bytes mem ~addr:ea ~size expected arch ctxt ;
...
let stdx ctxt =
  ...
  let init =
    Bil.
      [ r1 := int data; r8 := int @@ Word.of_int ~width:64 x
      ; r9 := int @@ Word.of_int ~width:64 y ] in
  let expected = data in
  check_mem init bytes mem ~addr:ea ~size:`r64 expected arch ctxt
...
let stbrx name opt_opcode size arch ctxt =
  ...
  let init =
    Bil.
      [ r1 := int data; r2 := int (Word.of_int ~width addr)
      ; r3 := int (Word.of_int ~width disp) ] in
  let expected =
...
let suite =
  ...
  >::: [ "stb32 +imm" >:: stb `ppc ~d_addr:16
       ...
       ; "stw32 -imm" >:: stw `ppc ~d_addr:(-16)
       ; "stw32 0reg" >:: stw_zero_reg `ppc ~d_addr:16; "stbx32" >:: stbx `ppc
       ; "stbx32 0reg" >:: stbx_zero_reg `ppc; "sthx32" >:: sthx `ppc
       ; "sthx32 0reg" >:: sthx_zero_reg `ppc; "stwx32" >:: stwx `ppc
       ...
       ; "stwu32 -imm" >:: stwu `ppc ~d_addr:(-16); "stbux32" >:: stbux `ppc
       ; "sthux32" >:: sthux `ppc; "stwux32" >:: stwux `ppc
       ; "sthbrx32" >:: sthbrx `ppc; "stwbrx32" >:: stwbrx `ppc
       ...
       ; "stbx64" >:: stbx `ppc64; "stbx64 0reg" >:: stbx_zero_reg `ppc64
       ; "sthx64" >:: sthx `ppc64; "sthx64 0reg" >:: sthx_zero_reg `ppc64
       ; "stwx64" >:: stwx `ppc64; "stwx64 0reg" >:: stwx_zero_reg `ppc64
       ; "stbu64 +imm" >:: stbu `ppc64 ~d_addr:16
       ; "stbu64 -imm" >:: stbu `ppc64 ~d_addr:(-16)
       ; "sthu64 +imm" >:: sthu `ppc64 ~d_addr:16
       ; "sthu64 -imm" >:: sthu `ppc64 ~d_addr:(-16)
       ; "stwu64 +imm" >:: stwu `ppc64 ~d_addr:16
       ; "stwu64 -imm" >:: stwu `ppc64 ~d_addr:(-16); "stbux64" >:: stbux `ppc64
       ; "sthux64" >:: sthux `ppc64; "stwux64" >:: stwux `ppc64; "std64" >:: std
       ; "stdx64" >:: stdx; "stdu64" >:: stdu; "stdux64" >:: stdux
       ; "stdux64a" >:: stdux_big_addr; "sthbrx64" >:: sthbrx `ppc64
       ; "stwbrx64" >:: stwbrx `ppc64; "stdbrx64" >:: stdbrx ]

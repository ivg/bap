...
let slw arch sh ctxt =
  ...
  if sh >= 32 then check_gpr init bytes r10 (Word.zero width) arch ctxt
    ...
    let expected =
      match arch with `ppc -> x | _ -> Word.concat (Word.zero 32) x in
...
let srawi arch sh x ?(carry = false) ctxt =
  ...
  let left_bits =
    ...
    else Word.zero left_bits_num in
...
let suite =
  "shift"
  >::: [ "slw 4" >:: slw `ppc 4; "slw 0" >:: slw `ppc 0
       ; "slw zero res" >:: slw `ppc 42; "srw 4" >:: srw `ppc 4
       ; "srw 0" >:: srw `ppc 0; "srw zero res" >:: srw `ppc 42
       ...
       ; "slw64 4" >:: slw `ppc64 4; "slw64 0" >:: slw `ppc64 0
       ; "slw64 zero res" >:: slw `ppc64 42; "srw64 4" >:: srw `ppc64 4
       ; "srw64 0" >:: srw `ppc64 0; "srw64 zero res" >:: srw `ppc64 42

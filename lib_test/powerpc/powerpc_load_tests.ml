...
let lz name opcode size arch ~d_addr ~value ctxt =
  ...
  let init =
    Bil.
      ...
      ; r9 := int (Word.of_int64 ~width 0xFFFFFFFFFFFFFFFFL) ] in
...
let lzx name opt size arch ~value ctxt =
  ...
  let init =
    Bil.
      [ mem := store ~mem:(var mem) ~addr:(int ea) (int data) endian size
      ; r29 := int @@ Word.of_int ~width x; r10 := int @@ Word.of_int ~width y
      ; r9 := int (Word.of_int64 ~width 0xFFFFFFFFFFFFFFFFL) ] in
  let expected = Word.of_int ~width value in
  check_gpr init bytes r9 expected arch ctxt
...
let lha arch ctxt =
  ...
  let init =
    Bil.
      ...
      ; r9 := int (Word.of_int ~width addr) ] in
...
let lhax arch ctxt =
  ...
  let init =
    Bil.
      [ mem := store ~mem:(var mem) ~addr:(int ea) (int data) endian `r16
      ; r5 := int (Word.of_int ~width disp); r9 := int (Word.of_int ~width addr)
      ] in
  let expected = Word.of_int64 ~width 0xffffffff_ffffffabL in
  check_gpr init bytes r1 expected arch ctxt
...
let lwa arch ctxt =
  ...
  let init =
    Bil.
      ...
      ; r11 := int (Word.of_int ~width addr) ] in
...
let ld ctxt =
  ...
  let init =
    Bil.
      ...
      ; r9 := int (Word.of_int ~width:64 addr) ] in
...
let ldx ctxt =
  ...
  let init =
    Bil.
      ...
      ; r8 := int (Word.of_int ~width:64 disp) ] in
  let expected = data in
  check_gpr init bytes r1 expected arch ctxt
...
let lbrx name opt_opcode size arch ctxt =
  ...
  let init =
    Bil.
      ...
      ; r3 := int (Word.of_int ~width d_addr) ] in
...
let suite =
  ...
  >::: [ "lbz32 +imm" >:: lbz `ppc ~d_addr:20 ~value:0x42
       ...
       ; "lwzux32" >:: lwzux `ppc ~value:0xAAAA4242; "lha32" >:: lha `ppc
       ; "lhax32" >:: lhax `ppc; "lhau32" >:: lhau `ppc
       ; "lhaux32" >:: lhaux `ppc; "lwa32" >:: lwa `ppc; "lwax32" >:: lwax `ppc
       ; "lwaux32" >:: lwaux `ppc; "lhbrx32" >:: lhbrx `ppc
       ; "lwbrx32" >:: lwbrx `ppc
       ...
       ; "lwzux64" >:: lwzux `ppc64 ~value:0xAAAA4242; "lha64" >:: lha `ppc64
       ; "lhax64" >:: lhax `ppc64; "lhau64" >:: lhau `ppc64
       ; "lhaux64" >:: lhaux `ppc64; "lwa64" >:: lwa `ppc64
       ; "lwax64" >:: lwax `ppc64; "lwaux64" >:: lwaux `ppc64; "ld64" >:: ld
       ; "ldx64" >:: ldx; "ldu64" >:: ldu; "ldux64" >:: ldux
       ; "ldux64a" >:: ldux_big_addr; "lhbrx64" >:: lhbrx `ppc64
       ; "lwbrx64" >:: lwbrx `ppc64; "ldbrx64" >:: ldbrx ]

...
module Make (B : Bitwidth) = struct
  ...
  let cr_bits =
    [ cr0; cr1; cr2; cr3; cr4; cr5; cr6; cr7; cr8; cr9; cr10; cr11; cr12; cr13
    ; cr14; cr15; cr16; cr17; cr18; cr19; cr20; cr21; cr22; cr23; cr24; cr25
    ; cr26; cr27; cr28; cr29; cr30; cr31 ]
  ...
  let crn =
    Int.Map.fold cri ~init:String.Map.empty ~f:(fun ~key:_ ~data:var acc ->
        Map.set acc (Var.name var) var)

  let fields =
    [ ("CR0", 0, (cr28, cr29, cr30, cr31)); ("CR1", 1, (cr24, cr25, cr26, cr27))
    ; ("CR2", 2, (cr20, cr21, cr22, cr23)); ("CR3", 3, (cr16, cr17, cr18, cr19))
    ; ("CR4", 4, (cr12, cr13, cr14, cr15)); ("CR5", 5, (cr8, cr9, cr10, cr11))
    ; ("CR6", 6, (cr4, cr5, cr6, cr7)); ("CR7", 7, (cr0, cr1, cr2, cr3)) ]
...
module P32 = Make (struct let gpr_bitwidth = 32 let addr_size = `r32 end)
module P64 = Make (struct let gpr_bitwidth = 64 let addr_size = `r64 end)
module Any_ppc = P32

let raise_arch () = failwith "powerpc arch is the only expected"
let cr_bit n = Int.Map.find_exn Any_ppc.cri n
let nf = cr_bit 0
let pf = cr_bit 1
let zf = cr_bit 2
let ca = Any_ppc.ca
...
let create_memory arch s addr =
  ...
  Memory.create endian addr @@ Bigstring.of_string s
  |> function Ok r -> r | Error _ -> failwith "can't create memory"
...
type form = [`B | `D | `I | `M | `MD | `MDS | `VA | `X | `XL | `XFX | `XO | `XS]
[@@deriving sexp]
...
let make_insn ?name ?(arch = `ppc) form fields =
  ...
  let bytes =
    ...
    | `B, [opcode; bo; bi; bd; aa; lk] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 bo; word ~width:5 bi
          ; word ~width:14 bd; word ~width:1 aa; word ~width:1 lk ]
    | `D, [opcode; rt; ra; d] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rt; word ~width:5 ra
          ; word ~width:16 d ]
    | `I, [opcode; imm; aa; lk] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:24 imm; word ~width:1 aa
          ; word ~width:1 lk ]
    | `M, [opcode; rs; ra; sh; mb; me; rc] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rs; word ~width:5 ra
          ; word ~width:5 sh; word ~width:5 mb; word ~width:5 me
          ; word ~width:1 rc ]
    | `MD, [opcode; rs; ra; sh1; me; opt; sh2; rc] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rs; word ~width:5 ra
          ; word ~width:5 sh1; word ~width:6 me; word ~width:3 opt
          ; word ~width:1 sh2; word ~width:1 rc ]
    | `MDS, [opcode; rs; ra; rb; mb; opt; rc] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rs; word ~width:5 ra
          ; word ~width:5 rb; word ~width:6 mb; word ~width:4 opt
          ; word ~width:1 rc ]
    | `VA, [opcode; rt; ra; rb; rc; opt_opcode] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rt; word ~width:5 ra
          ; word ~width:5 rb; word ~width:5 rc; word ~width:6 opt_opcode ]
    | `X, [opcode; rt; ra; rb; opt_opcode; rc] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rt; word ~width:5 ra
          ; word ~width:5 rb; word ~width:10 opt_opcode; word ~width:1 rc ]
    | `X, [opcode; rt; ra; rb; opt_opcode] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rt; word ~width:5 ra
          ; word ~width:5 rb; word ~width:10 opt_opcode; b0 ]
    | `XL, [opcode; bf; x; bfa; y; z; opt_opcode; w] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:3 bf; word ~width:2 x
          ; word ~width:3 bfa; word ~width:2 y; word ~width:5 z
          ; word ~width:10 opt_opcode; word ~width:1 w ]
    | `XL, [opcode; bt; ba; bb; opt_opcode; x] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 bt; word ~width:5 ba
          ; word ~width:5 bb; word ~width:10 opt_opcode; word ~width:1 x ]
    | `XL, [opcode; bo; bi; empty; bh; opt_opcode; lk] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 bo; word ~width:5 bi
          ; word ~width:3 empty; word ~width:2 bh; word ~width:10 opt_opcode
          ; word ~width:1 lk ]
    | `XFX, [opcode; rs; x; data; y; opt_opcode; z] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rs; word ~width:1 x
          ; word ~width:8 data; word ~width:1 x; word ~width:10 opt_opcode
          ; word ~width:1 z ]
    | `XFX, [opcode; rs; data; opt_opcode; x] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rs; word ~width:10 data
          ; word ~width:10 opt_opcode; word ~width:1 x ]
    | `XO, [opcode; rt; ra; rb; oe; opt_opcode; rc] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rt; word ~width:5 ra
          ; word ~width:5 rb; word ~width:1 oe; word ~width:9 opt_opcode
          ; word ~width:1 rc ]
    | `XS, [opcode; rs; ra; sh1; opt; sh2; rc] ->
        make_bytes
          [ word ~width:6 opcode; word ~width:5 rs; word ~width:5 ra
          ; word ~width:5 sh1; word ~width:9 opt; word ~width:1 sh2
          ; word ~width:1 rc ]

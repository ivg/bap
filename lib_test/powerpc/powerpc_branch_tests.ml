...
let make_bo ?ctr_before ?ctr_after ?cond_reg0 ?(expect_jump = false) bo_field
  ...
  let ctr_after = Option.map ~f:(fun x -> Word.of_int ~width:bits x) ctr_after in
  let cond_reg0 =
    match cond_reg0 with None -> Word.b0 | Some x -> Word.of_bool (x = 1) in
...
let bcXrX name opt_opcode lk reg arch case (_ctxt : test_ctxt) =
  ...
  let cr_num = 31 in
  let bytes =
    make_insn ~name ~arch `XL [19; case.bo_field; cr_num; 0; 0; opt_opcode; lk]
  in
...
let suite =
  "branch"
  >::: [ "b32 +" >:: b `ppc pos; "b32 -" >:: b `ppc neg; "ba32 +" >:: ba `ppc pos
       ; "ba32 -" >:: ba `ppc neg; "bl32 +" >:: bl `ppc pos
       ; "bl32 -" >:: bl `ppc neg; "bla32 +" >:: bla `ppc pos
       ; "bla32 -" >:: bla `ppc neg
       ...
       ; "bcctrl32 cond_not" >:: bcctrl `ppc cond_not; "b64 +" >:: b `ppc64 pos
       ; "b64 -" >:: b `ppc64 neg; "ba64 +" >:: ba `ppc64 pos
       ; "ba64 -" >:: ba `ppc64 neg; "bl64 +" >:: bl `ppc64 pos
       ; "bl64 -" >:: bl `ppc64 neg; "bla64 +" >:: bla `ppc64 pos
       ; "bla64 -" >:: bla `ppc64 neg

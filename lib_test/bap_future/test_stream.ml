...
let watch ctxt =
  ...
  let buf = Queue.create () in
  let once id x =
    if x > 3 then Stream.unsubscribe ss id else Queue.enqueue buf x in
...
let apply ctxt =
  ...
  let ss, signal, _ =
    Array.init (List.length expected) ~f:(fun i x -> x + i) |> Stream.of_array
  in
  let ss', signal', _ = Stream.of_list values in
...
let foldw_range ~max_stride ~max_width ~f ~init ctxt =
  ...
  let r = try List.iter ~f:run all ; true with _ -> false in
...
let suite =
  ...
  >::: [ "send signal" >:: send_signal
       ...
       ; "repeat signal: 4" >:: repeat_signal ~times:4; "create" >:: create
       ; "from" >:: from; "unfold" >:: unfold; "unfold_until" >:: unfold_until
       ; "unfold'" >:: unfold'; "repeat" >:: repeat 42 ~times:3
       ; "repeat" >:: repeat 42 ~times:0; "of_list" >:: of_list
       ; "of_array" >:: of_array; "of_sequence" >:: of_sequence
       ; "watch" >:: watch; "observe" >:: observe; "subscribe" >:: subscribe
       ; "unsubscribe" >:: unsubscribe; "on_wait" >:: on_wait
       ...
       ; "same watch id" >:: same_watch_id; "map'" >:: map'; "map" >:: map
       ; "apply" >:: apply; "split" >:: split; "unzip" >:: unzip
       ; "once" >:: once; "parse" >:: parse; "before" >:: before
       ; "hd" >:: hd values; "hd empty" >:: hd []; "tl" >:: tl

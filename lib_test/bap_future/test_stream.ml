...
let foldw ?(stride=1) ~f ~width ~values ~expect ctxt =
  let ss,signal,_ = Stream.of_list values in
  let init = Int.zero in
  ...
  let rec run ind = function
    ...
    | (time_ind :: times) as line ->
      if time_ind = ind then
        (Signal.send clk_signal ();
         run ind times)
      else
        (Signal.send signal ();
         run (Int.succ ind) line) in
  run 0 time_line
...
let suite =  
  ...
  [
    ...
    foldw ~f:(+) ~width:3 ~values ~expect:[3; 6; 9; 12; 15; 18] ;

    "foldw stride=2 width=3" >::
    foldw ~f:(+) ~stride:2 ~width:3 ~values ~expect:[3; 9; 15];

    "foldw stride=2 width=1" >::
    foldw ~f:(+) ~stride:2 ~width:1 ~values ~expect:[0;2;4;6;];

    "foldw stride=3 width=1" >:: 
    foldw ~f:(+) ~stride:3 ~width:1 ~values ~expect:[0; 3; 6;] ;

    "foldw stride=4 width=2" >:: 
    foldw ~f:(+) ~stride:4 ~width:2 ~values ~expect:[1; 9;] ;

    "foldw stride=1 width=1" >::
    foldw ~f:(+) ~stride:1 ~width:1 ~values ~expect:values;

...
module Test_algo (Gl : Graph_for_algo) = struct
  ...
  module Span = struct
    ...
    let add_relation t ~parent ~child =
      { children=
          Map.change t.children parent (function
  ...
  let theorem_1_5 stamps graph ctxt : bool =
    Graphlib.depth_first_search
      (module Gl)
      graph ~init:true
      ~enter_edge:(fun kind e -> function false -> false
        | true -> (
            let x =
  ...
  let random_flowgraph size =
    let g =
      Seq.range 0 size
      |> Seq.fold ~init:Gl.empty ~f:(fun g i -> Gl.Node.insert (node i) g) in
    let g =
      Seq.range 0 (size - 1)
  ...
  let suite name =
    name
    >::: [ "Random Graphs" >::: randoms; "Random Flow Graphs" >::: doms
...
module Test_IR = struct
  ...
  let insert blks =
    ...
    let etalon =
      G.edges g
      |> Seq.fold ~init:etalon ~f:(fun etalon e -> G.Edge.remove e etalon) in
    [ ( "all edges inserted"
      >:: fun ctxt ->
...
let suite () =
  ...
  >::: [ "Algo"
       ...
       ; "Construction" >::: [Test_int100.suite]; "IR" >::: Test_IR.suite ()

...
type case =
  {name: string; arch: arch; addr: int; code: string; bil: string; asm: string}
...
let test_substitute case =
  ...
  [ test case.asm "$asm"; test case.bil "$bil"; test sub_name "$symbol"
  ; test sub_name "$symbol_name"; test min_addr "$symbol_min_addr"
  ; test max_addr "$symbol_max_addr"; test min_addr "$symbol_addr"
  ; test min_addr "$block_addr"; test max_addr "$block_max_addr"
  ; test min_addr "$addr"; test min_addr "$min_addr"; test max_addr "$max_addr"
  ; has_filename ]

let suite () =
  "Project" >::: ["ARM" >::: test_substitute arm; "386" >::: test_substitute x86]

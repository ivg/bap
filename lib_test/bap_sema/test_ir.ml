...
let remove_with_nil =
  transform Term.concat_map (fun v d -> if Var.(Def.lhs d = v) then [] else [d])
...
let split split xs ys blk ctxt =
  ...
  def_order xs b1 ctxt ; def_order ys b2 ctxt
...
module Example = struct
  ...
  let check_free_vars =
    "free_vars"
    >::: [ "entry" >:: blk_free_vars [] entry; "b1" >:: blk_free_vars [] b1
         ; "b2" >:: blk_free_vars [i; j] b2; "b3" >:: blk_free_vars [i] b3
         ; "b4" >:: blk_free_vars [k] b4; "b5" >:: blk_free_vars [j] b5
         ; "b6" >:: blk_free_vars [i] b6; "exit" >:: blk_free_vars [] exit
  ...
  let phi_node sub var var_ver blk blk_ver ctxt =
    ...
    | Some phi -> (
        ...
        |> Seq.find ~f:(fun (tid, exp) ->
               ...
               | _ -> assert false)
        |> function None -> assert_string "wrong phi-node" | Some _ -> () )

  let phi_node = phi_node (Sub.ssa sub)

  let check_phi_nodes =
    "phi nodes"
    >::: [ "i.2=i.1" >:: phi_node i 2 b1 1; "i.2=i.3" >:: phi_node i 2 b3 3
         ; "j.2=j.1" >:: phi_node j 2 b1 1; "j.2=j.3" >:: phi_node j 2 b3 3
         ; "k.2=k.1" >:: phi_node k 2 b1 1; "k.2=k.3" >:: phi_node k 2 b3 3
         ; "i.5=i.2" >:: phi_node i 5 b5 2; "i.5=i.4" >:: phi_node i 5 b6 4 ]

  let tests = [check_free_vars; check_phi_nodes]
end
...
let suite () =
  ...
  >::: [ "order(xyoz)" >:: def_order [x; y; o; z] xyoz
       ...
       ; "lookup(a)" >:: lookup def_t def_a; "lookup(y)" >:: lookup def_t def_y

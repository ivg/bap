...
let x86_64 =
  ( "x86_64"
  , [ (* sub $0x8,%rsp *) (sub, ["SUB64ri8"; "RSP"; "RSP"; "0x8"], [])
    ; (* callq 942040 *)
      (call, ["CALL64pcrel32"; "-0x11b9"], [`Call; `May_affect_control_flow])
    ; (* mov 0x10(%rax),%eax *)
      (mov, ["MOV32rm"; "EAX"; "RAX"; "0x1"; "Nil"; "0x10"; "Nil"], [`May_load])
    ; (* add $0x8, %rsp *) (add, ["ADD64ri8"; "RSP"; "RSP"; "0x8"], [])
    ; (* "retq" *)
      (ret, ["RET"], [`Return; `Barrier; `Terminator; `May_affect_control_flow])
    ] )
...
let test_run_all (arch, samples) ctxt =
  ...
  Dis.with_disasm ~backend:"llvm" arch ~f:(fun dis ->
      ...
      Dis.run ~return:Or_error.return ~init:()
        ...
        ~stopped:(fun s () ->
          Or_error.return
          @@ List.iter2_exn samples (Dis.insns s) ~f:(fun (data, exp, kinds) ->
               function
...
let block1 =
  [ "\x04\xB0\x2D\xE5"; "\x00\xB0\x8D\xE2"; "\x14\xD0\x4D\xE2"
  ; "\x10\x00\x0B\xE5"; "\x00\x30\xA0\xE3"; "\x08\x30\x0B\xE5"
  ; "\x05\x00\x00\xEA" ]

let block2 =
  [ "\x10\x30\x1B\xE5"; "\x01\x30\x83\xE2"; "\x10\x30\x0B\xE5"
  ; "\x08\x30\x1B\xE5"; "\x01\x30\x83\xE2"; "\x08\x30\x0B\xE5" ]

let block3 = ["\x10\x30\x1B\xE5"; "\x00\x00\x53\xE3"; "\xF6\xFF\xFF\x1A"]

let block4 =
  [ "\x08\x30\x1B\xE5"; "\x03\x00\xA0\xE1"; "\x00\xD0\x4B\xE2"
  ; "\x04\xB0\x9D\xE4"; "\x1E\xFF\x2F\xE1" ]
...
let blocks =
  [| create_block 0x840Cl block1; create_block 0x8428l block2
   ; create_block 0x8440l block3; create_block 0x844Cl block4 |]
...
let build_graph dis : graph =
  ...
  |> Seq.fold ~init:[] ~f:(fun graph blk ->
         let preds =
           Seq.map (Cfg.Node.preds blk cfg) ~f:(fun blk -> blk_num blk) in
         let dests =
...
let test_micro_cfg insn ctxt =
  ...
  Rec.cfg dis |> Cfg.nodes |> Seq.to_list |> List.hd_exn |> Block.insns
  |> function
  | [(mem, _)] ->
...
let suite () =
  ...
  >::: [ "x86_64/one" >:: test_insn_of_mem x86_64
       ; "x86_64/all" >:: test_run_all x86_64; "recurse" >:: test_cfg amount
       ; "addresses" >:: test_cfg addresses; "structure" >:: test_cfg structure
       ; "ret" >:: test_micro_cfg ret; "call" >:: test_micro_cfg call

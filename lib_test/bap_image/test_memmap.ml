  ...
  List.iter seq ~f:(fun (k, v) ->
      let msg =
        sprintf "(%d,[%a,%a]) is not dominated by [%a,%a]" v Addr.pps
          ...
          (uw (Memmap.min_addr map))
          Addr.pps
          (uw (Memmap.max_addr map)) in
...
let three_equal cons size ctxt =
  ...
  let map = map_of_list lst in
  let map =
    List.foldi ~init:map [c3; c3; c3] ~f:(fun i map key -> Memmap.add map key i)
  in
...
let intersections cons size ctxt =
  ...
  let is_in mem mem' =
    Memory.(contains mem (min_addr mem') || contains mem (max_addr mem')) in
  let collect mem' =
    List.filter_map lst ~f:(fun (mem, x) ->
  ...
  List.iter (cons size) ~f:(fun (mem, _) ->
      ...
      let got =
        ...
        |> List.dedup_and_sort ~compare:Int.compare in

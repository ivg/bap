  ...
  let img1, img2 =
    let ((img1_opt, img2_opt) as images) =
      ( Bap_fileutils.readfile file |> Bap_native_loader.of_data,
        Bap_fileutils.readfile file |> Bap_llvm_loader.of_data )
    in
    let () =
      match images with
      ...
      | _ -> ()
    in
    (Option.value_exn img1_opt, Option.value_exn img2_opt)
  in

  assert_equal ~ctxt ~printer:Arch.to_string ~msg:"loaders gives different arch"
    (Img.arch img1) (Img.arch img2);

  assert_equal ~ctxt ~printer:Addr.to_string
    ~msg:"loaders gives different entry point" (Img.entry img1) (Img.entry img2);

  assert_equal ~ctxt
    ~printer:(fun e -> List.sexp_of_t Segment.sexp_of_t e |> Sexp.to_string)
    ~msg:"loaders gives different segments"
    (Img.segments img1 |> fun (e, ee) -> e :: ee)
    (Img.segments img2 |> fun (e, ee) -> e :: ee);

  assert_equal ~ctxt
    ~printer:(fun e -> List.sexp_of_t Section.sexp_of_t e |> Sexp.to_string)
    ~msg:"loaders gives different sections" (Img.sections img1)
    (Img.sections img2);

  let sym1, sym2 =
    let symbols img = Img.symbols img |> SS.of_list in
    (symbols img1, symbols img2)
  in

  if not (SS.subset sym1 sym2) then
    let string_of_set s =
      SS.fold ~init:[]
        ~f:(fun acc s -> (Symbol.sexp_of_t s |> Sexp.to_string) :: acc)
        s
      |> String.concat ~sep:"\n"
    in
    let diff12 = SS.diff sym1 sym2 |> string_of_set in
    let diff21 = SS.diff sym2 sym1 |> string_of_set in

    String.concat ~sep:"\n"
      [
        file;
        "llvm loader losted symbols:";
        diff12;
        "llvm loader surplused symbols:";
        diff21;
      ]
    |> assert_string

let suite = "Image" >::: [ "llvm_loader" >:: run_test ]

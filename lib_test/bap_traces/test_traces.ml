...
let bin =
  Binary.{path= "home"; args= [|"go"|]; envp= [|"drink=coffee"|]; md5sum= ""}
  ...
  let s =
    Trace.read_all_matching t
      Value.Match.(
        ...
        @@ default (fun () -> `Unknown)) in
  let s' =
    Seq.filter s ~f:(fun x -> [%compare.equal: [`Memory | `Unknown]] x `Memory)
  in
...
let suite () =
  "Trace"
  >::: [ "save_and_load" >:: save_and_load; "id" >:: id; "tool" >:: trace_tool
       ; "set_and_get_attr" >:: set_and_get_attr empty binary bin
       ; "has_attr" >:: has_attr empty binary bin; "set_meta" >:: set_meta empty
       ; "find_all_matching" >:: find_all_matching; "supports" >:: supports
       ; "fail_monitor" >:: fail_monitor; "miss_monitor" >:: miss_monitor
       ; "stop_monitor" >:: stop_monitor; "pack_monitor" >:: pack_monitor
       ; "warn_monitor" >:: warn_monitor; "user_monitor" >:: user_monitor ]

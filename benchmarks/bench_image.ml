...
let sum_foldi () =
  let (_ : word) =
    Memory.foldi mem ~f:(fun _ w1 w2 -> Word.Int_exn.(w1 + w2)) ~init:zero in
  ()

let sum_fold () =
  let (_ : word) =
    Memory.fold mem ~f:(fun w1 w2 -> Word.Int_exn.(w1 + w2)) ~init:zero in
  ()
...
let sum_fold_tab () =
  let (_ : word) =
    Table.fold tab ~f:(fun w1 w2 -> Word.Int_exn.(w1 + w2)) ~init:zero in
  ()

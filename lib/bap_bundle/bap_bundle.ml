...
module Std = struct
  ...
  module Manifest = struct
    ...
    let create ?(author = getenv "USER") ?(version = "1.0.0") ?main
        ?(date = Unix.time ()) ?(desc = "description not provided")
  ...
  module Nameof = struct let manifest = "MANIFEST.scm" end
  ...
  module Bundle = struct
    ...
    let of_uri uri =
      ...
      let name =
        if String.mem base '.' then Filename.chop_extension base else base in
      {name; path}

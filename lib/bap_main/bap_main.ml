      ...
      | Malformed_line (linenum, str) ->
          fail
            "File %S, line %d, characters %d-%d\n\
             Syntax error: expects <parameter> = <value>" filename (linenum + 1)
            1
            (String.length str + 1)
end
...
module Type = struct
  ...
  let refine t validate =
    { t with
      print= (fun x -> validate x ; t.print x)
    ; parse=
        (fun s ->
          let x = t.parse s in
          validate x ; x)
  ...
  let wrap ?digest (parser, printer) default =
    let parse s = match parser s with `Ok s -> s | `Error s -> invalid_arg s in
  ...
  let digest_files ?(max_checks = 4096) path =
    ...
    let digest_path path time =
      Digest.string @@ sprintf "%s:%Ld" path (Int64.bits_of_float time) in
...
module Error = struct
  ...
  let pp ppf e = Format.pp_print_string ppf (Caml.Printexc.to_string e)
    ...
    let rec build = function
      ...
      | para :: paras -> (
        ...
        | None -> (
          ...
          | `End para -> `P "ERROR: Unbalanced ```" :: `P para :: build paras
          | `Nocode -> (
...
let first_sentence text = List.hd @@ String.split_on_chars text ~on:['\n'; '.']
...
module Context = struct
  type value = {scope: string; value: string; digest: string} [@@deriving sexp]
  type t = {env: value Map.M(String).t; plugins: Set.M(String).t Map.M(String).t}
  ...
  let builder =
    ref
    @@ Building
         {env= Map.empty (module String); plugins= Map.empty (module String)}
  ...
  let set_plugins disabled plugins commands =
    ...
    List.iter commands ~f:(fun (plugin, commands) ->
        if not (Set.mem disabled plugin) then
          Hashtbl.add_exn command_descriptions plugin commands) ;
    update
    @@ fun ctxt ->
    { env= Map.filter ctxt.env ~f:(fun {scope} -> not (Set.mem disabled scope))
    ; plugins=
  ...
  let make_filter features exclude =
    ...
    and filtered =
      match exclude with None -> fun _ -> false | Some xs -> intersects xs in
  ...
  let digest ctxt =
    ...
    Caml.Digest.string @@ Buffer.contents buffer
...
module Grammar : sig
    ...
    Key
      (List.fold keys ~init ~f:(fun terms key ->
           ...
           let doc =
             ...
             | Some doc -> doc key in
           let vopt =
             match as_flag with None -> None | Some f -> Some (f key) in
  ...
  let switch ?doc ?(aliases = fun _ -> []) opts inj =
    let doc = Option.value doc ~default:(fun s -> sprintf "Select %s" (inj s)) in
    let opts =
    ...
    plugin_info :=
      ...
      @@ Option.value_map !plugin_info
           ~f:(fun {tags; cons; docs} ->
             {tags= tags @ provides; cons= cons @ features; docs= docs ^ doc})
  ...
  let () =
    ...
    | `Loaded p ->
        ...
        if not (List.is_empty !actions) then
          Hashtbl.add_exn plugin_cmds name !actions ;
        reset_plugin () ;
  ...
  let get_from_env name =
    ...
    try Some (Caml.Sys.getenv name) with Caml.Not_found -> None
  ...
  let ( >>> ) t1 t2 =
    Term.(
      const (fun t1 t2 -> match t1 with Error _ as err -> err | Ok () -> t2)
      $ t1 $ t2)
  ...
  let progname = Caml.Filename.basename Caml.Sys.executable_name
  ...
  let eval_plugins disabled plugins_term =
    ...
    Context.set_plugins disabled plugins commands
    >>= fun () ->
  ...
  let help_options ?version ppf plugins =
    ...
    let make_help_option name =
      let doc =
        sprintf "prints more information about the $(b,%s) plugin" name in
      let docs = plugin_section name in
      Arg.(
        value
        & opt ~vopt:(Some `Auto) (some (enum fmts)) None
        & info ~doc ~docs [name ^ "-help"]) in
  ...
  let eval ?(man = "") ?(name = progname) ?version ?env
    ...
    let argv =
      ...
      | Some cmd -> (
        ...
        | [_] | [] -> argv ) in
    match
      Term.eval_choice ~catch:false ?env ~help ?err ~argv (main, main_info)
        commands
    with
    | `Ok (Ok ()) -> Ok ()
...
module Extension = struct
  ...
  module Configuration = struct
    ...
    let parameter ?as_flag conv =
      Grammar.extend atom conv
      @@ fun conv def info -> Arg.value @@ Arg.opt ?vopt:as_flag conv def info

    let parameter ?doc ?as_flag ?aliases typ name =
      parameter ?doc ?as_flag ?aliases typ name

    let parameters ?as_flag conv =
      Grammar.extend list conv
      @@ fun conv def info ->
  ...
  module Syntax = struct let ( --> ) ctxt v = Configuration.get ctxt v end
  module Error = Error
end
...
let load_recipe recipe =
  let paths =
    [Stdlib.Filename.current_dir_name; Extension.Configuration.datadir] in

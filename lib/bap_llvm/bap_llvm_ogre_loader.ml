...
module Ogre_loader (P : Parameters) = struct
  ...
  let filetype_of_string s =
    try typ_of_sexp (Sexp.of_string s) with _ -> Unknown

  let make x =
    let module Target = (val x : Loader_target) in
    Fact.require is_relocatable
    >>= fun is_rel ->
    if is_rel then
      let module S = Target.Relocatable.Make (Fact) in
      Fact.return (module S : S)
    else Fact.return (module Target.Make (Fact) : S)

  let of_filetype =
    Fact.require file_type
    >>= fun s ->
  ...
  let image_base =
    Fact.require is_relocatable
    >>= fun is_rel ->
    if Option.is_none P.image_base && is_rel then
      Fact.return (Some relocatable_base)
    else Fact.return P.image_base

  let provide_base =
    Fact.require default_base_address
    >>= fun default ->
    image_base
    >>= function
  ...
  let provide_entry =
    ...
    >>= fun base ->
    Fact.require entry
    >>= fun addr -> Fact.provide entry_point Int64.(base + addr)
  ...
  let provide x =
    ...
    S.segments
    >>= fun () ->
    S.sections >>= fun () -> S.symbols >>= fun () -> S.code_regions
...
module Loader (P : Parameters) = struct
  ...
  let _ = Callback.register_exception "Llvm_loader_fail" (Llvm_loader_fail 0)
  ...
  let map_file path =
    ...
    try
      let data =
        ...
        Mmap.V1.map_file fd Bigarray.char Bigarray.c_layout false [|-1|] in
      Unix.close fd ;
  ...
  let from_file path = Or_error.(map_file path >>= from_data path)
  let from_data data = from_data "" data
end

let init ?base ?(pdb_path = Sys.getcwd ()) () =
  Image.register_loader ~name:"llvm"
    (module Loader (struct let image_base = base let pdb_path = pdb_path end))

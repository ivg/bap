...
let make_segment arch bin ind =
  ...
  let perm =
    Backend.(
      [ (segment_is_readable bin ind, R); (segment_is_writable bin ind, W)
...
let make_symbol arch bin ind =
  ...
  let locations =
    ( Location.Fields.create ~addr:(make_addr arch addr)
        ~len:(Int64.to_int_exn size)
    , [] ) in
...
let collection arch bin (f, max) =
  List.init (max bin) ~f:(f arch bin) |> List.filter_opt

let nonempty = function [] -> None | hd :: tl -> Some (hd, tl)
let segment = (make_segment, segments_number)
let section = (make_section, sections_number)
let symbol = (make_symbol, symbols_number)

let with_err_message msg f x =
  match f x with None -> eprintf "%s\n" msg ; None | r -> r

let arch_of_string arch =
  with_err_message (sprintf "unknown arch %s\n" arch) Arch.of_string arch
...
let of_data data : Backend.Img.t option =
  ...
  create_binary data
  >>= fun bin ->
  arch_of_string (image_arch bin)
  >>= fun arch ->
  collect_segments arch bin
  >>= fun segments ->
  ...
  Some (Backend.Img.Fields.create ~arch ~entry ~segments ~symbols ~sections)

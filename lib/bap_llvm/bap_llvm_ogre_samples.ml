...
module Symbols (Fact : Ogre.S) = struct
  ...
  let symbols =
    ...
    >>= fun base ->
    Fact.collect Ogre.Query.(select (from symbol_entry))
    >>= fun s ->
    Fact.Seq.iter s ~f:(fun (name, relative_addr, size, off) ->
        if Int64.(size = 0L) then Fact.return ()
          ...
          Fact.provide named_symbol full_addr name
          >>= fun () ->
          Fact.provide symbol_chunk full_addr size full_addr
          >>= fun () ->
          Fact.request code_entry ~that:(fun (n, o, s) ->
              Int64.(o = off) && String.(n = name) && Int64.(s = size))
          >>= fun f ->
...
module Sections (Fact : Ogre.S) = struct
  ...
  let sections =
    Fact.require base_address
    >>= fun base ->
    Fact.collect Ogre.Query.(select (from section_entry))
    >>= fun s ->
    Fact.Seq.iter s ~f:(fun (name, relative_addr, size, _) ->
...
module Relocatable_symbols (Fact : Ogre.S) = struct
  ...
  let relocations =
    ...
    >>= fun base ->
    Fact.collect Ogre.Query.(select (from ref_internal))
    >>= fun ints ->
  ...
  let externals =
    Base.from_sections_offset
    >>= fun base ->
    Fact.collect Ogre.Query.(select (from ref_external))
    >>= fun exts ->
  ...
  let symbols =
    ...
    Fact.Seq.iter s ~f:(fun (name, _, size, off) ->
        if Int64.(size = 0L) then Fact.return ()
          ...
          Fact.provide named_symbol addr name
          >>= fun () ->
...
module Code_regions (Fact : Ogre.S) = struct
  ...
  let code_regions =
    Fact.foreach
      Ogre.Query.(
        select
          (from named_region $ code_entry)
          ~join:
            [ [field name]
            ; [field size ~from:named_region; field size ~from:code_entry] ])
      ~f:(fun {addr; size} (_, off, _) -> (addr, size, off))
    >>= fun s ->

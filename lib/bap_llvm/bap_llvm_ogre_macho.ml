...
module Make (Fact : Ogre.S) = struct
  ...
  let segments =
    ...
    Fact.foreach
      Ogre.Query.(
        select
          (from segment_cmd $ segment_cmd_flags $ virtual_segment_cmd)
          ~join:[[field name]])
      ~f:(fun (name, offset, size) (_, rwx) (_, addr, vsize) ->
        (name, offset, size, Int64.(base + addr), vsize, rwx))
    >>= fun s ->
    Fact.Seq.iter s ~f:(fun (name, off, size, addr, vsize, (r, w, x)) ->
        Fact.provide segment addr vsize r w x
        >>= fun () ->
        Fact.provide named_region addr vsize name
        >>= fun () -> Fact.provide mapped addr size off)
  ...
  module Make (Fact : Ogre.S) = struct
    ...
    let segments =
      ...
      Fact.Seq.iter s ~f:(fun (name, _, size, off) ->
          ...
          Fact.provide segment addr size true true true
          >>= fun () ->

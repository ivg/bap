...
type jumptarget = Jabs of operand | Jrel of addr * addr

(* next ins address, offset *)
...
module Pcmpstr = struct
  ...
  type outselectsig = LSB | MSB

  (* For PCMPESTRI/PCMPISTRI, choosees LSB or MSB.  *)

  type outselectmask = Bitmask | Bytemask

  (* For PCMPESTRM/PCMPISTRM, represents bit mask/word mask. *)

...
module Oid : Tid = struct
  ...
  let is_atom x = x land of_int 0b01 = zero && x land of_int 0b10 <> zero
  ...
  let fits_int x =
    x >= of_int Int.min_value && x <= of_int Int.max_value
    [@@inline]
  ...
  let untagged x = if is_number x then x asr 1 else x asr 2 [@@inline]
  let pp ppf x = Format.fprintf ppf "<%#0Lx>" (Int63.to_int64 x)
end
...
type slot_status = Sleep | Awoke | Ready
...
module Name = struct
  ...
  let read ?(package = user_package) s =
    ...
    | None -> {package; name= normalize_name `Reading ~package s}
    | Some 0 -> {package= keyword_package; name= escape ~is_keyword:true s}
    | Some len ->
        let package = escape (String.sub s ~pos:0 ~len) in
        { package
        ; name= normalize_name `Reading ~package @@ String.subo s ~pos:(len + 1)
        }
...
module Opinions : sig
  ...
  type 'a t = {opinions: 'a opinion list; equal: 'a -> 'a -> bool; empty: 'a}
  ...
  let add_opinion op ({opinions; equal} as ops) =
    ...
    if casted then {ops with opinions} else {ops with opinions= op :: opinions}
  ...
  let choice {opinions; empty} =
    List.max_elt opinions ~compare
    |> function Some {opinion} -> opinion | None -> empty
end
...
module Domain = struct
  ...
  let mapping (type k o d)
    ...
    let order x y =
      ...
      |> Sequence.fold ~init:(0, 0, 0) ~f:(fun (l, m, r) ->
           function
           | _, `Left _ -> (l + 1, m, r)
           | _, `Right _ -> (l, m, r + 1)
           | _, `Unequal _ -> (l, m + 1, r))
      |> function
      | 0, 0, 0 -> Order.EQ | 0, 0, _ -> LT | _, 0, 0 -> GT | _, _, _ -> NC
    in
  ...
  let optional ?(inspect = sexp_of_opaque) ?join ~equal name =
    ...
    let join x y =
      ...
      | None, x | x, None -> Ok x
      | Some x, Some y -> (
        match join_data x y with Ok x -> Ok (Some x) | Error err -> Error err )
    in
...
module Persistent = struct
  type 'a t =
    ...
    | Derive :
        {of_persistent: 'b -> 'a; to_persistent: 'a -> 'b; persistent: 'b t}
        -> 'a t
  ...
  module Chunks = struct type t = Chunk.t list [@@deriving bin_io] end
  module KVS = struct type t = KV.t list [@@deriving bin_io] end
  ...
  let array p =
    derive chunks
      ~to_persistent:
        (Array.fold ~init:[] ~f:(fun xs x -> {Chunk.data= to_string p x} :: xs))
      ~of_persistent:
...
module Registry = struct
  ...
  let register kind namespace ?desc ?(package = user_package) name =
    if is_present ~package namespace name then
      failwithf
        "Failed to declare new %s, there is already a %s named `%s' in package \
         `%s'"
...
module Documentation = struct
  ...
  module Pair = struct
    ...
    let name = fst
    let desc = function _, None -> "" | _, Some d -> d
  end
...
module Class = struct
  type +'s info = {id: Cid.t; name: fullname; sort: 's}
  ...
  let newclass ?(public = false) ?desc ?package name sort =
    Cid.incr classes ;
    let id = !classes and name = Registry.add_class ?desc ?package name in
...
module Dict = struct
  module Key = struct
    ...
    type 'a t = {ord: Uid.t; key: 'a typeid; name: string; show: 'a -> Sexp.t}

    let newtype (type a) () : a typeid =
      let module Type = struct
        type t = a type _ witness += Id : t witness
      end in
      (module Type)
 ...
 *)
  ...
  type 'r visitor = {visit: 'a. 'a key -> 'a -> 'r -> 'r} [@@unboxed]
  ...
  let rec pop_min t {app} =
    ...
    | LL (x, ka, a, y) ->
        pop_min x {app= (fun kb b x -> app kb b (LL (x, ka, a, y)))}
    | EQ (x, ka, a, y) ->
        pop_min x {app= (fun kb b x -> app kb b (EQ (x, ka, a, y)))}
    | LR (x, ka, a, y) ->
        pop_min x {app= (fun kb b x -> app kb b (LR (x, ka, a, y)))}
    [@@inline]

  let rec pop_max t {app} =
    match t with
    ...
    | LL (x, ka, a, y) ->
        pop_max y {app= (fun kb b y -> app kb b (LL (x, ka, a, y)))}
    | EQ (x, ka, a, y) ->
        pop_max y {app= (fun kb b y -> app kb b (EQ (x, ka, a, y)))}
    | LR (x, ka, a, y) ->
        pop_max y {app= (fun kb b y -> app kb b (LR (x, ka, a, y)))}
    [@@inline]
  ...
  let shake_left = function
    | LL
        ...
        LL
          ...
          , kb
          , b
          , EQ (T3 (kc, c, kd, d, ke, e), kf, f, T4 (kg, g, kh, h, ki, i, kj, j))
          )
    | LL
        ( T1 (kx, x)
        , ka
  ...
  *)
  let rec insert : type a. a key -> a -> record -> record =
   fun ka a -> function
    ...
    | EQ (T0, kb, b, T4 (kc, c, kd, d, ke, e, kf, f)) ->
        if ka <$ kd then
          if ka <$ kc then
            if ka <$ kb then make6 ka a kb b kc c kd d ke e kf f
            else make6 kb b ka a kc c kd d ke e kf f
          else make6 kb b kc c ka a kd d ke e kf f
        else if ka <$ ke then make6 kb b kc c kd d ka a ke e kf f
        else if ka <$ kf then make6 kb b kc c kd d ke e ka a kf f
    ...
    | EQ (T1 (kb, b), kc, c, T4 (kd, d, ke, e, kf, f, kg, g)) ->
        if ka <$ kd then
          if ka <$ kc then
            if ka <$ kb then make7 ka a kb b kc c kd d ke e kf f kg g
            else make7 kb b ka a kc c kd d ke e kf f kg g
          else make7 kb b kc c ka a kd d ke e kf f kg g
        else if ka <$ kf then
          if ka <$ ke then make7 kb b kc c kd d ka a ke e kf f kg g
          else make7 kb b kc c kd d ke e ka a kf f kg g
        else if ka <$ kg then make7 kb b kc c kd d ke e kf f ka a kg g
    ...
    | EQ (T2 (kb, b, kc, c), kd, d, T4 (ke, e, kf, f, kg, g, kh, h)) ->
        if ka <$ ke then
          if ka <$ kc then
            if ka <$ kb then make8 ka a kb b kc c kd d ke e kf f kg g kh h
            else make8 kb b ka a kc c kd d ke e kf f kg g kh h
          else if ka <$ kd then make8 kb b kc c ka a kd d ke e kf f kg g kh h
          else make8 kb b kc c kd d ka a ke e kf f kg g kh h
        else if ka <$ kg then
          if ka <$ kf then make8 kb b kc c kd d ke e ka a kf f kg g kh h
          else make8 kb b kc c kd d ke e kf f ka a kg g kh h
        else if ka <$ kh then make8 kb b kc c kd d ke e kf f kg g ka a kh h
    ...
    | EQ (T3 (kb, b, kc, c, kd, d), ke, e, T4 (kf, f, kg, g, kh, h, ki, i)) ->
        if ka <$ ke then
          if ka <$ kc then
            if ka <$ kb then make9 ka a kb b kc c kd d ke e kf f kg g kh h ki i
            else make9 kb b ka a kc c kd d ke e kf f kg g kh h ki i
          else if ka <$ kd then
            make9 kb b kc c ka a kd d ke e kf f kg g kh h ki i
          else make9 kb b kc c kd d ka a ke e kf f kg g kh h ki i
        else if ka <$ kg then
          if ka <$ kf then make9 kb b kc c kd d ke e ka a kf f kg g kh h ki i
          else make9 kb b kc c kd d ke e kf f ka a kg g kh h ki i
        else if ka <$ kh then make9 kb b kc c kd d ke e kf f kg g ka a kh h ki i
        else if ka <$ ki then make9 kb b kc c kd d ke e kf f kg g kh h ka a ki i
        else make9 kb b kc c kd d ke e kf f kg g kh h ki i ka a
    ...
    | EQ
        (T4 (kb, b, kc, c, kd, d, ke, e), kf, f, T4 (kg, g, kh, h, ki, i, kj, j))
      ->
        if ka <$ kf then
          if ka <$ kc then
            if ka <$ kb then
            ...
            if ka <$ kd then
              make10 kb b kc c ka a kd d ke e kf f kg g kh h ki i kj j
          ...
          if ka <$ kh then
            if ka <$ kg then
              make10 kb b kc c kd d ke e kf f ka a kg g kh h ki i kj j
          ...
          make10 kb b kc c kd d ke e kf f kg g kh h ki i ka a kj j
        else make10 kb b kc c kd d ke e kf f kg g kh h ki i kj j ka a
    | LL (((T0 | T1 _ | T2 _ | T3 _) as b), k, x, (EQ (T4 _, _, _, T4 _) as c))
      as t ->
        if ka <$ k then LL (insert ka a b, k, x, c)
        else insert ka a (shake_left t)
    | LL ((T4 _ as b), k, x, c) when ka <$ k -> EQ (insert ka a b, k, x, c)
    | LR (b, k, x, (T4 _ as c)) when k <$ ka -> EQ (b, k, x, insert ka a c)
    ...
    | EQ (x, kb, b, (EQ (T4 _, _, _, T4 _) as y)) ->
        if ka <$ kb then EQ (insert ka a x, kb, b, y)
        else
          pop_min y
          @@ {app= (fun kc c y -> EQ (insert kb b x, kc, c, insert ka a y))}
    | EQ ((EQ (T4 _, _, _, T4 _) as x), kb, b, y) ->
        if ka <$ kb then
          pop_max x
          @@ {app= (fun kc c x -> EQ (insert ka a x, kc, c, insert kb b y))}
  ...
  (* [merge k x y] *)
  type merge = {merge: 'a. 'a key -> 'a -> 'a -> 'a} [@@unboxed]
  ...
  let sexp_of_t dict =
    Sexp.List
      (foreach ~init:[] dict
         { visit=
             (fun k x xs ->
               Sexp.List [Sexp.Atom (Key.name k); Key.to_sexp k x] :: xs) })

  let pp_field ppf (k, v) =
    Format.fprintf ppf "%s : %a" (Key.name k) Sexp.pp_hum (Key.to_sexp k v)
  ...
  let pp_elt ppf (k, v) = Format.fprintf ppf "%a" Sexp.pp_hum (Key.to_sexp k v)
  ...
  let pp_key ppf {Key.name} = Format.fprintf ppf "%s" name
end
...
module Record = struct
  ...
  module Repr = struct
    type entry = {name: string; data: string} [@@deriving bin_io]
  ...
  type slot_io =
    {reader: string -> record -> record; writer: record -> string option}

  let io : slot_io Hashtbl.M(String).t = Hashtbl.create (module String)
  ...
  let merge_or_keep old our =
    Dict.foreach our ~init:old
      { visit=
          (fun kb b out ->
            match Dict.find kb old with
            | None -> Dict.insert kb b out
            | Some a -> (
              match (domain kb).join kb a b with
...
module Knowledge = struct
  ...
  module Env = struct
    type workers = {waiting: Pid.Set.t; current: Pid.Set.t} [@@deriving bin_io]
    ...
    type t = {classes: objects Cid.Map.t; package: string} [@@deriving bin_io]
  end

  type state = Env.t

  let of_bigstring = Binable.of_bigstring (module Env)
  let to_bigstring = Binable.to_bigstring (module Env)
  let empty : Env.t = {package= user_package; classes= Map.empty (module Cid)}
  ...
  module Slot = struct
    type 'p promise = {run: Oid.t -> unit Knowledge.t; pid: pid}
  ...
  module Value = struct
    ...
    let put {Slot.key} v x =
      {v with data= Record.put key v.data x; time= next_second ()}
  ...
  module Class = struct
    ...
    module Abstract = struct let property = Slot.declare end
  end
  ...
  let provide : type a p. (a, p) slot -> a obj -> p -> unit Knowledge.t =
   fun slot obj x ->
    if Domain.is_empty slot.dom x then Knowledge.return ()
    else
      get ()
      >>= function
      | {classes} as s -> (
          ...
          try
            put
              { s with
                classes=
                  Map.set classes ~key:slot.cls.id
                    ~data:
                      { objs with
                        vals=
                          Map.update vals obj ~f:(function
                            | None -> Record.(put slot.key empty x)
                            | Some v -> (
                              match Record.commit slot.dom slot.key v x with
                              | Ok r -> r
  ...
  let objects {Class.id} =
    get ()
    >>| fun {classes} ->
    match Map.find classes id with None -> Env.empty_class | Some objs -> objs
  ...
  let update_slot : ('a, _) slot -> 'a obj -> _ -> unit knowledge =
   fun slot obj f ->
    ...
    >>= fun ({comp} as objs) ->
    let comp =
      Map.update comp obj ~f:(fun slots ->
          let slots =
            match slots with
  ...
  let enter_slot : ('a, _) slot -> 'a obj -> unit knowledge =
   fun s x ->
    ...
    | Some _ -> assert false
    | None ->
        Work {waiting= Set.empty (module Pid); current= Set.empty (module Pid)}
  ...
  let enter_promise s x p =
    update_work s x
    @@ fun {waiting; current} -> Work {waiting; current= Set.add current p}

  let leave_promise s x p =
    update_work s x
    @@ fun {waiting; current} -> Work {waiting; current= Set.remove current p}
  ...
  let collect_waiting : ('a, 'p) slot -> 'a obj -> _ Knowledge.t =
   fun s x ->
    objects s.cls
    >>| fun {comp} ->
    Map.find_exn (Map.find_exn comp x) (uid s)
    |> function
  ...
  let rec collect_inner : ('a, 'p) slot -> 'a obj -> _ -> _ =
   fun slot obj promises ->
    ...
    Knowledge.List.iter promises ~f:(fun {Slot.run; pid} ->
        enter_promise slot obj pid
        >>= fun () -> run obj >>= fun () -> leave_promise slot obj pid)
    >>= fun () ->
    collect_waiting slot obj
    >>= fun waiting ->
    dequeue_waiting slot obj
    >>= fun () ->
    match waiting with
    | [] -> Knowledge.return ()
    | promises -> (
  ...
  let collect : type a p. (a, p) slot -> a obj -> p Knowledge.t =
   fun slot id ->
    status slot id
    >>= function
    | Ready -> current slot id
    | Awoke -> enqueue_promises slot id >>= fun () -> current slot id
    | Sleep ->
        enter_slot slot id
        >>= fun () ->
        collect_inner slot id (initial_promises slot)
        >>= fun () -> leave_slot slot id >>= fun () -> current slot id

  let resolve slot obj = collect slot obj >>| Opinions.choice

  let suggest agent slot obj x =
    current slot obj
    >>= fun opinions -> provide slot obj (Opinions.add agent x opinions)

  let propose agent s get =
    register_promise s @@ fun obj -> get obj >>= suggest agent s obj
  ...
  module Object = struct
    ...
    let with_new_object objs f =
      match Map.max_elt objs.Env.vals with
      | None ->
          f Oid.first_atom
            { objs with
              vals= Map.singleton (module Oid) Oid.first_atom Record.empty }
      | Some (key, _) ->
          let key = Oid.next key in
          f key {objs with vals= Map.add_exn objs.vals ~key ~data:Record.empty}

    let create : ('a, _) cls -> 'a obj Knowledge.t =
     fun cls ->
      objects cls
      >>= fun objs ->
      with_new_object objs
      @@ fun obj objs ->
      ( update
      @@ function
      | {classes} as s ->
    ...
    *)
    let delete {Class.id} obj =
      ...
      | {classes} as s ->
          { s with
            classes=
              Map.change classes id ~f:(function
                | None -> None
                | Some objs ->
                    Some
                      { objs with
    ...
    let scoped cls scope =
      create cls
      >>= fun obj -> scope obj >>= fun r -> delete cls obj >>| fun () -> r
        ...
        | Some names -> (
          ...
          | Some obj ->
              if is_public ~package obj objects then unchanged obj
                ...
                put {s with classes= Map.set classes id objects}
                >>| fun () -> obj )
    ...
    let intern ?public ?desc ?package name cls =
      match package with
      | Some package -> do_intern ?public ?desc (Name.create ~package name) cls
      | None ->
          get ()
          >>= fun {Env.package} ->
          let name =
            {package; name= Name.normalize_name `Literal ~package name} in
          do_intern ?public ?desc name cls

    let uninterned_repr cls obj = Format.asprintf "#<%s %a>" cls Oid.pp obj

    let to_string {Class.name= cls as fname; id= cid} {Env.package; classes} obj
        =
      let cls =
        if String.equal package cls.package then cls.name
        else Name.to_string fname in
      match Map.find classes cid with
      | None -> uninterned_repr cls obj
      | Some {Env.syms} -> (
        match Map.find syms obj with
        | Some fname ->
            if String.equal fname.package package then fname.name
            else Name.to_string fname
        | None -> uninterned_repr cls obj )

    let repr cls obj = get () >>| fun env -> to_string cls env obj

    let read cls input =
      try
        Scanf.sscanf input "#<%s %s@>"
        @@ fun _ obj -> Knowledge.return (Oid.atom_of_string obj)
      with _ ->
        get () >>= fun {Env.package} -> do_intern (Name.read ~package input) cls
  ...
  module Symbol = struct
    ...
    let intern_symbol ~package name obj cls =
      Knowledge.return
        Env.
          { cls with
            objs=
              Map.update cls.objs package ~f:(function
                | None -> Map.singleton (module String) name obj
    ...
    *)
     ...
     fun cls ->
      ...
      |> Knowledge.Seq.fold ~init:cls ~f:(fun cls (obj, sym) ->
             if not (needs_import cls sym obj) then Knowledge.return cls
               ...
               let obj' =
                 match Map.find cls.objs package with
                 | None -> Oid.zero
                 | Some names -> (
                   match Map.find names sym.name with
                   | None -> Oid.zero
                   | Some obj' -> obj' ) in
               if (not strict) || Oid.(obj' = zero || obj' = obj) then
                 intern_symbol ~package sym.name obj cls
               else
                 let sym' = Map.find_exn cls.syms obj' in
                 Knowledge.fail (Import (sym, sym')))

    let package_exists package =
      Map.exists ~f:(fun {Env.objs} -> Map.mem objs package)

    let name_exists {package; name} =
    ...
    let check_name classes = function
      | `Pkg name ->
          if package_exists name classes then Knowledge.return ()
  ...
  module Data : sig
    ...
    let add_cell {Class.id} objects oid cell =
      ...
      update (fun s ->
          {s with classes= Map.set s.classes id {objects with data; heap}})
      >>| fun () -> oid
    ...
    let cons cls car cdr =
      ...
      | {data; heap} as s -> (
        ...
        | None -> (
          match Map.max_elt heap with
          | None -> add_cell cls s Oid.first_cell cell
          | Some (id, _) -> add_cell cls s (Oid.next id) cell ) )

    let case cls x ~null ~atom ~cons =
      if Oid.is_null x then null
      else if Oid.is_atom x || Oid.is_number x then atom x
      else
        objects cls
        >>= fun {Env.heap} ->
        ...
        Map.iteri vals ~f:(fun ~key:oid ~data ->
            if not (Dict.is_empty data) then
              let () =
                ...
                | None -> Format.fprintf ppf "@[<2>(%a@ " Oid.pp oid
                | Some name ->
                    Format.fprintf ppf "@[<2>(%a@ " (pp_fullname ~package) name
              in

    ...
    let join_data =
      ...
      | None ->
          fun x y ->
            ...
            else Error (Join (name, inspect x, inspect y))
    in
...
module Knowledge = struct
  ...
  module Env = struct
    type workers = { waiting : Pid.Set.t; current : Pid.Set.t }
    [@@deriving bin_io]
    ...
    type t = { classes : objects Cid.Map.t; package : string }
    [@@deriving bin_io]
  end
  ...
  let empty : Env.t =
    { package = user_package; classes = Map.empty (module Cid) }
  ...
  let enter_slot : ('a, _) slot -> 'a obj -> unit knowledge =
   fun s x ->
    ...
    | None ->
        Work
          { waiting = Set.empty (module Pid); current = Set.empty (module Pid) }
  ...
  module Object = struct
    ...
    let with_new_object objs f =
      ...
      | Some (key, _) ->
          ...
          f key
            { objs with vals = Map.add_exn objs.vals ~key ~data:Record.empty }
    ...
    *)
    ...
    let do_intern =
      ...
      let createsym ~public ~package name classes clsid objects s =
        ...
        let objects =
          if public then publicize ~package obj objects else objects
        in
    ...
    let to_string { Class.name = cls as fname; id = cid }
        { Env.package; classes } obj =
      let cls =

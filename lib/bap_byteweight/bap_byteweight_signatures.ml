...
let zip_error entry err = fail (`Corrupted (sprintf "%s: %s" entry err))
...
let load_exn ?comp ?path ~mode arch =
  ...
  let r =
    ...
    with
    | Caml.Not_found -> fail (`No_entry entry_path)
    | Zip.Error (_, ent, err) -> zip_error ent err in
  Zip.close_in zip ; r
...
let save_exn ?comp ~mode ~path arch data =
  ...
  let old =
    ...
    | Zip.Error (_, ent, err) -> zip_error ent err in
  let zip =
    try Zip.open_out path with Sys_error msg -> fail (`Sys_error msg) in
  try

  ...
  module Make2 (M : B2) : S2 with type ('a, 'e) t := ('a, 'e) M.t = struct
    ...
    module Lift = struct
      ...
      let quaternary f a b c d =
        a >>= fun a -> b >>= fun b -> c >>= fun c -> d >>| fun d -> f a b c d

      let quinary f a b c d e =
        a
        >>= fun a ->
        b >>= fun b -> c >>= fun c -> d >>= fun d -> e >>| fun e -> f a b c d e
    ...
    open Syntax
    module Pair = struct let fst x = !$fst x let snd x = !$snd x end
    ...
    module Collection = struct
      ...
      module Eager_base (T : Types.Collection.Eager) = struct
        ...
        let foldr xs ~f ~init =
          T.fold xs ~init:M.return ~f:(fun k x a -> f x a >>= k) init
      ...
      module Delay (B : Types.Collection.Delay) = struct
        module Base = Delay_base (B) include Make (Base)
      end

      module Eager (B : Types.Collection.Eager) = struct
        module Base = Eager_base (B) include Make (Base)
      end
    end
...
module Ident : Monad.S with type 'a t = 'a = struct
  ...
  module Pair = struct let fst = fst let snd = snd end

  module Triple = struct
    let fst (x, _, _) = x let snd (_, x, _) = x let trd (_, _, x) = x
  end
...
module OptionT = struct
  ...
  module T1 (M : T1) = struct
    type 'a t = 'a option M.t type 'a m = 'a M.t type 'a e = 'a t
  end
  ...
  module Make2 (M : Monad.S2) :
    ...
    module Basic = struct
      ...
      let bind m f =
        M.bind m (function Some r -> f r | None -> M.return None)
        [@@inline]
...
module ResultT = struct
  ...
  module Makep (T : T1) (M : Monad.S) :
    ...
    module Base = struct
      ...
      let map' m ~f =
        m
        >>= function Ok r -> return (f r) | Error err -> M.return (Error err)
        [@@inline]
  ...
  module Error = struct
    module T (M : Monad.S) = struct
      type 'a m = 'a M.t type 'a t = 'a Or_error.t m type 'a e = 'a Or_error.t m
    end
  ...
  module Self :
    ...
    include T2 (Ident) include Make2 (Ident)
  end
...
module ListT = struct
  ...
  module T1 (M : T1) = struct
    type 'a t = 'a list M.t type 'a m = 'a M.t type 'a e = 'a t
  end
  ...
  module Make2 (M : Monad.S2) :
    ...
    module Base = struct
      ...
      let bind xsm f =
        ...
        List.fold xs ~init:!![] ~f:(fun ysm x ->
            ysm >>= fun ys -> f x >>| fun xs -> List.rev_append xs @@ ys)
        >>| List.rev
    ...
    let plus xsm ysm = xsm >>= fun xs -> ysm >>= fun ys -> M.return (xs @ ys)
    let run = ident
  ...
  struct
    include T1 (Ident) include Make (Ident)
  end
...
module Seq = struct
  ...
  module T1 (M : T1) = struct
    type 'a t = 'a Sequence.t M.t type 'a m = 'a M.t type 'a e = 'a t
  end
  ...
  module Make2 (M : Monad.S2) :
    ...
    module Base = struct
      ...
      let bind xsm f =
        ...
        Sequence.fold xs ~init:!!Sequence.empty ~f:(fun ysm x ->
            ysm >>= fun ys -> f x >>| fun xs -> Sequence.append xs ys)
    ...
    let plus xsm ysm =
      xsm >>= fun xs -> ysm >>= fun ys -> M.return (Sequence.append xs ys)
...
module State = struct
  ...
  module Makep (T : T1) (M : Monad.S) :
    ...
    module Basic = struct
      ...
      let bind m f =
        make @@ fun s -> m => s >>= fun {x; s} -> f x => s
        [@@inline]

      let map m ~f =
        make @@ fun s -> m => s >>| fun {x; s} -> {x= f x; s}
        [@@inline]
  ...
  module Multi = struct
    ...
    type 'e contexts =
      { created: id
      ; (* the id of last created fork *)
        current: id
      ; (* the id of current context   *)
        parents: id Id.Map.t
      ; (* tree of forks               *)
    ...
    module Makep (T : T1) (M : Monad.S) :
      ...
      module SM = struct
        module Env = struct type 'a t = 'a T.t contexts end
        include STp (Env) (M)
        include Makep (Env) (M)
      end
...
module Fun = struct
  ...
  module T1 (M : Monad.S) = struct
    type 'a m = 'a M.t type 'a t = 'a m thunk type 'a e = 'a m
  end
...
module LazyT = struct
  ...
  module T1 (M : Monad.S) = struct
    type 'a m = 'a M.t type 'a t = 'a m Lazy.t type 'a e = 'a m
  end
...
module Cont = struct
  ...
  module Makep (T : T1) (M : Monad.S) :
    ...
    module Base = struct
      ...
      let bind m f = cont @@ fun k -> run m @@ fun x -> run (f x) k

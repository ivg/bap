...
let report_missing_meta pkg =
  failwithf
    "Package %s was used to build the host program, but its meta information \
     is not available at runtime. To be able to use plugins without installed \
     META files, either update the version of OCaml to 4.08 or newer or \
     provide them at the compilation time via the `used_MOD` predicates for \
     each `MOD` linked into the\n\
    \    host binary." pkg ()
...
let extract_units_from_packages ~findlib_is_required =
  ...
  |> List.iter ~f:(fun pkg ->
         Hashtbl.set units (unit_of_package ~findlib_is_required pkg) `In_core)

...
let endian proj =
  Project.arch proj |> Arch.endian
  |> function LittleEndian -> "little" | BigEndian -> "big"
  ...
  Name.Map.of_alist_exn
    [ ( "arch"
      , features
        ...
        @ attr proj Bap_abi.name ); ("abi", features @@ attr proj Bap_abi.name)
...
let t = Attribute.register ~name:"context" ~add ~parse
...
let ( <= ) ctxt ctxt' =
  let sups =
    Map.merge ctxt ctxt' ~f:(fun ~key:_ ->
      function
      | `Left _ -> None
      | `Right features' ->
          if Set.is_empty features' then None else Some features'
      | `Both (features, features') ->
          if Set.is_subset features' ~of_:features then None else Some features')
  in
  Map.is_empty @@ sups

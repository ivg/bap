...
module Signal = struct
  ...
  let create ?(docs = "") ~types name : t =
    { data= {meta= {name; docs; attrs= Attribute.Set.empty}; code= types}
    ; id= Source.Id.null
    ; eq= Source.Eq.null }
...
module Para = struct
  ...
  let with_default t default = {t with data= {t.data with code= {default}}}
end
...
module Macro = struct
  ...
  let subst bs body =
    let rec sub : tree -> tree list = function
      ...
      | {data= Atom x} as atom -> (
        match find bs x with None -> [atom] | Some cs -> cs ) in
    match body with
    | {data= List xs; id} ->
        {data= List (List.concat_map xs ~f:sub); id; eq= unknown}
    | {data= Atom x} as atom -> (
...
module Primitive = struct
  ...
  let create ?(docs = "") name code =
    { data= {meta= {name; docs; attrs= Attribute.Set.empty}; code}
    ; id= Id.null
    ; eq= Eq.null }
...
module Closure = struct
  ...
  let create ?types ?(docs = "") name lambda =
    { data=
        {meta= {name; docs; attrs= Attribute.Set.empty}; code= {types; lambda}}
    ; id= Id.null
    ; eq= Eq.null }

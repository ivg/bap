...
module Repository = struct
  ...
  let get ?package name = require @@ Name.create ?package name
...
let pp ppf {name; components; depends_on; desc} =
  Format.fprintf ppf
    "@[<v2>(defsystem %a@\n\
     :description \"@[<hov>%a@]\"@\n\
     :depends-on (@[<v>%a@])@\n\
     :components (@[<v>%a@]))@]" Name.pp name Format.pp_print_text desc pp_names
    depends_on pp_names components
...
module Parser = struct
  ...
  let comp name s = {s with components= Name.read ~package name :: s.components}
  let deps name s = {s with depends_on= Name.read ~package name :: s.depends_on}
  ...
  let rec parse_items : t -> Sexp.t list -> (t, error) Result.t =
   fun sys -> function
    ...
    | [Atom ":description"] -> Error {expects= Literal_description; got= List []}
    ...
    | [Atom ":components"] -> Error {expects= List_of_components; got= List []}
    | Atom ":depends-on" :: (Atom _ as atom) :: _ ->
        Error {expects= List_of_systems; got= atom}
    | [Atom ":depends-on"] -> Error {expects= List_of_systems; got= List []}
    | (Atom _ as unknown) :: _ -> Error {expects= Known_keyword; got= unknown}

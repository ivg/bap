...
module MCG = struct
  ...
  module Bitvec_M16 = Bitvec.Make (struct let modulus = Bitvec.modulus 16 end)
  ...
  module Cat (Output : Size) (Sub : S) : S = struct
    ...
    module Z = Bitvec.Make (struct let modulus = Bitvec.modulus Output.size end)
    ...
    let create seed : t =
      Array.init generators (fun salt -> Sub.create (seed + salt))
  ...
  *)
  module Range (R : Range) (Gen : S) = struct
    include Gen
    module Z = Bitvec.Make (struct let modulus = Bitvec.modulus R.size end)

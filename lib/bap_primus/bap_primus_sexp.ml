...
let rec sexp_of_exp = function
  ...
  | Bil.Load (e1, e2, endian, size) ->
      Sexp.List
        [ Sexp.Atom "load"; sexp_of_exp e1; sexp_of_exp e2; sexp_of_endian endian
        ; sexp_of_size size ]
  | Bil.Store (e1, e2, e3, _, `r8) ->
      Sexp.List (Sexp.Atom "store" :: sexps_of_exps [e1; e2; e3])
  | Bil.Store (e1, e2, e3, endian, size) ->
      Sexp.List
        [ Sexp.Atom "store"; sexp_of_exp e1; sexp_of_exp e2; sexp_of_exp e3
        ; sexp_of_endian endian; sexp_of_size size ]
  | Bil.BinOp (op, x, y) ->
      Sexp.List
  ...
  | Bil.Cast (ct, n, x) ->
      Sexp.List [Sexp.Atom (Bil.string_of_cast ct); sexp_of_int n; sexp_of_exp x]
  | Bil.Let (v, x, y) ->
      Sexp.List
        [ Sexp.Atom "let"; Sexp.List [Sexp.List [sexp_of_var v; sexp_of_exp x]]
        ; sexp_of_exp y ]
  | Bil.Ite (c, x, y) ->

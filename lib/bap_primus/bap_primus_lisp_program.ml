...
let merge p1 p2 =
  let p1, p2 = if Source.is_empty p1.sources then (p1, p2) else (p2, p1) in
  { p2 with
...
let rec concat_prog p =
  List.concat_map p ~f:(function {data= Seq xs} -> concat_prog xs | x -> [x])
  ...
  *)
  ...
  module Tvals : sig
    ...
    let meet id x y =
      ...
      | Error _, _ -> x
      | _, Error _ -> y
      | Ok t1, Ok t2 -> {result= meet_tvals x.origin y.origin t1 t2; origin= id}
  ...
  *)
  module Gamma : sig
    ...
    *)
    ...
    let add_var x v g = {g with vars= Map.add_exn g.vars x v}
    let set_var x v g = {g with vars= Map.set g.vars x v}
    ...
    *)
    ...
    let merge g1 g2 =
      ...
      let g =
        { vars=
            Map.merge g1.vars g2.vars ~f:(fun ~key:_ ->
              function
              | `Left u | `Right u -> Some u
              | `Both (u, v) when Tvar.equal u v -> Some u
              | `Both _ -> None)
        ; vals=
            Map.merge g1.vals g2.vals ~f:(fun ~key:_ ->
              function
    ...
    let partition {vars; vals} =
      ...
      |> Map.fold ~init:[] ~f:(fun ~key ~data xs ->
             {var= key; elements= Id.Set.of_list data; values= Map.find vals key}
             :: xs)

    module Group = struct
      let elements g = g.elements let values g = g.values
    end
  ...
  let apply glob id name args gamma =
    ...
    |> List.hd
    |> function
    | None -> Gamma.fail id (Unresolved_function (id, name, sigs)) gamma
    | Some gamma -> gamma
  ...
  let constr_special {paras} var gamma =
    ...
    if is_special name && not (Set.mem paras name) then
      Gamma.fail var.id (Unresolved_parameter (var.id, name)) gamma
    else gamma
  ...
  let make_globs =
    Seq.fold ~init:String.Map.empty ~f:(fun vars v ->
        ...
        | Type.Imm x -> Map.set vars ~key:(Var.name v) ~data:x
        | Type.Mem _ | Type.Unk -> vars)
  ...
  let applicable global =
    List.filter ~f:(fun def ->
        match Lisp.Attribute.Set.get (Def.attributes def) Lisp.Context.t with
        | None -> true
      ...
      | No_unification (x, t1, y, t2) ->
          fprintf ppf
            "\n\
             ...
             %ahas type %a, but it is expected to have type %a after its \
             unification\n\
             with the expression:@\n\
             %a@\n"

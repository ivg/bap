...
let sexp_of_cast ((t, s, v), r) =
  results r @@ sexps [Bil.string_of_cast t; string_of_int s; string_of_value v]
...
module Make (Machine : Machine) = struct
  ...
  let failf fmt =
    Format.ksprintf (fun msg () -> Machine.raise (Runtime_error msg)) fmt
  ...
  let undefined t =
    value (word_of_type t) >>= fun r -> !!on_undefined r >>| fun () -> r

  let get v =
    !!on_reading v
    >>= fun () ->
    Env.get v >>= fun r -> post on_read ~f:(fun k -> k (v, r)) >>| fun () -> r

  let call_when_provided name =
    Code.is_linked (`symbol name)
    >>= fun provided ->
  ...
  let tick =
    ...
    >>= fun s ->
    !!tick s.time
    >>= fun () -> Machine.Local.put state {s with time= Time.succ s.time}

  let unop op x =
    value (Bil.Apply.unop op x.value)
    >>= fun r ->
    tick >>= fun () -> post on_unop ~f:(fun k -> k ((op, x), r)) >>| fun () -> r

  let cast t s x =
    value (Bil.Apply.cast t s x.value)
    >>= fun r -> post on_cast ~f:(fun k -> k ((t, s, x), r)) >>| fun () -> r

  let concat x y =
    value (Word.concat x.value y.value)
    >>= fun r -> post on_concat ~f:(fun k -> k ((x, y), r)) >>| fun () -> r

  let extract ~hi ~lo x =
    value (Word.extract_exn ~hi ~lo x.value)
    >>= fun r ->
  ...
  let binop op x y =
    ...
    | (Bil.DIVIDE | Bil.SDIVIDE | Bil.MOD | Bil.SMOD) when Word.is_zero y.value
      ->
        !!will_divide_by_zero ()
        >>= fun () ->
        call_when_provided division_by_zero_handler
        >>= fun called ->
    ...
    | _ ->
        ...
        coerce s y
        >>= fun y ->
        value (Bil.Apply.binop op x.value y.value)
        >>= fun r ->
        tick
        >>= fun () ->
        post on_binop ~f:(fun k -> k ((op, x, y), r)) >>| fun () -> r

  let const c = value c >>= fun r -> !!on_const r >>| fun () -> r
  ...
  let trapped_memory_access access =
    Machine.catch access (function
      | Bap_primus_memory.Pagefault a ->
          !!page_fail a
          >>= fun () ->
          call_when_provided pagefault_handler
          >>= fun trapped ->
  ...
  let load_byte a =
    !!on_loading a
    >>= fun () ->
    trapped_memory_access (Memory.get a.value)
    >>= fun r ->
    tick >>= fun () -> post on_loaded ~f:(fun k -> k (a, r)) >>| fun () -> r

  let store_byte a x =
    !!on_storing a
    >>= fun () ->
    trapped_memory_access (Memory.set a.value x)
    >>= fun () -> tick >>= fun () -> post on_stored ~f:(fun k -> k (a, x))

  let ite cond yes no =
    value (if Word.is_one cond.value then yes.value else no.value)
    >>= fun r ->
  ...
  let rec eval_exp x =
    ...
    !!exp_entered x
    >>= fun () -> eval x >>= fun r -> !!exp_left x >>| fun () -> r

  and eval_let v x y =
    eval_exp x
    >>= fun x ->
    push_lexical v x
    >>= fun () -> eval_exp y >>= fun r -> pop_lexical v >>| fun () -> r

  and eval_ite cond yes no =
    eval_exp yes
    >>= fun yes ->
    eval_exp no >>= fun no -> eval_exp cond >>= fun cond -> ite cond yes no

  and eval_load m a =
    eval_exp a
    >>= fun a ->
    switch_memory m >>= fun () -> eval_storage m >>= fun () -> load_byte a
  ...
  and eval_store m a x =
    ...
    >>= fun x ->
    switch_memory m
    >>= fun () -> eval_storage m >>= fun () -> store_byte a x >>| fun () -> a

  and eval_binop op x y =
    eval_exp x >>= fun x -> eval_exp y >>= fun y -> binop op x y
  ...
  and eval_concat x y =
    eval_exp x >>= fun x -> eval_exp y >>= fun y -> concat x y
  ...
  let rec do_store a x s hd tl =
    ...
    if s = 8 then Machine.return ()
      ...
      succ a
      >>= fun a -> cast tl (s - 8) x >>= fun x -> do_store a x (s - 8) hd tl
  ...
  let update_pc t =
    ...
    | Some addr ->
        Machine.Local.get state
        >>= fun s ->
        Machine.Local.put state {s with addr}
        >>= fun () ->
  ...
  let leave cls curr t =
    ...
    >>= fun () -> !!term_left (Term.tid t) >>= fun () -> !!pos_left curr

  let branch cnd yes no =
    Value.zero (Value.bitwidth cnd)
    >>= fun zero ->
    binop Bil.EQ cnd zero
    >>= fun is_zero ->
  ...
  let term return cls f x t =
    ...
    | Ok curr ->
        update_pc t
        >>= fun () ->
        Machine.Local.update state (fun s -> {s with curr})
        >>= fun () ->
        enter cls curr t
        >>= fun () -> f x t >>= fun r -> leave cls curr t >>= fun () -> return r
  ...
  let label cond : label -> _ = function
    ...
    | Indirect x -> (
        ...
        tick
        >>= fun () ->
        post will_jump ~f:(fun k -> k (cond, dst))
        >>= fun () ->
        Code.resolve_tid (`addr value)
        >>= function
        | None -> Code.exec (`addr value) | Some dst -> exec_to_prompt dst )
  ...
  let trap_cfi_violation callsite =
    !!cfi_will_diverge callsite
    >>= fun () ->
    call_when_provided cfi_violation_handler
    >>= function
    | true -> Machine.return ()
  ...
  let call cond call =
    ...
    | {prompts= p :: _} -> (
      ...
      | None -> Machine.return ()
      | Some p' when Tid.(p <> p') ->
          trap_cfi_violation callsite
          >>= fun () -> pop_prompt >>= fun () -> label cond (Direct p')
  ...
  let jmp constr t =
    ...
    | Init, false ->
        !!on_cond v >>= fun () -> unop Bil.NOT v >>| fun u -> Conj u
    | Conj u, false ->
        binop Bil.AND u v >>= !!on_cond
        >>= fun () ->
        unop Bil.NOT v >>= fun v -> binop Bil.AND u v >>| fun w -> Conj w
    | Conj u, true ->
        binop Bil.AND u v >>= fun w -> !!on_cond w >>| fun () -> Disj (w, t)
    | Disj (u, t), _ ->
        unop Bil.NOT u
        >>= fun p ->
        binop Bil.AND p v >>= !!on_cond
        >>= fun () -> binop Bil.OR u v >>| fun w -> Disj (w, t)

  let jmp = term normal jmp_t jmp

  let blk () t =
    Machine.Seq.iter (Term.enum def_t t) ~f:def
    >>= fun () -> Machine.Seq.fold ~init:Init (Term.enum jmp_t t) ~f:jmp
  ...
  let sub () t =
    ...
    | Some entry ->
        ...
        iter_args t arg_def
        >>= fun () ->
        get_args ~input:true t >>| Seq.to_list_rev
        >>= fun inputs ->
        ...
        post Linker.Trace.call_returned ~f:(fun k ->
            get_args ~input:false t >>| Seq.to_list
            >>= fun rets ->
...
module LinkBinaryProgram (Machine : Machine) = struct
  ...
  let is_linked name t =
    ...
    |> Machine.List.all >>| List.exists ~f:ident
    >>= function
  ...
  let link name code t m =
    ...
    | true -> Machine.return ()
    | false ->
        Linker.link ~name:(name t) ~tid:(Term.tid t)
          ?addr:(Term.get_attr t address) code
  ...
  let init () =
    Machine.get ()
    >>= fun proj -> linker#run (Project.program proj) (Machine.return ())
end

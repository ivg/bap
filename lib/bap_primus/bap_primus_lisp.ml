...
let width_of_ctxt proj = Size.in_bits (Arch.addr_size (Project.arch proj))
...
let () =
  Exn.add_printer (function
    ...
    | Unresolved (name, res) ->
        let msg =
          asprintf "unable to resolve function %s, because %a" name
            Lisp.Resolve.pp_resolution res in
        Some msg
    | _ -> None)
...
module Interpreter (Machine : Machine) = struct
  ...
  let allocate_stack_frame args =
    ...
    | Some (sp, dir, max) ->
        ...
        >>= Eval.set sp
        >>= fun () -> Machine.return (Some (sp, sp_value))
  ...
  let eval_sub : value list -> 'x = function
    ...
    | sub_addr :: sub_args -> (
        ...
        |> Seq.find ~f:(fun sub ->
               ...
               | None -> false
               | Some addr -> Word.(addr = sub_addr.value))
        |> function
        | None ->
        ...
        | Some sub -> (
            ...
            |> Machine.Seq.iter ~f:(fun (arg, x) ->
                   ...
                   if not (is_out_intent arg) then
                     ...
                     | Load (_, BinOp (op, Var sp, Int off), endian, size) ->
                         ...
                         Eval.const off
                         >>= fun off ->
                         Eval.binop op sp off
                         >>= fun addr -> Eval.store addr x endian size
                     | exp ->
            ...
            >>= fun () ->
            Linker.exec (`addr sub_addr.value)
            >>= fun () ->
  ...
  *)
  let rec eval_lisp name args : value Machine.t =
    ...
    | Some (Ok (fn, bs)) ->
        ...
        Eval.const Word.b0
        >>= fun init ->
        notify_when is_external Trace.call_entered name args
        >>= fun () ->
        eval_advices Advice.Before init name args
        >>= fun _ ->
        Machine.Local.put state (Vars.push_frame bs s)
        >>= fun () ->
        eval_exp (Lisp.Def.Func.body fn)
        >>= fun r ->
  ...
  and eval_advices stage init primary args =
    ...
    |> Machine.List.fold ~init ~f:(fun r def ->
           ...
           | Some adv ->
               if Set.mem (Advice.targets adv stage) primary then
                 match stage with
  ...
  and eval_exp exp =
    ...
    and let_ v e1 e2 =
      ...
      Machine.Local.update state ~f:(Vars.push v w)
      >>= fun () ->
      eval e2
      >>= fun r ->
    ...
    and lookup v =
      ...
      if Map.mem env.vars v then
        ...
        | false -> (
            ...
            |> List.find ~f:(fun p ->
                   String.equal (Lisp.Def.name p) (Var.name v))
            |> function
            | None -> Eval.get v
            | Some p ->
                eval (Lisp.Def.Para.default p)
                >>= fun x -> Eval.set v x >>| fun () -> x )
    and app n args =
      Machine.List.map args ~f:eval
      >>= fun args ->
      match n with
    ...
    in
    Machine.Local.update state (fun s -> {s with cur= exp.id})
    >>= fun () -> eval exp
  ...
  let eval_signal name args : unit Machine.t =
    ...
    | Some (Ok mets) ->
        Machine.List.iter mets ~f:(fun (met, bs) ->
            let bs, frame_size = Vars.make_frame s.width bs in
            Machine.Local.update state ~f:(Vars.push_frame bs)
            >>= fun () ->
            eval_exp (Lisp.Def.Meth.body met)
            >>= fun _ -> Machine.Local.update state ~f:(Vars.pop frame_size))
end
...
module Closure = struct
  ...
  module Make (Machine : Machine) = struct
    ...
    let name =
      Machine.Local.get closure_context
      >>= function
...
module Type = struct
  ...
  module Spec = struct
    ...
    let int arch = Lisp.Type.word (Size.in_bits (Arch.addr_size arch))
    let bool = word 1
    let byte = word 8
    let a : t = var "a"
    let b : t = var "b"
...
module Make (Machine : Machine) = struct
  ...
  module Typechecker = struct
    ...
    let run =
      ...
      let externals =
        invoke_subroutine_signature :: signatures_of_subs (Project.program proj)
        |> List.map ~f:(fun (n, s) -> (n, s arch)) in
      let typeenv = Lisp.Program.Type.infer ~externals vars s.program in
  ...
  let link_feature (name, _defs) =
    ...
    Lisp.Resolve.extern Lisp.Check.arg s.program Lisp.Program.Items.func name
      args
    |> function
    | None -> Machine.return ()
    ...
    | Some (Ok (fn, bs)) ->
        ...
        let module Code (Machine : Machine) = struct
          ...
          let failf ppf =
            Format.ksprintf
              (fun msg () -> Machine.raise (Runtime_error msg))
              ppf
          ...
          let eval_ret r =
            ...
            | None -> Machine.return ()
            | Some v -> (
              match Arg.rhs v with
              | Bil.Var reg -> Eval.set reg r
              | Bil.(Cast (LOW, rsize, Var reg)) ->
                  let vsize = size_of_reg reg in
                  Eval.get reg
                  >>= fun lhs ->
                  Eval.extract ~hi:(vsize - 1) ~lo:rsize lhs
                  >>= fun high -> Eval.concat high r >>= fun r -> Eval.set reg r
          ...
          let exec =
            ...
            Value.b0
            >>= fun init ->
            Interp.eval_advices Advice.Before init name args
            >>= fun _ ->
            Machine.Local.update state ~f:(Vars.push_frame bs)
            >>= fun () ->
            Interp.notify_when true Trace.call_entered name args
            >>= fun () ->
            Interp.notify_when true Trace.lisp_call_entered name args
            >>= fun () ->
            Interp.eval_exp (Lisp.Def.Func.body fn)
            >>= fun r ->
            Machine.Local.update state ~f:(Vars.pop frame_size)
            >>= fun () ->
            ...
            >>= fun () ->
            Interp.eval_advices Advice.After r name args >>= fun r -> eval_ret r
        end in
  ...
  let link_program program =
    ...
    Machine.Local.put state s >>= fun () -> link_globals () >>= link_features
  ...
  let define ?types ?docs name body =
    ...
    Lisp.Def.Closure.create ?types ?docs name body |> link_primitive
    ...
    |> Machine.List.iter ~f:(fun def ->
           let module Packed (M : Machine) = struct
             ...
             let run =
               ...
               |> function
               | Some code -> Lisp.Def.Primitive.body code | _ -> assert false
           end in
  ...
  let optimize () =
    ...
    let useless_subscriptions =
      Map.fold s.signals ~init:[] ~f:(fun ~key:name ~data:sub subs ->
          if Set.mem known_methods name then subs else sub :: subs) in
...
module Doc = struct
  ...
  let index p =
    Lisp.Program.Items.
      [ ("Macros", describe p macro); ("Substitutions", describe p subst)
      ; ("Constants", describe p const); ("Functions", describe p func)
      ; ("Methods", describe p meth); ("Parameters", describe p para)
      ; ("Primitives", describe p primitive); ("Signals", describe p signal) ]

  module Make (Machine : Machine) = struct
    open Machine.Syntax

    let generate_index : index Machine.t =
      Machine.Local.get state >>| fun s -> index s.program
  end
end

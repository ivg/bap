...
module Interpreter (Machine : Machine) = struct
  ...
  *)
  let rec eval_lisp name args : value Machine.t =
    ...
    Lisp.Resolve.defun Lisp.Check.value s.program func name args |> function
  ...
  let collect_globals arch s =
    ...
    let add attr def vars =
      ...
      | Some vars' ->
          ...
          @@ List.map ~f:(var_of_lisp_var default_width) vars'
    in
    Lisp.Program.get s.program Lisp.Program.Items.func
          ...
          let eval_ret r =
            ...
            | Some v -> (
                ...
                | Bil.(Cast (LOW, rsize, Var reg)) ->
                    ...
                    Eval.concat high r >>= fun r -> Eval.set reg r
  ...
  let optimize () =
    ...
    let known_methods =
      ...
      |> List.fold ~init:String.Set.empty ~f:(fun mets met ->
             Set.add mets (Lisp.Def.name met))
    in

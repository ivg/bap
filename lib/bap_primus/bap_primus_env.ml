...
let sexp_of_values values =
  Sexp.List
    ...
    |> Seq.map ~f:(fun (v, {value}) ->
           Sexp.List
             [ Sexp.Atom "set-var"; Sexp.Atom (Var.name v)
...
let sexp_of_random map =
  Sexp.List
    ...
    |> Seq.map ~f:(fun (v, gen) ->
           Sexp.List
             [ Sexp.Atom "gen-var"; Sexp.Atom (Var.name v)
...
let inspect_environment {values; random} =
  ...
  let sexp_of_var sexp_of_value var value =
    Sexp.(
      List
        [ Atom (Var.name var); sexp_of_value value
        ; Atom (Type.to_string (Var.typ var)) ]) in
...
module Make (Machine : Machine) = struct
  ...
  let null =
    Machine.get () >>| Project.arch >>| Arch.addr_size
    >>= fun s -> Value.zero (Size.in_bits s)
  ...
  let get var =
    ...
    | None -> (
      ...
      | Type.Imm width -> (
        ...
        | Some gen ->
            Generator.word gen width >>= Value.of_word
            >>= fun x ->
            set var x
            >>= fun () ->
  ...
  let has var =
    Machine.Local.get state
    >>| fun t -> Map.mem t.values var || Map.mem t.random var

  let keys dic = Map.to_sequence dic |> Seq.map ~f:fst

  let all =
    Machine.Local.get state
    >>| fun {values; random} -> Seq.append (keys values) (keys random)
end

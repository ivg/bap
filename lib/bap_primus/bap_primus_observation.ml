...
let add_observer observers key obs =
  ...
  | None ->
      (Map.add_exn observers key {last= 1; subs= [(1, obs)]}, Subs (key, 1))
  | Some {last; subs} ->
      ( Map.set observers key {last= last + 1; subs= (last + 1, obs) :: subs}
      , Subs (key, last + 1) )
...
module Make (Machine : Monad.S) = struct
  ...
  let push_data p key os ws x =
    ...
    apply ident x os >>= fun () -> apply Lazy.force sexp ws

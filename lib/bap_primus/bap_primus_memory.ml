...
type memory = {name: string; addr: int; size: int} [@@deriving bin_io, sexp]
...
let sexp_of_dynamic {lower; upper; value} =
  Sexp.(
    List
      [ Sexp.Atom "dynamic"; sexp_of_word lower; sexp_of_word upper
      ; Generator.sexp_of_t value ])

let sexp_of_mem mem =
  Sexp.List
    [ Sexp.Atom "static"; sexp_of_word (Memory.min_addr mem)
...
let inspect_state {values; layers} =
  ...
  Sexp.(
    List [List [Atom "values"; List values]; List [Atom "layers"; List layers]])
...
let virtual_memory arch =
  ...
  | (Type.Imm _ | Type.Unk) as t ->
      invalid_argf "The CPU.mem variable %a:%a is not a storage" Var.pps mem
        Type.pps t ()
  | Type.Mem (ks, vs) ->
...
let state =
  ...
  @@ fun p -> {mems= Descriptor.Map.empty; curr= virtual_memory (Project.arch p)}
...
module Make (Machine : Machine) = struct
  ...
  let update state f =
    ...
    | Some m ->
        Machine.Local.put state
          {s with mems= Map.set s.mems ~key:s.curr ~data:(f m)}
  ...
  let store_word arch bytesize ~addr word values =
    ...
    let rec loop written ~key word values =
      if written < bytes then
        Value.of_word (bite word)
        >>= fun data ->
        notify_if_generated key data values
        >>= fun () ->
  ...
  let remembered {values; layers} addr word =
    ...
    >>= fun arch ->
    store_word arch size addr word values
    >>= fun values ->
  ...
  let write addr value {values; layers} =
    ...
    | Some _ ->
        Machine.return {layers; values= Map.set values ~key:addr ~data:value}
  ...
  let initialize values lower upper f =
    let rec loop values addr =
      if addr < upper then
        ...
        >>= fun data ->
        Value.of_word data
        >>= fun data -> loop (Map.set values ~key:addr ~data) (Addr.succ addr)
      else Machine.return values in
    loop values lower
  ...
  let add_region ?(readonly = false) ?(executable = false) ?init ?generator
    ...
    | None -> put_curr s
    | Some f ->
        initialize s.values lower upper f
        >>= fun values -> put_curr {s with values}
  ...
  let del addr =
    update state @@ fun s -> {s with values= Map.remove s.values addr}
  ...
  let is_writable addr =
    ...
    |> function Some {perms= {readonly}} -> not readonly | None -> false
end

...
type resolution =
  ...
  ; (* definitions with the given name *)
    stage2: stage
  ; (* definitions applicable to the ctxt *)
    stage3: stage
  ; (* lower bounds of all definitions *)
    stage4: stage
  ; (* infinum *)
    stage5: stage (* overload *) }

type ('t, 'a, 'b) resolver =
  Program.t -> 't Program.item -> string -> 'a -> ('b, resolution) result option
...
let compare_def d1 d2 = Context.(compare (context d1) (context d2))
  ...
  | x :: xs ->
      if
        List.for_all xs ~f:(fun y ->
            match compare_def x y with Same -> true | _ -> false)
      then x :: xs
      else []
...
let overload_defun typechecks args s3 =
  let open Option in
  List.filter_map s3 ~f:(fun def ->
      zip (Def.Func.args def) args
      >>= fun bs -> if all_bindings typechecks bs then Some (def, bs) else None)
...
let subst prog item name () = run one interns overload_primitive prog item name
let const = subst

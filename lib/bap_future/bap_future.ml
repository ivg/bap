...
module Std = struct
  ...
  module Stream = struct
    ...
    let merge s1 s2 ~f =
      ...
      let drop () = ndrop q1 1; ndrop q2 1 in
      let step src q x =
        Queue.enqueue q x;
        let rec process () =
    ...
    let foldw ?(stride = 1) xs width ~init ~f =   
      let s, Signal publish = create () in
      let buf = Queue.create () in
      let to_drop = ref 0 in
      let is_enough () = 
        Queue.length buf = width && to_drop.contents = 0 in
      let reset () = 
        let y = Queue.fold ~init ~f buf in
        publish y;
        to_drop := stride in
      let drop () = 
        let available = min (Queue.length buf) !to_drop in
        ndrop buf available;
        to_drop := !to_drop - available in
      let fold x = 
        Queue.enqueue buf x;
        if is_enough () then reset (); 
        drop () in
      link xs s fold;
      s

  ...
  module Stream = struct
    ...
    let publish t event =
      ...
      |> List.iter ~f:(fun (id, notify) ->
             try notify id event with exn -> unsubscribe t id ; raise exn)
    ...
    let unfold_until ~init ~f =
      ...
      let do_publish () =
        match f !state with
        | None -> Promise.fulfill promise () ; `finished
        | Some (x, s) ->
            state := s ;
            publish x ;
            `continue in
    ...
    let map' s ~f =
      ...
      let rec push q =
        match Queue.dequeue q with None -> () | Some x -> publish x ; push q
      in
    ...
    let merge s1 s2 ~f =
      ...
      let drop () =
        let drop : type a. a Queue.t -> unit =
         fun q -> ignore (Queue.dequeue_exn q : a) in
        drop q1 ; drop q2 in
      let step src q x =
        Queue.enqueue q x ;
        let rec process () =
    ...
    let on_result xs id x p = unsubscribe xs id ; Promise.fulfill p x
    ...
    let find_map xs ~f =
      let future, promise = Future.create () in
      let f id x =
        match f x with None -> () | Some y -> on_result xs id y promise in
      watch xs f ; future
    ...
    let foldw ?(stride = 1) xs width ~init ~f =
      ...
      let drop n = match ndrop buf n with n' when n = n' -> 0 | n' -> n - n' in
      let publish d =
        let r = Queue.fold ~f ~init buf in
        (Some r, drop (estimate_drop d)) in
      let add x =
        Queue.enqueue buf x ;
        if is_enough () then publish `Pushed else (None, 0) in
      let try_push x = if is_enough () then publish `Not_pushed else add x in
      let fold drop x = match drop with 0 -> try_push x | n -> (None, n - 1) in

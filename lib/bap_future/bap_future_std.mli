...
module Std : sig
  ...
  (** A stream of elements.
      ...
      - [on_wait] that is cold when any consumer requested for a
      pause.
  ...
  *)
  module Stream : sig
    ...
    (** [has_subscribers s] is true if someone is watching for the list  *)
    ...
    *)
    ...
    (** [parse ss ~init ~f] parses stream [ss] and builds new stream
        ...
        [s',None], then no value is produced in the output state and
        state [s'] is passed to the next invocation of function
        [f]. If it returns [s',Some x], then value [x] is produced by
    ...
    (** [frame ~clk s] will gather elements of [s] into frames, where
        the start of the new frame is signaled by a stream [clk]. The
        function is very similar to [foldw] except, that the window is
        determined dynamically by a [clk] stream. This function is
    ...
    val frame : clk:unit t -> 'a t -> f:('b -> 'a -> 'b) -> 'b t

    (** [sample ~clk ss] is semantically the same as
        [frame ~clk ss >>| fst] *)
    val sample : clk:unit t -> 'a t -> 'a t

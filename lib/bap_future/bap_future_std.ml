...
module Stream = struct
  ...
  type 'a t = {
    subs : (Id.t -> 'a -> unit) Id.Table.t;
    mutable last_id : Id.t;
    mutable on_subs: (Id.t -> unit) list;
    mutable on_unsubs: (Id.t -> unit) list;
  ...
  let unfold' ~init ~f =
    let stream,publish = create () in
    let state = ref init in
    let push () =
      let q,s = f !state in
      state := s;
      Queue.iter q ~f:publish in
    stream, push
  ...
  let from f =
    let stream,publish = create () in
    let push () = Option.iter (f ()) ~f:publish in
    stream, push
  ...
  let of_list xs = unfold ~init:xs ~f:(function
      | [] -> None,[]
      | x::xs -> Some x,xs)
  ...
  let of_array xs =
    ...
    unfold ~init:0 ~f:(fun i ->
        if i < n then Some xs.(i),i+1 else None,i)

  let of_sequence xs = unfold ~init:(Some xs) ~f:(function
      | None -> None,None
      | Some seq -> match Sequence.next seq with
        | None -> None,None

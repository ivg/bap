...
type t =
  {root: workspace; args: string list; descr: string; spec: spec; loads: t list}
...
let spec_of_items =
  List.fold ~init:empty_spec ~f:(fun spec ->
    function
...
let make_subst root pars vars arg =
  if String.equal arg "prefix" then root.path
    ...
    | None -> (
        ...
        |> function Some x -> x | None -> raise (Bad_substitution arg) )
...
let check_vars env spec loads =
  ...
  |> function None -> Ok () | Some x -> Error (Unbound_param x)
...
let rec load_path env path =
  ...
  get_recipe root
  >>= fun spec ->
  List.map spec.uses ~f:(load env)
  |> Result.all
  >>= fun loads ->
  check_vars env.vars spec loads
  >>= fun () ->
  apply_subst root spec.pars env.vars spec.opts
  >>| fun args -> {root; descr; spec; loads; args}
...
let parse_vars ?(env = []) vars =
  List.map vars ~f:parse_var |> Result.all
  >>= fun vars ->
  ...
  | Some cmd ->
      let argv =
        ...
        | self :: arg :: rest when String.is_prefix ~prefix:arg cmd ->
            self :: cmd :: rest
        | self :: arg :: rest -> self :: cmd :: arg :: rest in

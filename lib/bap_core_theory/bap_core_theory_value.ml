...
module Sort : sig
  ...
  type +'a num
  type cls = Values
  type name = {package: string; name: string} [@@deriving bin_io, compare, sexp]

  type names =
    {unique: Hash_set.M(String).t; packages: Set.M(String).t Hashtbl.M(String).t}
  ...
  let cls =
    KB.Class.declare ~package:"core-theory" "value" () ~public:true
  ...
  module Name = struct
    ...
    let declare ?(package = "user") name =
      Hashtbl.update registry.packages package ~f:(function
        ...
        | Some names ->
            if Set.mem names name then
              failwithf
                "Type name `%s' is already defined for package `%s'. Please, \
                 pick a unique name or a different package."
                name package () ;
            Set.add names name) ;
      if Hash_set.mem registry.unique name then
        Hash_set.remove registry.unique name
  ...
  module Top = struct
    ...
    include Base.Comparable.Inherit
              (Exp)
              (struct
                type t = top

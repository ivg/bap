...
module Make (L : Minimal) = struct
  ...
  include struct
    ...
    let eq x y = x >>= fun x -> y >>= fun y -> ule !!x !!y && ule !!y !!x
    let neq x y = not (eq x y)
    let slt x y = x >>= fun x -> y >>= fun y -> sle !!x !!y && not (sle !!y !!x)
    let ult x y = x >>= fun x -> y >>= fun y -> ule !!x !!y && not (ule !!y !!x)
    let sgt x y = slt y x
    let ugt x y = ult y x
    let sge x y = sle y x
    let uge x y = ule y x
  ...
  let bind exp body =
    exp >>-> fun s exp -> Var.scoped s @@ fun v -> let_ v !!exp (body v)
  ...
  let loadw out dir mem key =
    ...
    let rec loop chunks loaded =
      if loaded < needed then
        ...
        bind (load !!mem key)
        @@ fun chunk -> loop (var chunk :: chunks) (loaded + chunk_size)
      else ite !!dir (concat out (List.rev chunks)) (concat out chunks) in
    loop [] 0
  ...
  let storew dir mem key data =
    ...
    let nth stored =
      let shift_amount =
        ite dir (small data_t stored) (small data_t (needed - stored)) in

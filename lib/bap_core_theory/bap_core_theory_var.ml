...
let const =
  Knowledge.Class.declare ~package "const-var" Const
    ~desc:"local immutable variables"

let mut =
  Knowledge.Class.declare ~package "mut-var" Mut
...
let version v = match ident v with Let _ -> 0 | Reg {ver} | Var {ver} -> ver

let fresh s =
  Knowledge.Object.create mut
  >>| fun v -> create s (Var {num= Knowledge.Object.id v; ver= 0})
...
let scoped : 'a sort -> ('a t -> 'b pure) -> 'b pure =
 fun s f ->
  Knowledge.Object.scoped const
  @@ fun v -> f @@ create s (Let {num= Knowledge.Object.id v})

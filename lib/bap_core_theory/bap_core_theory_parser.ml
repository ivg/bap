...
module Make (S : Core) = struct
    ...
    self.bitv
      ( module struct
        ...
        let append x y =
          ...
          x >>-> fun sx x -> y >>-> fun sy y -> append (join sx sy) !!x !!y
        ...
        let let_bit v x y =
          ...
          >>-> fun s x ->
          Var.scoped s
          @@ fun r -> let_ r !!x (run ((v, Var.ident r) :: ctxt) self y)
   ...
   fun ctxt self ->
    self.mem
      ( module struct
        ...
        let unknown ks vs = unk (Mem.define (bits ks) (bits vs)) end )
   ...
   fun ctxt self ->
    self.bool
      ( module struct
        ...
        let is_inf x = is_inf (expf x) end )

  and expf :
      type s b e r k n i g a.
    ...
    self.float
      ( module struct
        ...
        let let_float v x y =
          ...
          >>-> fun s x ->
          Var.scoped s
          @@ fun r -> let_ r !!x (run ((v, Var.ident r) :: ctxt) self y) end )
    ...
    | x :: xs ->
        self.stmt
          ( module struct
            ...
            let special _ =
              newlabel >>= fun lbl -> seq (blk lbl pass skip) (next xs)

            let cpuexn n =
              Label.for_ivec n
              >>= fun dst ->
              newlabel >>= fun lbl -> seq (blk lbl pass (goto dst)) (next xs)
            ...
            let call name =
              ...
              >>= fun () -> seq (blk lbl pass (goto dst)) (next xs)

            let goto addr =
              newlabel
              >>= fun lbl ->
              Label.for_addr addr
              >>= fun dst -> seq (blk lbl pass (goto dst)) (next xs)

            let move eff =
              newlabel >>= fun lbl -> seq (blk lbl eff skip) (next xs)
            ...
            let seq ys = seq (next ys) (next xs) end )
          x

  and set_bit :
      type e s r. context -> (e, r, s) parser -> string -> e -> data eff =
   fun ctxt self v x -> set (mkvar bool v) (expb ctxt self x)

  and set_reg :
      type e s r. context -> (e, r, s) parser -> string -> int -> e -> data eff
      =
  ...
  and set_rmode :
      type e s r. context -> (e, r, s) parser -> string -> r -> data eff =

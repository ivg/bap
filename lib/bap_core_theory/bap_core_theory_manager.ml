...
let ( ++ ) x y = x >>= fun x -> y >>| fun y -> Value.merge x y [@@inline]
let join1 p q x = x >>= fun x -> p !!x ++ q !!x [@@inline]
let join1s p q s x = x >>= fun x -> p s !!x ++ q s !!x [@@inline]

let join2 p q x y =
  x >>= fun x -> y >>= fun y -> p !!x !!y ++ q !!x !!y
  [@@inline]

let join2s p q s x y =
  x >>= fun x -> y >>= fun y -> p s !!x !!y ++ q s !!x !!y
  [@@inline]

let join3 p q x y z =
  x >>= fun x -> y >>= fun y -> z >>= fun z -> p !!x !!y !!z ++ q !!x !!y !!z
  [@@inline]

let join3s p q s x y z =
  x
  >>= fun x -> y >>= fun y -> z >>= fun z -> p s !!x !!y !!z ++ q s !!x !!y !!z
  [@@inline]
...
let join4 p q r x y z =
  ...
  x
  >>= fun x ->
  y >>= fun y -> z >>= fun z -> p !!r !!x !!y !!z ++ q !!r !!x !!y !!z
  [@@inline]
...
let slot =
  Knowledge.Class.property theory "instance" domain ~package:"core-theory"
    ~desc:"The theory structure"

let str_ctxt () ctxt = List.to_string (Set.to_list ctxt) ~f:ident
...
let theories () =
  ...
  |> Knowledge.List.fold ~init ~f:(fun theories (name, def) ->
         ...
         >>| fun name -> Map.add_exn theories name def)
...
let theory_for_id id =
  ...
  Knowledge.Symbol.intern sym theory ~package:"core-theory-internal"
...
let instance ?context ?requires () =
  theories () >>| Map.data >>| refine ?context ?requires
  >>= function
  ...
  | theories -> (
      List.fold theories
        ...
        ~f:(fun names t -> Set.union names t.id)
      |> theory_for_id
      >>= fun id ->
      is_instantiated id
      >>= function
      | true -> Knowledge.return id
      | false ->
          Knowledge.List.map theories ~f:(instantiate >=> Knowledge.collect slot)
...
let require name =
  ...
  | Some t -> t.structure
  | None -> Knowledge.collect slot name >>| fun t -> t.structure

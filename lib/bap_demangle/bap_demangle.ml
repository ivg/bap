...
let demangle_internal str =
  ...
  let extract_number pos_ref =
    ...
    lfindi str ~pos:s1_p0 ~f:(fun _ c -> not (Char.is_digit c)) >>= fun s1_p1 ->
  ...
  let extract_name pos_ref =
    ...
    let name = Substring.create ~pos:!pos_ref ~len str |> Substring.to_string in
    pos_ref := !pos_ref + len;
    Some name
  in
  let pos = ref 0 in
...
let demangle_internal name =
  if maybe_mangled name then Option.try_with (fun () -> demangle_internal name)
  else None
...
module Std = struct
  type demangler = { name : string; run : string -> string }
  ...
  let internal = { name = "internal"; run = run_internal }

...
let rename t =
  ...
  in
  let new_name x =
    Hashtbl.change nums x (function None -> Some 1 | Some x -> Some (x + 1)) ;
  ...
  let pop_defs blk =
    let pop v =
      Hashtbl.change vars (Var.base v) (function
        | Some (_ :: xs) -> Some xs
        | xs -> xs) in
    Term.enum phi_t blk |> Seq.iter ~f:(fun phi -> pop (Ir_phi.lhs phi)) ;
    Term.enum def_t blk |> Seq.iter ~f:(fun def -> pop (Ir_def.lhs def)) in
...
let insert_phi_node ins blk x =
  if has_phi_for_var blk x then blk
    ...
    |> Seq.to_list_rev |> Ir_phi.of_list x |> Term.append phi_t blk

...
class context =
  object (self)
    ...
    method taint_reg r ts =
      ...
      if Set.is_empty ts then {<tvs = Values.remove tvs key>}
...
let union_map m1 m2 ~f =
  Map.merge m1 m2 ~f:(fun ~key ->
    function `Both (v1, v2) -> Some (f v1 v2) | `Left v | `Right v -> Some v)
  ...
  class ['a] propagator =
    object (self)
      ...
      method! eval_store ~mem ~addr v e s =
        self#eval_exp v
        >>= fun rv ->
        super#eval_store ~mem ~addr v e s
        >>= fun rr ->
        self#eval_exp addr >>| Bil.Result.value
        >>= function
        | Bil.Bot | Bil.Mem _ -> SM.return rr
        | Bil.Imm a ->
            SM.get ()
            >>= fun ctxt ->
            SM.put (ctxt#taint_ptr a s (ctxt#reg_taints rv))
            >>= fun () -> SM.return rr

      method! eval_load ~mem ~addr e s =
        super#eval_load ~mem ~addr e s
        >>= fun r ->
        super#eval_exp addr >>| Bil.Result.value
        >>= function
        | Bil.Bot | Bil.Mem _ -> SM.return r
        | Bil.Imm a ->
            SM.get ()
            >>= fun ctxt ->
            SM.put (ctxt#taint_reg r (ctxt#ptr_taints a))
            >>= fun () -> SM.return r
      ...
      method private eval2 e1 e2 r3 =
        ...
        >>= fun r1 ->
        self#eval_exp e2
        >>= fun r2 -> self#propagate [r1; r2] r3 >>= fun () -> SM.return r3

      method private eval e rr =
        self#eval_exp e
        >>= fun re -> self#propagate [re] rr >>= fun () -> SM.return rr

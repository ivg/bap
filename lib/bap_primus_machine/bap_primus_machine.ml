...
module Inspector = struct type 'm t = {id: int; inspect: Info.t -> 'm} end
...
module Primus = struct
  ...
  module PE = struct type t = (unit, exn) Monad.Result.result end
  ...
  module Observation : sig
    ...
    let rec loop kill k = function
      ...
      | {Observer.id; observe} :: fs -> (
          k (observe return)
          >>= function
          | Stop -> loop (Set.add kill id) k fs | _ -> loop kill k fs )
    ...
    let get_info {Observation.name; inspect} k =
      ...
      | None -> return noinfo
      | Some inspect -> (
          k (inspect (fun data -> Info data))
          >>| function Info data -> data | _ -> noinfo )
    ...
    let call_inspectors obs k =
      ...
      | fs ->
          ...
          List.fold ~init:[] fs ~f:(fun fs ({inspect} as f) ->
              inspect info
              >>= function Stop -> return fs | _ -> return (f :: fs))
          >>= set_inspectors obs
  ...
  let execute_sentenced =
    with_global_context (fun () ->
        lifts @@ SM.get ()
        >>= fun s ->
        lifts @@ SM.List.iter s.deathrow ~f:SM.kill
  ...
  let die next =
    ...
    switch_state next
    >>= fun () ->
    lifts (SM.get ()) >>= fun s -> lifts (SM.kill pid) >>= fun () -> s.curr ()

  let raise exn =
    Observation.provide exn_raised ~f:(fun observe -> observe exn)
    >>= fun () -> fail exn

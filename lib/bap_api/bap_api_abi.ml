...
type t = {arch: arch; name: string} [@@deriving bin_io, compare, sexp]
...
module Abi = Regular.Make (struct
  ...
  let pp ppf {arch; name} = Format.fprintf ppf "%a-%s" Arch.pp arch name
end)
...
let resolve arch attr name =
  match Hashtbl.find resolvers arch with
  | Some f -> {arch; name= f attr name}
  | None -> {arch; name= "unknown"}

let apply abi pos =
  match Hashtbl.find abis abi with
  | Some abi -> abi pos
  | None ->
...
let known_abi arch =
  Hashtbl.keys abis
  |> List.filter_map ~f:(fun abi -> Option.some_if (abi.arch = arch) abi.name)

...
module Blocks = struct
  ...
  let mark_open_tag tag : string =
    filter_attrs tag |> Option.value_map ~default:"" ~f:(sprintf "begin(%s) ")

  let mark_close_tag tag : string =
    filter_attrs tag |> Option.value_map ~default:"" ~f:(sprintf "end(%s)")

  let print_open_tag fmt tag : unit =
    if Option.is_some (filter_attrs tag) then (
      pp_open_vbox fmt 1 ; pp_print_cut fmt () )

  let print_close_tag fmt tag =
    if Option.is_some (filter_attrs tag) then (
      pp_close_box fmt () ; pp_print_cut fmt () )
...
let install fmt mode =
  List.Assoc.find ~equal:String.equal !modes mode
  |> function Some install -> install fmt | None -> raise (Unknown_mode mode)
...
let with_mode fmt mode =
  ...
  install fmt mode ; Exn.protect ~finally
  [@@warning "-D"]

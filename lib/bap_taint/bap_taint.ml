...
module Object = struct
  ...
  module Make (Machine : Primus.Machine.S) = struct
    ...
    let create kind =
      Machine.Local.get kinds
      >>= fun s ->
      next_key s
      >>= fun key ->
      Machine.Local.put kinds {objects= Map.set s.objects ~key ~data:kind}
      >>| fun () -> key
 ...
 *)
type tainter = {direct: objects Vid.Map.t; indirect: objects Primus.Value.Map.t}
[@@deriving fields]
...
let empty_tainter = {direct= Vid.Map.empty; indirect= Primus.Value.Map.empty}
...
let gc =
  Primus.Machine.State.declare ~name:"primus-taint-gc"
    ~uuid:"2357826e-d5b7-40a3-8f90-0cfd7b48eadc" (fun _ -> {old= empty_tainter})

let vid = Primus.Value.id

let indirect =
  Rel {field= Fields_of_tainter.indirect; key= ident; kind= Indirect}
...
module Propagation = struct
  type policies =
    {servers: Primus.value Kind.Map.t; default: Primus.value option}
  ...
  module Policy = struct
    ...
    module Make (Machine : Primus.Machine.S) = struct
      ...
      let propagate p ms md srcs dst =
        Machine.Local.get tainter
        >>= fun s ->
        collect_taints p ms srcs s
        >>= fun taints -> Tracker.attach dst md taints
  ...
  module Conservative (Machine : Primus.Machine.S) = struct
    ...
    let collect_direct =
      ...
      Machine.Seq.fold vars ~init:Vid.Set.empty ~f:(fun live var ->
          Env.get var >>| fun v -> Set.add live (Value.id v))
      >>= fun live ->
    ...
    let main _ =
      ...
      Set.to_sequence dead
      |> Machine.Seq.iter ~f:(finish ~live:false)
      >>= fun () -> Machine.Local.put gc {old= cur}

    let finalize _ =
      Machine.Local.get tainter
      >>= fun cur ->
      Set.to_sequence (objects cur) |> Machine.Seq.iter ~f:(finish ~live:true)

    let init () =
      Machine.sequence
        Primus.Interpreter.
          [leave_blk >>> main; Primus.Machine.kill >>> finalize]
  end
end

...
type entry = {fields: string String.Map.t} [@@deriving compare]
type row = {row: entry array}
type 'a seq = 'a Sequence.t

module Type = struct
  type typ = Int | Str | Bool | Float [@@deriving compare, enumerate, sexp]
  type 'a t = {parse: string -> 'a option; pack: 'a -> string; typ: typ}
  type 'a field = {t: 'a t; name: string}
  type header = {fname: string; ftype: typ} [@@deriving compare, sexp]
  ...
  let str = {parse= atom string_of_sexp; pack= ident; typ= Str}
  let float = {parse= atom float_of_sexp; pack= pack sexp_of_float; typ= Float}
...
module Doc = struct
  ...
  type t =
    {scheme: Type.signature String.Map.t; entries: entry list String.Map.t}
  [@@deriving compare]
  ...
  let update_entries name packed doc =
    typecheck_entry name packed doc
    >>| fun () ->
    ...
    let entries =
      Map.update doc.entries name ~f:(function
        | None -> [packed]
        | Some data ->
            if List.mem ~equal data packed then data else packed :: data) in
    {doc with entries}
  ...
  let parse_tname name =
    List.Assoc.find ~equal:String.equal Type.names name
    |> function
    | Some typ -> Ok typ
    | None ->
  ...
  let parse_fdecl = function
    | i, Sexp.Atom tname ->
        parse_tname tname >>| fun ftype -> {Type.fname= sprintf "%d" i; ftype}
    | _, Sexp.List [Sexp.Atom fname; Sexp.Atom tname] ->
        parse_fname fname
        >>= fun fname -> parse_tname tname >>| fun ftype -> {Type.ftype; fname}
    | _ ->
  ...
  let named hdr name x =
    ...
    |> function None -> errorf "unknown field %s" name | Some _ -> Ok (name, x)
  ...
  let parse_entry doc = function
    | Sexp.List (Sexp.Atom "declare" :: Sexp.Atom name :: flds) ->
        Error.List.map ~f:parse_fdecl (tabulate flds)
        >>= fun sign -> update_scheme name sign doc
...
module Exp = struct
  ...
  let parsers =
    [ parser Type.int (fun x -> Int x); parser Type.str (fun x -> Str x)
    ...
    | Bop
        ...
        Var
          (fun row ->
            bool
              String.(
                Map.find_exn row.(x).fields n1 = Map.find_exn row.(y).fields n2))
...
module Query = struct
  ...
  module String = struct
    let get field p = Exp.var {attr= Exp.Pos p; field= header_of_field field}
  end
...
module Make (B : Monad.S) = struct
  ...
  let foreach {Query.where; join; tables= {Query.read; names}} ~f : 'b seq t =
    M.get ()
    >>| fun doc ->

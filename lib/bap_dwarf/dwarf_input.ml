...
type 'a inj =
  {of_int64: int64 -> 'a Or_error.t; of_int32: int32 -> 'a Or_error.t}

let int = {of_int64= ok_some Int64.to_int; of_int32= ok_some Int32.to_int}
let int32 = {of_int64= ok_some Int64.to_int32; of_int32= ok_any ident}
let int64 = {of_int64= ok_any ident; of_int32= ok_any Int64.of_int32}
...
let map read ~f str ~pos_ref = read str ~pos_ref >>| fun x -> f x

let pair read1 read2 str ~pos_ref =
  read1 str ~pos_ref
  >>= fun w1 -> read2 str ~pos_ref >>= fun w2 -> return (w1, w2)

let unit_size endian str ~pos_ref =
  read_int32 endian W32 str ~pos_ref
  >>= function
...
let block lenspec endian src ~pos_ref =
  const lenspec endian src ~pos_ref
  >>= fun len ->
  Result.of_option

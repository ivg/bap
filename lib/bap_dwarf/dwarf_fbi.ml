...
module Fn = struct
  module T = struct
    type t = {pc_hi: addr sexp_option; pc_lo: addr}
    ...
    let create ?pc_hi ~pc_lo () =
      let pc_hi =
        ...
        | Some (Value.Rel off) ->
            int_of_int64 off >>| fun off -> Some Addr.(pc_lo ++ off) in
...
type spec = {endian: endian; elf_size: Word_size.t; dwarf_size: Word_size.t}
...
let fn_of_values vs : (string * fn) Or_error.t =
  ...
  | Id name :: vs ->
      errorf "'%s' is degenerate" name
      |> fun err -> tag_arg err "attrs" vs (sexp_of_list sexp_of_value)
  | vs ->
      errorf "got anonymous function"
      |> fun err -> tag_arg err "attrs" vs (sexp_of_list sexp_of_value)

let is_done str ~pos_ref =
  Input.(pair char char) str ~pos_ref
  >>| function
  | 0, 0 -> true
  | _ ->
      pos_ref := !pos_ref - 2 ;
...
let read_scheme s str_sec action str ~pos_ref =
  ...
  let rec loop acc =
    ...
    | false ->
        Input.(pair attr form) str ~pos_ref
        >>= fun (attr, form) -> loop (emit attr form :: acc) in
  loop []

let rec create_table s str_sec str ~pos_ref =
  let abbrs : table = Table.create () in
  let rec fill () =
    Input.code str ~pos_ref
    >>= function
    | 0 -> return ()
    | code -> (
        Input.tag str ~pos_ref
        >>= fun tag ->
        Input.skip str ~pos_ref ~bytes:1
        >>= fun () ->
        (* children flag *)
        let action =
          let open Tag in
        ...
        >>= fun scheme ->
        match Table.add abbrs ~key:code ~data:scheme with
        | `Ok -> fill ()
...
let create data : t Or_error.t =
  ...
  Dwarf_data.section data Section.info
  >>= fun info_sec ->
  Dwarf_data.section data Section.abbrev
  >>= fun abbr_sec ->
  Dwarf_data.section data Section.str
  >>= fun str_sec ->
  ...
  let read_header () =
    from_info (Input.unit_size endian)
    >>= fun (dwarf_size, unit_size) ->
    let cu_end = !info_pos + unit_size in
    from_info Input.version
    >>= fun _version ->
    from_info (Input.offset endian dwarf_size)
    >>= fun offset ->
    from_info Input.address_size
    >>= fun elf_size ->
    abbr_pos := !abbr_pos + offset ;
    let s = {endian; dwarf_size; elf_size} in
    create_table s str_sec abbr ~pos_ref:abbr_pos
    >>= fun abbrs -> return (abbrs, cu_end) in
  let rec read_unit () =
    read_header ()
    >>| fun init ->

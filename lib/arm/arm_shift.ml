...
let i_shift ~src shift_type t =
  ...
  let r =
    Word.Int_err.(!$shift_type land !$mask)
    >>| shift_of_word
    >>= fun shift_t ->
    (* other bits are immediate *)
    Word.Int_err.((!$shift_type land lnot !$mask) lsr !$three)
    >>= fun shift_amt -> return (shift_t, shift_amt) in
  match r with
...
let mem_shift ~src shift typ =
  ...
  let to_neg w exp =
    ...
    | _ -> exp in
  let r =
    shift_typ shift >>= fun t -> shift_amt shift >>= fun amt -> return (t, amt)
  in
  match r with
  | Error err -> fail [%here] "%s" Error.(to_string_hum err)
  | Ok (t, amount) ->

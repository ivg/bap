...
let lift_r ~(dst1 : Var.t) ?(dst2 : Var.t option) ~(base : Var.t)
  ...
  | Ld ->
      ...
      let extend =
        match size with B | H -> [Bil.move dst1 rhs] | W | D -> [] in
      let loads =
        ...
        else [assn temp (load mem address)] in
      List.concat
        [ pre_write_back; loads; extend; (* sign/zero extend if necessary *)
          write_back ]
  | St ->
      ...
      let stores =
        ...
        | D ->
            ...
            ; Bil.move m (store v Bil.(address + four) Bil.(var (uw dst2))) ]
        | B | H | W -> [Bil.move m (store v address Bil.(var temp))] in
      List.concat
...
let lift_m dest_list base mode update operation =
  ...
  let writeback =
    ...
    | Update ->
        let ( +- ) =
          match mode with IB | IA -> Bil.( + ) | DB | DA -> Bil.( - ) in
        [Bil.move base Bil.(var base +- int dest_len)] in
  ...
  move_jump_to_end
    (List.concat
       [ Bil.[o_base := var base]; List.mapi ~f:create_access dest_list
       ; writeback ])

...
let lift_move mem ops (insn : move_insn) : stmt list =
  let open Mov in
  match (insn, ops) with
  | `MOVi, [|dest; src; cond; _; wflag|] | `MOVr, [|dest; src; cond; _; wflag|]
    ->
  ...
  | `MVNi, [|dest; src; cond; _; wflag|] | `MVNr, [|dest; src; cond; _; wflag|]
    ->
      lift ~dest src `MVN mem cond ~wflag
  | `MVNsr, [|dest; src; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src `MVN ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  ...
  | `ANDrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `AND ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `ANDrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `AND ~simm:shift_imm mem cond ~wflag
  ...
  | `BICrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `BIC ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `BICrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `BIC ~simm:shift_imm mem cond ~wflag
  ...
  | `EORrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `EOR ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `EORrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `EOR ~simm:shift_imm mem cond ~wflag
  ...
  | `ORRrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `ORR ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `ORRrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `ORR ~simm:shift_imm mem cond ~wflag
  ...
  | `TEQrsr, [|src1; src2; shift_reg; shift_imm; cond; _|] ->
      lift src1 ~src2 `EOR ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `TEQrsi, [|_dest; src1; src2; shift_imm; cond; _|] ->
      lift src1 ~src2 `EOR ~simm:shift_imm mem cond ~wflag:(`Reg `CPSR)
  ...
  | `TSTrsr, [|src1; src2; shift_reg; shift_imm; cond; _|] ->
      lift src1 ~src2 `AND ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `TSTrsi, [|src1; src2; shift_imm; cond; _|] ->
      lift src1 ~src2 `AND ~simm:shift_imm mem cond ~wflag:(`Reg `CPSR)
  ...
  | `ADDrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `ADD ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `ADDrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `ADD ~simm:shift_imm mem cond ~wflag
  ...
  | `SUBrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `SUB ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `SUBrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `SUB ~simm:shift_imm mem cond ~wflag
  ...
  | `ADCrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `ADC ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `ADCrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `ADC ~simm:shift_imm mem cond ~wflag
  ...
  | `SBCrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `SBC ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `SBCrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `SBC ~simm:shift_imm mem cond ~wflag
  ...
  | `RSBrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `RSB ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `RSBrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `RSB ~simm:shift_imm mem cond ~wflag
  ...
  | `RSCrsr, [|dest; src1; src2; shift_reg; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `RSC ~sreg:shift_reg ~simm:shift_imm mem cond ~wflag
  | `RSCrsi, [|dest; src1; src2; shift_imm; cond; _; wflag|] ->
      lift ~dest src1 ~src2 `RSC ~simm:shift_imm mem cond ~wflag
  ...
  | `CMPrsr, [|src1; src2; shift_reg; shift_imm; cond; _|] ->
      lift src1 ~src2 `SUB ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `CMPrsi, [|src1; src2; shift_imm; cond; _|] ->
      lift src1 ~src2 `SUB ~simm:shift_imm mem cond ~wflag:(`Reg `CPSR)
  ...
  | `CMNzrsr, [|src1; src2; shift_reg; shift_imm; cond; _|] ->
      lift src1 ~src2 `ADD ~sreg:shift_reg ~simm:shift_imm mem cond
        ~wflag:(`Reg `CPSR)
  | `CMNzrsi, [|src1; src2; shift_imm; cond; _|] ->
      lift src1 ~src2 `ADD ~simm:shift_imm mem cond ~wflag:(`Reg `CPSR)
...
let lift_bits mem ops (insn : bits_insn) =
  ...
  | `SXTB, [|dest; src; rot; cond; _|] -> extend ~dest ~src Signed `B ~rot cond
  | `SXTH, [|dest; src; rot; cond; _|] -> extend ~dest ~src Signed `H ~rot cond
  ...
  | `RBIT, [|dest; src; cond; _|] ->
      ...
      exec
        Bil.
          [ v := exp_of_op src lsr int32 1; r := exp_of_op src; s := int32 31
          ; while_
              (var v <> int32 0)
              [ r := var r lsl int32 1; r := var r lor (var v land int32 1)
              ; v := var v lsr int32 1; s := var s - int32 1 ]
  ...
  | `PKHTB, [|`Reg dest; src1; src2; shift; cond; _|] ->
      ...
      exec
        [ assn (Env.of_reg dest)
            Bil.(extract 31 16 (exp_of_op src1) ^ extract 15 0 shifted) ]
        cond
  (* reverses *)
  | `REV, [|`Reg dest; src; cond; _|] ->
      ...
      let rev =
        let open Bil in
        (s lsl i24) lor (s lsr i24)
        lor ((s land umask) lsr i8)
        lor ((s land lmask) lsl i8) in
  ...
  | `CLZ, [|`Reg dest; src; cond; _|] ->
      ...
      Bil.(
        exec
          [ shift := exp_of_op src; accum := int32 32
          ; while_
              (var shift <> int32 0)
              [shift := var shift lsr int32 1; accum := var accum - int32 1]
...
let lift_mult ops insn =
  ...
  | `UMLAL, [|lodest; hidest; src1; src2; _loadd; _hiadd; cond; _rflag; wflag|]
    ->
      lift_mull ~lodest ~hidest ~src1 ~src2 Unsigned ~addend:true ~wflag cond
  | `SMLAL, [|lodest; hidest; src1; src2; _loadd; _hiadd; cond; _rflag; wflag|]
    ->
...
let lift_mem ops insn =
  ...
  | `STRH, [|dest1; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset Offset Unsigned H St in
      exec insns cond
  | `LDRH, [|dest1; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset Offset Unsigned H Ld in
      exec insns cond
  | `STRH_PRE, [|_unknown; dest1; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PreIndex Unsigned H St in
      exec insns cond
  | `LDRH_PRE, [|dest1; _unknown; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PreIndex Unsigned H Ld in
      exec insns cond
  | `STRH_POST, [|_unknown; dest1; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned H St in
      exec insns cond
  ...
  | `LDRH_POST, [|dest1; _unknown; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned H Ld in
      exec insns cond
  (* POS_SIGN_BIT *)
  ...
  | `LDRSH, [|dest1; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns = Mem_shift.lift_r_exp ~dest1 ~base ~offset Offset Signed H Ld in
      exec insns cond
  | `LDRSH_PRE, [|dest1; _unknown; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PreIndex Signed H Ld in
      exec insns cond
  | `LDRSH_POST, [|dest1; _unknown; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Signed H Ld in
      exec insns cond
  (* POS_SIGN_BIT *)
  ...
  | `LDRSB, [|dest1; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns = Mem_shift.lift_r_exp ~dest1 ~base ~offset Offset Signed B Ld in
      exec insns cond
  | `LDRSB_PRE, [|dest1; _unknown; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PreIndex Signed B Ld in
      exec insns cond
  | `LDRSB_POST, [|dest1; _unknown; base; reg_off; `Imm imm_off; cond; _|] ->
      let offset = Mem_shift.mem_offset_reg_or_imm_neg reg_off imm_off in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Signed B Ld in
      exec insns cond
  (* POS_SIGN_BIT *)
  ...
  | `STRi12, [|dest1; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset Offset Unsigned W St in
      exec insns cond
  | `LDRi12, [|dest1; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset Offset Unsigned W Ld in
      exec insns cond
  | `STRBi12, [|dest1; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset Offset Unsigned B St in
      exec insns cond
  | `LDRBi12, [|dest1; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset Offset Unsigned B Ld in
      exec insns cond
  ...
  | `STR_POST_IMM, [|_unknown; dest1; base; _invalid; `Imm offset; cond; _|] ->
      let offset =
        Mem_shift.repair_imm offset ~sign_mask:0x1000 ~imm_mask:0xfff `NEG in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned W St in
      exec insns cond
  | `LDR_POST_IMM, [|dest1; _unknown; base; _invalid; `Imm offset; cond; _|] ->
      let offset =
        Mem_shift.repair_imm offset ~sign_mask:0x1000 ~imm_mask:0xfff `NEG in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned W Ld in
      exec insns cond
   ...
   |`STRBT_POST_IMM, [|_unknown; dest1; base; _invalid; `Imm offset; cond; _|]
    ->
      let offset =
        Mem_shift.repair_imm offset ~sign_mask:0x1000 ~imm_mask:0xfff `NEG in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned B St in
      exec insns cond
   ...
   |`LDRBT_POST_IMM, [|dest1; _unknown; base; _invalid; `Imm offset; cond; _|]
    ->
      let offset =
        Mem_shift.repair_imm offset ~sign_mask:0x1000 ~imm_mask:0xfff `NEG in
      let insns =
        Mem_shift.lift_r_exp ~dest1 ~base ~offset PostIndex Unsigned B Ld in
      exec insns cond
  ...
  | `STR_PRE_IMM, [|_unknown; dest1; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset PreIndex Unsigned W St in
      exec insns cond
  | `LDR_PRE_IMM, [|dest1; _unknown; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset PreIndex Unsigned W Ld in
      exec insns cond
  | `STRB_PRE_IMM, [|_unknown; dest1; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset PreIndex Unsigned B St in
      exec insns cond
  | `LDRB_PRE_IMM, [|dest1; _unknown; base; offset; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base ~offset PreIndex Unsigned B Ld in
      exec insns cond
  ...
  | `LDREX, [|dest1; base; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base
          ~offset:(`Imm (word 0))
          Offset Unsigned W Ld in
      exec insns cond
  | `LDREXB, [|dest1; base; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base
          ~offset:(`Imm (word 0))
          Offset Unsigned B Ld in
      exec insns cond
  | `LDREXH, [|dest1; base; cond; _|] ->
      let insns =
        Mem_shift.lift_r_op ~dest1 ~base
          ~offset:(`Imm (word 0))
          Offset Unsigned H Ld in
      exec insns cond
...
let lift_special ops insn =
  ...
  | `MRS, [|`Reg dest; cond; _|] ->
      ...
      let vd = Bil.var d in
      exec
        [ Bil.move d (int32 0); Bil.move d (get_bits Env.nf vd 31)
        ; Bil.move d (get_bits Env.zf vd 30); Bil.move d (get_bits Env.cf vd 29)
        ; Bil.move d (get_bits Env.vf vd 28); Bil.move d (get_bits Env.qf vd 27)
  ...
  | `MSR, [|`Imm imm; `Reg src; cond; _|] ->
      ...
      let s2 =
        if Word.(Int_exn.(imm land word 0x4) = word 0x4) then
          [ Env.ge.(3) := 19; Env.ge.(2) := 18; Env.ge.(1) := 17
          ; Env.ge.(0) := 16 ]
        else [] in
      exec (s1 @ s2) cond
...
module CPU = struct
  ...
  let regs =
    Var.Set.of_list
      [ r0; r1; r2; r3; r4; r5; r6; r7; r8; r9; r10; r11; r12; pc; sp; lr; spsr
      ; cpsr; itstate ]

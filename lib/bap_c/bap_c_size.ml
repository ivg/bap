...
class base (m : model) =
  object (self)
    ...
    method basic : basic -> size =
      function
      | #integer as t -> self#integer t | #floating as t -> self#floating t

    method scalar : scalar -> size =
      function
      | `Basic {Spec.t} -> self#basic t | `Pointer _ -> (self#pointer :> size)
    ...
    method union : compound unqualified -> Int.t option =
      fun {Spec.t= {Compound.fields}} ->
        List.map fields ~f:(fun (_, t) -> self#bits t)
        |> Option.all
        |> function
        | None -> None
        | Some ss -> (
            List.max_elt ~compare:Int.compare ss
      ...
      fun {Spec.t= {Compound.fields}} ->
        List.fold fields ~init:(Some 0) ~f:(fun sz (_, field) ->
            ...
            | Some sz -> (
              ...
              | Some sz' ->
                  let pad =
                    match self#padding field sz with
                    | None -> 0

...
let create_arg i addr_size intent name t (data, exp) sub =
  let typ =
    ...
    | _ -> Type.Imm (Size.in_bits addr_size)
  in
    ...
    object (self)
      ...
      method private apply_args sub attrs t =
        ...
        | Some { return; hidden; params } ->
            ...
            let args =
              List.map2_exn params t.Bap_c_type.Proto.args
                ~f:(fun (i, a) (n, t) ->
            ...
            let ret =
              ...
              | Some ret ->
                  ...
                  [ create_arg 0 addr_size Out "result" t ret sub ]
            in
            let hid =
              List.mapi hidden ~f:(fun i (t, a) ->
                  let n = "hidden" ^ if i = 0 then "" else Int.to_string i in
                  create_arg 0 addr_size Both n t a sub)
            in

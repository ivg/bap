  ...
  class ['e] base =
    object (self)
      method private visit
          : 'a.    ('a -> (unit, 'e) m) -> ('a -> ('a, 'e) m)
            -> ('a -> (unit, 'e) m) -> 'a -> ('a, 'e) m =
        fun enter visit leave t ->
          enter t
          >>= fun () -> visit t >>= fun t -> leave t >>= fun () -> M.return t
      ...
      method private map_spec
          : 'a 'b.    ('a -> ('a, 'e) m) -> ('b -> ('b, 'e) m) -> ('a, 'b) spec
            -> (('a, 'b) spec, 'e) m =
        fun map_qualifier map_t s ->
          let open Spec in
          map_t s.t
          >>= fun t ->
          map_qualifier s.qualifier
          >>= fun qualifier ->
          self#map_attrs s.attrs >>= fun attrs -> M.return {t; qualifier; attrs}
      ...
      method private do_map_function {Proto.args; return; variadic} =
        self#map_type return
        >>= fun return ->
        self#map_fields args
        >>= fun args -> M.return {Proto.args; return; variadic}

      method private map_fields fields =
        M.all
        @@ List.map fields ~f:(fun (n, t) ->
               self#map_name n
               >>= fun n -> self#map_type t >>= fun t -> M.return (n, t))

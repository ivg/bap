...
let empty = {map= Map.empty; bound= Bound.empty}
let singleton k v = {map= Map.singleton k v; bound= Bound.(update empty k)}
...
let change tab mem ~f =
  ...
  | (`update _ | `rebind _ | `remove) as cmd ->
      let map = Seq.fold ins ~init:tab.map ~f:(fun m (k, _) -> Map.remove m k) in
      let map, bound =
        match cmd with
        ...
        | `rebind (mem, data) ->
            (Map.set map ~key:mem ~data, Bound.update tab.bound mem) in
      {map; bound}
...
let foldi ?start ?until (tab : 'a t) ~(init : 'b) ~f : 'b =
  let f_labeled ~key ~data s = f key data s in
    ...
    let map, bound =
      foldi ?start ?until tab ~init:(Map.empty, Bound.empty)
        ~f:(fun addr x (map, bound) ->
          (add map ~key:addr ~data:(f addr x), Bound.update bound addr)) in
    {map; bound}
...
let filter_mapi ?start ?until tab ~f =
  let add map ~key ~data =
    match data with None -> map | Some data -> Map.set map ~key ~data in
...
let link_maybe_one t (t1 : 'a t) (t2 : 'b t) : 'a -> 'b option =
  let mapping =
    Map.merge t1.map t2.map ~f:(fun ~key ->
      function `Both (v1, v2) -> Some (v1, v2) | `Left _ | `Right _ -> None)
    |> make_mapping t in
...
let link_one t t1 t2 =
  ...
  fun x ->
    ...
    | None ->
        let err =
          Error.create "link_exn: unbound value " x (sexp_of_hashable t) in
        Error.raise err

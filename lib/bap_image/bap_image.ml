...
type mapped = {off: int; len: int; endian: endian; r: bool; w: bool; x: bool}
[@@deriving bin_io, compare, sexp]
...
let create_segment_of_symbol_table syms secs =
  let tab =
    Symbol.Table.create () ~growth_allowed:false ~size:(Table.length syms) in
  Table.iteri syms ~f:(fun mem sym ->
      match Table.find_addr secs (Memory.min_addr mem) with
      | None -> ()
      | Some (_, sec) -> Symbol.Table.add_exn tab ~key:sym ~data:sec) ;
...
module Derive = struct
  ...
  let entry =
    addr_width
    >>= fun width -> Fact.require entry_point >>| Word.of_int64 ~width
  ...
  let segments : segment seq Fact.t =
    ...
    Fact.foreach
      ...
      ~f:(fun {addr; size; info= r, w, x} {size= len; info= off} {info= name} ->
        location ~addr ~size
        >>= fun locn ->
        int_of_int64 off
        >>= fun off ->
        int_of_int64 len
        >>| fun len -> {name; locn; info= {off; len; endian; r; w; x}})
    >>= Fact.Seq.all
  ...
  let symbols =
    Fact.foreach
      ...
      ~f:(fun start ->
        ...
        | Some name -> (
            symbol_locations start
            >>= fun locations ->
            match Seq.to_list locations with
            | [] -> Fact.return None
            | locn :: locns ->
                Fact.return
                  (Some {name; locn; info= {kinds= [`Code]; extra_locns= locns}})
            ))
  ...
  let code =
    ...
    >>= fun endian ->
    Fact.foreach
      Ogre.Query.(select (from code_region $ named_region) ~join:[[field addr]])
...
module Legacy = struct
  ...
  let vsize secs {Segment.name; location= {Location.addr; len}} =
    ...
    |> function None -> len | Some len -> len
  ...
  let provide_segment sections s =
    ...
    Fact.provide segment addr vsize r w x
    >>= fun () ->
    Fact.provide mapped addr fsize off
    >>= fun () -> Fact.provide named_region addr vsize (Segment.name s)

  let provide_section s =
    let addr, size = location_repr @@ Section.location s in
    Fact.provide section addr size
    >>= fun () -> Fact.provide named_region addr size (Section.name s)

  let provide_function s =
    let loc, locs = Symbol.locations s in
    let root, size = location_repr loc in
    Fact.provide named_symbol root (Symbol.name s)
    >>= fun () ->
    Fact.provide code_start root
    >>= fun () ->
    Fact.provide symbol_chunk root size root
    >>= fun () ->
  ...
  let provide_image {Img.arch= a; entry; segments= s, ss; sections; symbols} =
    Fact.provide arch (Arch.to_string a)
    >>= fun () ->
    Fact.provide entry_point (addr entry)
    >>= fun () ->
    Fact.List.iter (s :: ss) ~f:(provide_segment sections)
    >>= fun () ->
    Fact.List.iter sections ~f:provide_section
    >>= fun () -> Fact.List.iter symbols ~f:provide_symbol
end

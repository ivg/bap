...
type getter =
  {safe: pos_ref:addr ref -> word or_error; fast: pos_ref:int ref -> word}

type t =
  {endian: endian; data: Bigstring.Stable.V1.t; addr: addr; off: int; size: int}
[@@deriving bin_io]

module Repr = struct
  type t = {endian: endian; offset: int; base: addr; size: int}
  [@@deriving sexp_of]
end
...
let sexp_of_t mem =
  Sexp.List
    [ Atom (Addr.string_of_value mem.addr); sexp_of_int mem.size
...
let getter {endian; addr; off; size; data} =
  ...
  let make inj bytes read =
    ...
    let safe ~pos_ref : word or_error =
      Addr.Int_err.(!$(!pos_ref) - !$addr)
      >>= fun addr ->
      Addr.(to_int (addr ++ off))
      >>= fun pos ->
  ...
  let concat_int64X2 =
    make Word.(fun (a, b) -> concat (of_int64 a) (of_int64 b)) in
  let concat_int64X4 =
    make
      Word.(
        fun (a, b, c, d) ->
  ...
  if [%compare.equal: endian] endian BigEndian then
    ...
    let get_int64_beX4 t ~pos =
      ...
      , unsafe_get_int64_t_be t ~pos:(pos + 24) ) in
    function
    ...
    let get_int64_leX4rev t ~pos =
      ...
      , unsafe_get_int64_t_le t ~pos ) in
    function
    | `s8 -> int 1 unsafe_get_int8
    | `r8 -> int 1 unsafe_get_uint8
  ...
  let v =
    Validate.(
      name_list "Bap_memory.create"
        ...
        ; name "pos+size"
          @@ Int.validate_ubound (pos + size) ~max:(Incl data_len) ]) in
...
let sub copy ?(word_size = `r8) ?from ?words t : t or_error =
  ...
  let amax =
    Option.map words ~f:(fun w ->
        Addr.(amin ++ Int.((w * Size.in_bytes word_size) - 1)))
    |> Option.value ~default:(max_addr t) in
  Validate.(
    result
    @@ name "view must not be empty"
    @@ Addr.validate_lbound amax ~min:(Incl amin))
  >>= fun () ->
  Addr.Int_err.(!$amax - !$amin >>= Addr.to_int)
  >>= fun diff ->
  let size = diff + 1 in
  Addr.Int_err.(!$amin - !$(t.addr) >>= Addr.to_int)
  >>= fun off ->
  let off = t.off + off in
  let check_preconditions =
    Validate.(
      name_list "preconditions"
        [ name "offset in bounds"
          @@ Int.validate_bound off ~min:(Incl t.off)
               ~max:(Excl (t.off + t.size))
        ; name_list "size fits"
            [ Int.validate_bound size ~min:(Incl 1) ~max:(Incl t.size)
            ; Int.validate_ubound (off + size) ~max:(Incl (t.off + t.size)) ] ])
  in
  Validate.result check_preconditions
  >>= fun () ->
...
let range mem a1 a2 =
  Addr.Int_err.(!$a2 - !$a1)
  >>= Addr.to_int
  >>= fun bytes -> view ~from:a1 ~words:(bytes + 1) mem

let to_buffer {data; off; size} = Bigsubstring.create ~pos:off ~len:size data

let merge m1 m2 =
  let m1, m2 = Addr.(if min_addr m1 < min_addr m2 then (m1, m2) else (m2, m1)) in
...
module Make_iterators (M : Monad.S) = struct
  ...
  let exists ?word_size t ~f =
    with_return (fun s ->
        iteri ?word_size t ~f:(fun a x ->
            f a x
            >>= function true -> s.return (return true) | false -> return ())
        >>= fun () -> return false)
  ...
  let for_all ?word_size t ~f =
    with_return (fun s ->
        iteri ?word_size t ~f:(fun a x ->
            f a x
            >>= function false -> s.return (return true) | true -> return ())
        >>= fun () -> return false)
  ...
  let find_map ?word_size t ~f =
    with_return (fun s ->
        iteri t ~f:(fun a w ->
            f a w
            >>= function None -> return () | some -> s.return (return some))
        >>| fun () -> None)

  let find_if ?word_size t ~f =
    find_map t ~f:(fun a w ->
...
let pp_hex fmt t =
  let print_char c =
    let c =
      match Char.of_int c with Some c when Char.is_print c -> c | _ -> '.' in
...
module Trie = struct
  ...
  module Stable = struct
    module V1 = Make (Word.Stable.V1) module V2 = Make (Word.Stable.V2)
  end
...
let domain =
  KB.Domain.optional ~inspect:sexp_of_t "mem" ~equal:(fun x y ->
      Addr.equal x.addr y.addr && Int.equal x.size y.size)

let slot =
  KB.Class.property ~package:"bap.std" Theory.Program.cls "mem" domain
    ~public:true ~desc:"a memory region occupied by the program"

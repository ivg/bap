...
module Get = struct
  ...
  let def {R.Def.tid= t; lhs; rhs} = Def.create ~tid:(tid t) (var lhs) (exp rhs)
...
module Put = struct
  ...
  let def d =
    R.Def.{tid= tid (Term.tid d); lhs= var (Def.lhs d); rhs= exp (Def.rhs d)}
  ...
  let jmp t =
    R.Jmp.
      {tid= tid (Term.tid t); cond= exp (Jmp.cond t); jump= jump (Jmp.kind t)}

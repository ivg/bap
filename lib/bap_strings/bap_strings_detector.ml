...
type +'a t =
  ...
  ; (* w(e2|h2) *)
    p1: float
  ; (* a prior probability of text *)
    h: 'a decision
  ; (* current hypothesis *)
    alphabet: Char.Set.t (* alphabet *) }

let empty = {chars= []; data= []; n= 0; m= 0; cut= 0; len= 0; stage= Working}
...
let hprop {p1; w0; w1; h= {n; m}} = (float m *. w0) +. (float n *. w1) +. p1
...
let update t x char =
  ...
  let h =
    ...
    ; m= (if e1 then t.h.m else t.h.m + 1) } in
...
let step_char t x char =
  ...
  | Working, Accept -> {t with h= {t.h with stage= Accepted; n= 0; m= 0}}

...
let empty_set (type a cmp) map =
  let m :
    ...
    ( module struct
      ...
      let comparator = Map.comparator map end ) in
  Set.empty m

let create_ancestors empty find : ('a, 'a set) map =
  let rec walk set x =
    match find x with None -> set | Some p -> walk (Set.add set p) p in
  object
...
module Partition = struct
  type 'a t = {roots: 'a array; groups: 'a set array; find: 'a -> int option}
      ...
      | Some i_x, Some i_y ->
          ...
          let find x =
            Option.Monad_infix.(
              ...
              fun n ->
              if n < j then n else if n = j then i else n - 1) in
          {roots; groups; find}
...
module To_ocamlgraph (G : Graph) = struct
  ...
  let find_edge g x y =
    match G.Node.edge x y g with Some e -> e | None -> raise Caml.Not_found
...
module Of_ocamlgraph (G : Graph.Sig.P) = struct
  ...
  module Node = struct
    ...
    let edge x y g = try Some (G.find_edge g x y) with Caml.Not_found -> None
  ...
  include Printable.Make (struct
    ...
    let pp ppf graph =
      ...
      let string_of_node = by_natural_order symbols Node.compare (nodes graph) in
...
let compare (type e g n) (module E : Graph with type t = e and type node = n)
...
module Ordering (G : Graph) = struct
  type t = {numbers: number G.Node.Map.t; nodes_left: int; nodes_entered: int}
  ...
  let leave t u =
    ( { t with
      ...
      ; numbers=
          Map.change t.numbers u (function
            | None -> assert false
...
let depth_first_search (type g n e)
  ...
  let init =
    ...
    | None -> (ord, init)
    | Some s ->
        if G.Node.mem s g then visit ord s (start_tree s init) ident
        else (ord, init) in
  G.nodes g
...
let create_namer (type t n) (module G : Graph with type t = t and type node = n)
    g =
  let namer =
    depth_first_search
      (module G)
...
let to_dot (type t n e)
  ...
  Option.iter filename
    (Out_channel.with_file ~f:(fun chan -> Dot.output_graph chan g))
...
let idom (type t n e)
  ...
  let pnum =
    Hashtbl.find_and_call pnums ~if_found:ident ~if_not_found:(fun _ ->
        raise Unreachable) in
...
let dominators (type t n e)
  ...
  let children =
    ...
    |> Seq.fold ~init ~f:(fun tree n ->
           ...
           | None -> tree)
    |> Map.map ~f:G.Node.Set.of_list in
...
let dom_frontier_generic (type t n e) (module Algo : Dom_frontier_algo)
...
let strong_components (type t n e)
  ...
  |> function [] -> Partition.create (module G.Node) comps | _ -> assert false
...
let view (type t n e a b m f c d)
    (module G : Graph
      with type t = t
       and type node = n
       and type edge = e
...
module Fixpoint = struct
  type ('n, 'd) t =
    | Solution :
        {steps: int option; iters: int; default: 'd; approx: ('n, 'd, _) Map.t}
        -> ('n, 'd) t
  ...
  *)
  let compute (type g n d) (module G : Graph with type t = g and type node = n)
    ...
    let user_step =
      ...
      | Some step ->
          fun visits n x x' ->
            let i = match Map.find visits n with None -> 1 | Some x -> x + 1 in
            let visits = Map.set visits ~key:n ~data:i in
            (visits, step i nodes.(n) x x') in
    let get approx n : d =
      match Map.find approx n with Some x -> x | None -> default in
    let step visits works approx =
      ...
      | Some n ->
          ...
          |> continue in
    let can_iter iters =
      match steps with None -> true | Some steps -> iters < steps in
      ...
      Solution
        ...
        ; approx=
            Map.fold approx ~init ~f:(fun ~key:n ~data approx ->
                Map.set approx ~key:nodes.(n) ~data) } in

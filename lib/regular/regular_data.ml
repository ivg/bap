...
type 'a with_info =
  {serializer: 'a; name: string; ver: string; desc: string option}
...
module Class = struct
  type 'a t = {key: 'a Type_equal.Id.t}
  ...
  let refine {key} ~f =
    registry :=
      Registry.update !registry key ~f:(function
        | None ->
  ...
  let collect_info (get, _) def =
    let name =
      match def.module_name with None -> "unknown" | Some name -> name in
    ( name
  ...
  let writers_collector = {collect= (fun def -> collect_info writer_field def)}
  let readers_collector = {collect= (fun def -> collect_info reader_field def)}
...
module Extend (T : sig
  ...
  module Io = struct
    ...
    let dump ?ver ?fmt ch f =
      let rec loop () =
        match f () with None -> () | Some x -> save ?ver ?fmt ch x ; loop ()
      in
      loop ()

    let save_all ?ver ?fmt ch xs =
      let write = save ?ver ?fmt ch in
      let rec loop = function [] -> () | x :: xs -> write x ; loop xs in
      loop xs
...
module Make (T : Versioned) = struct
  module Self = New (T) include Self include Extend (Self)
end

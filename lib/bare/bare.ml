...
module Rule = struct
  type rule =
    {patterns: tuple list; template: fact list; bindings: tuple String.Map.t}
  ...
  let check_rule ~get_pos lhs rhs =
    ...
    | None -> (
        Map.merge def use ~f:(fun ~key ->
          function `Both _ -> None | `Left _ -> None | `Right p -> Some p)
        |> Map.min_elt
        |> function
        | Some (var, pos) -> Error (Unbound (var, pos)) | None -> Ok () )

  let of_sexp ~get_pos sexp =
    match (sexp : Sexp.t) with
    | (Atom _ as bad) | List [(Atom _ as bad); _] | List [_; (Atom _ as bad)] ->
  ...
  let empty_range =
    Parsexp.Positions.{start_pos= beginning_of_file; end_pos= beginning_of_file}
  ...
  let merge bs bs' =
    Map.fold bs' ~init:(Some bs) ~f:(fun ~key ~data ->
      function
      | None -> None
      | Some bs -> (
      ...
      ~f:
        Option.Monad_infix.(
          fun ~rev_prefix p ~suffix ->
            bindings p fact >>= merge rule.bindings
            >>| fun bindings ->

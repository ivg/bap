...
let is_symbol s = String.length s > 1 && Char.(s.[0] = '\'')

let unqoute s =
  if is_quoted s then String.sub ~pos:1 ~len:(String.length s - 2) s else s

let symbol s = if is_symbol s then String.subo ~pos:1 s else s
...
module Parse = struct
  ...
  let fmt prog fmt tree =
    ...
    let unescape off c =
      ...
      with Scanf.Scan_failure _ -> fail (Illegal_escape c) off in
    let nil = `Lit [] in
  ...
  let parse prog tree =
    let rec exp : tree -> ast =
     fun tree ->
      ...
      let forms =
        [ ("if", if_); ("let", let_); ("prog", prog_); ("while", while_)
        ; ("msg", msg); ("set", set); ("error", error) ] in
      let macro op args =
  ...
  let parse_declarations attrs = List.fold ~init:attrs ~f:Attribute.parse
  ...
  let is_keyarg = function {data= Atom s} -> Char.(s.[0] = ':') | _ -> false
  ...
  let defmacro ?docs ?(attrs = []) name ps body prog gattrs tree =
    ...
    @@ Def.Macro.create ?docs
         ~attrs:(parse_declarations gattrs attrs)
  ...
  let defsubst ?docs ?(attrs = []) name body prog gattrs tree =
    let syntax =
      match body with s :: _ when is_keyarg s -> Some s | _ -> None in
  ...
  let toplevels =
    String.Set.of_list
      [ "declare"; "defconstant"; "defparameter"; "defmacro"; "defsubst"; "defun"
      ; "defmethod"; "require" ]

  let declaration gattrs s =
    match s with
  ...
  let stmt gattrs state s =
    ...
    | { data=
          List
            ...
            :: {data= Atom name}
               :: params
                  :: {data= Atom docs}
                     :: {data= List ({data= Atom "declare"} :: attrs)} :: body)
      }
      when is_quoted docs ->
    ...
    | { data=
          List
            ({data= Atom "defun"}
            :: {data= Atom name} :: params :: {data= Atom docs} :: body) }
      when is_quoted docs ->
    ...
    | {data= List ({data= Atom "defun"} :: {data= Atom name} :: params :: body)}
      ->
    ...
    | { data=
          List ({data= Atom "defmethod"} :: {data= Atom name} :: params :: body)
      } ->
    ...
    | { data=
          List
            [ {data= Atom "defparameter"}; {data= Atom name}; body
            ; {data= List ({data= Atom "declare"} :: attrs)} ] } ->
    ...
    | { data=
          List
            [ {data= Atom "defparameter"}; {data= Atom name}; body
            ; {data= Atom docs}; {data= List ({data= Atom "declare"} :: attrs)}
            ] } ->
  ...
  let meta gattrs state s =
    ...
    | { data=
          List
            [ {data= Atom "defconstant"}; {data= Atom name}; {data= Atom body}
            ; {data= Atom docs}; {data= List ({data= Atom "declare"} :: attrs)}
            ] }
      when is_quoted docs ->
    ...
    | { data=
          List [{data= Atom "defconstant"}; {data= Atom name}; {data= Atom body}]
      } ->
        defconst name body state gattrs s
    | { data=
          List
            [ {data= Atom "defmacro"}; {data= Atom name}; params
            ; {data= Atom docs}; {data= List ({data= Atom "declare"} :: attrs)}
            ; body ] }
      when is_quoted docs ->
    ...
    | {data= List [{data= Atom "defmacro"}; {data= Atom name}; params; body]} ->
    ...
    | { data=
          List
            ({data= Atom "defsubst"}
            :: {data= Atom name} :: {data= Atom docs} :: body) }
      when is_quoted docs ->
  ...
  let declarations = List.fold ~init:Attribute.Set.empty ~f:declaration
...
module Load = struct
  ...
  let parse_require tree =
    match tree with
    | {data= List [{data= Atom "require"}; {data= Atom name}]} -> Some (Ok name)
...
let string_of_typ_error = function
  ...
  | Type.Bad_sort ->
      "the sort definition is not recognized, expects:\n\
       sort ::=\n\
...
let pp_parse_error ppf err =
  ...
  | Unresolved (k, n, r) ->
      fprintf ppf "unable to resolve %s `%s', because %a" (string_of_defkind k)
        n Resolve.pp_resolution r
  | Bad_def def ->

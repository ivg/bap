...
let read_int str =
  try Ok (Bitvec.of_string (String.strip str)) with _ -> Error Bad_literal
...
let infer_width x =
  ...
  let return n =
    String.concat [x; ":"; string_of_int (minimum_bitwidth ~base n + sign_bit)]
  in
  if base = 10 then return len else return (len - 1)

(* for the base designator *)

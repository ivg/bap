...
module Use = struct
  ...
  let vars bound ast =
    let use bound { exp = v } id =
      if Set.mem bound v then String.Map.empty else use v id
    in
    ...
    *)
    ...
    let meet id1 id2 g =
      let t, g =
        ...
        | Some u, Some v -> (Tvar.min u v, unify u v g)
      in
    ...
    let join id ids g =
      List.filter_map ids ~f:(get g) |> inter_list |> function
      | None -> g
      | Some vs -> (
          match Map.find g.vars id with
          | None ->
  ...
  let infer_ast glob bindings ast : Gamma.t -> Gamma.t =
    let rec infer vs expr =
      ...
      | { data = Set (v, x); id } ->
          ...
          ++ infer vs x
      | { data = Rep (c, x); id } ->
          Gamma.meet c.id id ++ infer vs c ++ infer vs x
      | { data = Msg (_, xs); id } ->

...
type 'a item =
  ([`Read | `Set_and_create], t, 'a Def.t list) Fieldslib.Field.t_with_perm
...
let add p (fld : 'a item) x = Field.fset fld p (x :: Field.get fld p)
...
let ( ++ ) =
  Map.merge ~f:(fun ~key:_ ->
    function `Both (id, _) | `Left id | `Right id -> Some id)
...
module Callgraph = struct
  ...
  let union xs ~f = List.map xs ~f |> String.Set.union_list
  ...
  let build_kernel defs =
    let ids =
      ...
      fun name -> match Map.find ids name with None -> [] | Some x -> x in
  ...
  let close dir g =
    ...
    |> Seq.fold ~init:g ~f:(fun g n ->
           if G.Node.degree ~dir n g = 0 then G.Edge.insert (edge n) g else g)

  let build defs = close `Out (close `In (build_kernel defs))
...
let rec concat_prog xs =
  List.concat_map xs ~f:(function {data= Seq xs} -> concat_prog xs | x -> [x])
...
module Ast = struct
  ...
  and pp_exp ppf = function
    | Int x -> pp_word ppf x
    | Sym x -> pp_print_string ppf x.data
    | Var x -> pp_var ppf x
    | Ite (c, t, e) ->
...
let pp ppf {pars; mets; defs} =
  let pp_items pp items = fprintf ppf "@[<v>%a@]" (pp_print_list pp) items in
  pp_items pp_par pars ; pp_items pp_met mets ; pp_items pp_def defs
...
module Use = struct
  ...
  let vars bound ast =
    let use bound {exp= v} id =
      if Set.mem bound v then String.Map.empty else use v id in
...
module Reindex = struct
    ...
    let rec map : ast -> ast m =
     fun t ->
      ...
      | Ite (x, y, z) ->
          map x
          >>= fun x ->
          map y >>= fun y -> map z >>| fun z -> {t with data= Ite (x, y, z)}
      | Let (c, x, y) ->
          rename c
          >>= fun c ->
          map x >>= fun x -> map y >>| fun y -> {t with data= Let (c, x, y)}
      | Rep (x, y) ->
          map x >>= fun x -> map y >>| fun y -> {t with data= Rep (x, y)}
      ...
      | Set (v, x) ->
          rename v >>= fun v -> map x >>| fun x -> {t with data= Set (v, x)}
  ...
  let reindex_all p =
    ...
    State.List.map p.defs ~f:(reindex def macros)
    >>= fun defs ->
    State.List.map p.mets ~f:(reindex met macros)
    >>= fun mets ->
    State.List.map p.pars ~f:(reindex par macros)
    >>= fun pars -> State.return (defs, mets, pars)

  let program p =
    let (defs, mets, pars), sources = State.run (reindex_all p) p.sources in
...
module Typing = struct
  ...
  *)
  ...
  type sort = Theory.Value.Sort.Top.t [@@deriving compare, sexp_of]
  ...
  type tval = Tsym | Grnd of sort [@@deriving compare, sexp_of]
  ...
  *)
  module Gamma : sig
    ...
    *)
    ...
    (* typing environment.
       ...
       are ill-typed.*)
    type t = {vars: tvar Id.Map.t; vals: Tval.Set.t Tvar.Map.t}
    ...
    let merge g g' =
      { vars=
          Map.merge g.vars g'.vars ~f:(fun ~key:_ ->
            function `Left t | `Right t -> Some t | `Both (_, t') -> Some t')
      ; vals=
          Map.merge g.vals g'.vals ~f:(fun ~key:_ ->
            function
            | `Left ts | `Right ts -> Some ts | `Both (_, ts) -> Some ts) }

    let add_var id t g = {g with vars= Map.set g.vars ~key:id ~data:t}
    let add_val t v g = {g with vals= Map.set g.vals ~key:t ~data:v}
    ...
    let join id ids g =
      ...
      | Some vs -> (
        ...
        | Some v ->
            { g with
              vals=
                Map.update g.vals v ~f:(function
                  | None -> vs
    ...
    let constr_name id n g =
      ...
      | None -> g'
      | Some v ->
          Map.fold g'.vars ~init:g ~f:(fun ~key:id' ~data:v' g ->
              if Tvar.equal v v' then meet id id' g else g)

    let constr_grnd id t g =
      let v = match Map.find g.vars id with None -> Tvar id | Some v -> v in
  ...
  type signature = Lisp.Type.signature =
    {args: typ list; rest: typ option; ret: typ}
  ...
  let pp_args ppf args = pp_print_list Lisp.Type.pp ppf args
  ...
  let signatures glob gamma name =
    ...
    | Some sign -> [sign]
    | None ->
        List.fold glob.funcs ~init:[] ~f:(fun sigs def ->
            if String.equal (Def.name def) name then
              signature_of_gamma def gamma :: sigs
            else sigs)
  ...
  let apply glob id name args gamma =
    ...
    |> List.reduce ~f:join_gammas
    |> function None -> gamma | Some gamma -> gamma
  ...
  let constr_glob {globs} vars var gamma =
    if Map.mem vars var.data.exp then gamma
      ...
      match Map.find globs var.data.exp with
      | None -> gamma
  ...
  let infer_ast glob bindings ast : Gamma.t -> Gamma.t =
    let rec infer vs expr =
      ...
      | {data= Var v; id} ->
          ...
          ++ Gamma.meet id v.id ++ constr_glob glob vs v
          ++ Gamma.constr v.id v.data.typ
      | {data= Ite (x, y, z); id} ->
          Gamma.join id [y.id; z.id] ++ infer vs x ++ infer vs y ++ infer vs z
      | {data= Let (v, x, y); id} ->
          Gamma.meet y.id id
          ++ infer (push vs v) y
          ++ Gamma.meet x.id v.id ++ infer vs x
          ++ Gamma.constr v.id v.data.typ
      | {data= App (Dynamic name, xs); id} ->
          apply glob id name xs ++ reduce vs xs
      | {data= Seq []} -> ident
      | {data= Seq xs; id} -> Gamma.meet (last xs) id ++ reduce vs xs
      | {data= Set (v, x); id} ->
          Gamma.join id [v.id; x.id]
          ++ Gamma.constr v.id v.data.typ
          ++ infer vs x
      | {data= Rep (c, x); id} -> Gamma.meet c.id id ++ infer vs c ++ infer vs x
      | {data= Msg (_, xs); id} ->
          Gamma.constr id (sort Theory.Bool.t) ++ reduce vs xs
      | {data= Err _} -> ident
  ...
  let find_func funcs id = List.find funcs ~f:(fun f -> Id.(f.id = id))
  ...
  let make_prims {primits} =
    List.fold primits ~init:String.Map.empty ~f:(fun ps p ->
        Map.set ps ~key:(Def.name p) ~data:(Def.Primitive.signature p))
  ...
  module Type = struct
    ...
    let check vars p : error list =
      ...
      List.fold (Gamma.exps gamma) ~init:Loc.Map.empty ~f:(fun errs exp ->
          ...
          if Source.has_loc p.sources exp then
            match Gamma.get gamma exp with
            | None -> errs
            | Some ts ->
                if Set.is_empty ts then
                  Map.set errs ~key:(Source.loc p.sources exp) ~data:exp
                else errs
          else errs)
      |> Map.to_alist

    let pp_error ppf (loc, id) =
      fprintf ppf "%a@\nType error - expression is ill-typed: %a" Loc.pp loc
        Id.pp id
  end
end

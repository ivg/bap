...
module Variables = struct
  ...
  let var t =
    ...
    | {data= Atom v; id; eq} -> (
      match Var.read id eq v with
      | Ok v -> v
      | Error err -> fail (Var_error err) [t] )

  let parse = List.map ~f:var
  let global = Attribute.register ~name:"global" ~add:List.append ~parse
  let static = Attribute.register ~name:"static" ~add:List.append ~parse
end
...
module External = struct
  ...
  let t = Attribute.register ~name:"external" ~add:List.append ~parse
end
...
module Advice = struct
  ...
  let methods = String.Map.of_alist_exn [(":before", Before); (":after", After)]
  ...
  let add d1 d2 =
    { methods=
        Map.merge d1.methods d2.methods ~f:(fun ~key ->
          function
          | `Both (xs, ys) -> Some (Set.union xs ys)
          | `Left xs | `Right xs -> Some xs) }

  let t = Attribute.register ~name:"advice" ~parse ~add
end

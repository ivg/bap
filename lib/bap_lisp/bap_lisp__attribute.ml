...
type 'a attr =
  {key: 'a Univ_map.Key.t; add: 'a -> 'a -> 'a; parse: tree list -> 'a}
...
let register ~name ~add ~parse =
  let attr = {key= Univ_map.Key.create ~name sexp_of_opaque; add; parse} in
  let parser = Parser (make_parser attr) in
  Hashtbl.add_exn parsers ~key:name ~data:parser ;
  attr.key

let expected_parsers () = String.Table.keys parsers |> String.concat ~sep:" | "

let parse s attrs name values =
...
module Set = struct let get = Univ_map.find let empty = Univ_map.empty end

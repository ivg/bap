...
module Props = struct
  ...
  let set_if cond flag = if cond then fun flags -> flags + flag else ident
  ...
  let slot =
    KB.Class.property ~package:"bap.std" Theory.Program.Semantics.cls
      "insn-properties" domain ~persistent ~public:true
...
module Slot = struct
  ...
  let name =
    KB.Class.property ~package:"bap.std" Theory.Program.Semantics.cls
      "insn-opcode" text ~persistent:KB.Persistent.string ~public:true
      ~desc:"instruction opcode"

  let asm =
    KB.Class.property ~package:"bap.std" Theory.Program.Semantics.cls "insn-asm"
      text ~persistent:KB.Persistent.string ~public:true
  ...
  let ops_persistent =
    KB.Persistent.of_binable
      (module struct type t = Op.t array option [@@deriving bin_io] end)

  let ops =
    KB.Class.property ~package:"bap.std" Theory.Program.Semantics.cls "insn-ops"
      ops_domain ~persistent:ops_persistent ~public:true
  ...
  let dests =
    ...
    KB.Class.property ~package:"bap.std" Theory.Program.Semantics.cls
      "insn-dests" data ~persistent ~public:true
...
let lookup_jumps bil =
  ...
  (object
    ...
    #run bil
    {jump= false; cond= false; indirect= false}
  |> fun v ->
  if not v.jump then []
  else
...
let lookup_side_effects bil =
  (object
     inherit [kind list] Stmt.visitor

     method! enter_store ~mem:_ ~addr:_ ~exp:_ _ _ acc = `May_store :: acc

     method! enter_load ~mem:_ ~addr:_ _ _ acc = `May_load :: acc
  end)
    #run bil []
...
let of_basic ?bil insn : t =
  ...
  let is_bil kind =
    if Option.is_some bil then
      List.mem ~equal:[%compare.equal: kind] bil_kinds kind
    else is kind in
  ...
  write effect
    Slot.
      [ Props.slot <-- props; name <-- Insn.name insn
      ; asm <-- normalize_asm (Insn.asm insn); ops <-- Some (Insn.ops insn) ]
...
let pp_asm ppf insn = Format.fprintf ppf "%s" (normalize_asm (asm insn))

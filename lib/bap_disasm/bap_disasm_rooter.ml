...
let union (Rooter r1) (Rooter r2) = Rooter (Seq.append r1 r2)
...
let provide rooter =
  ...
  let roots =
    roots rooter |> Seq.map ~f:Word.to_bitvec |> Seq.fold ~init ~f:Set.add in
  let promise prop =
    KB.promise prop
    @@ fun label ->
    KB.collect Theory.Label.addr label
    >>| function
    | None -> None | Some addr -> Option.some_if (Set.mem roots addr) true in

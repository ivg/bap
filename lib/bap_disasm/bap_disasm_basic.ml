...
type imm_info = {imm_small: int; imm_large: int64 sexp_option}
[@@deriving bin_io, compare, sexp]
...
type disassembler =
  {dd: int; insn_table: Table.t; reg_table: Table.t; mutable users: int}

let last_id = ref 0
let disassemblers = Hashtbl.create (module String)

type dis = {name: string; asm: bool; kinds: bool}

let get {name} =
  match Hashtbl.find disassemblers name with
  | None -> failwith "Trying to access a closed disassembler"
  | Some d -> d
...
module Imm = struct
  ...
  let create dis ~insn ~oper =
    let data =
      ...
      let imm_large =
        ...
        else Some (C.insn_op_imm_value !!dis ~insn ~oper) in
  ...
  module T = struct
    ...
    let hash {data= n} =
      if fits n.imm_small then n.imm_small else Int64.hash (uw n.imm_large)
  end
...
module Insn = struct
  ...
  let sexp_of_t ins =
    ...
    Sexp.List
      (Sexp.Atom name :: List.map ops ~f:(fun op -> Sexp.Atom (Op.to_string op)))
    ...
    let kinds =
      if kinds then
        List.filter_map Kind.all ~f:(fun k ->
            ...
            if C.is_supported !!dis p then
              Option.some_if (C.insn_satisfies !!dis ~insn p) k
            else None)
      else [] in
    let opers =
  ...
  let slot =
    KB.Class.property ~package:"bap.std" Theory.Program.cls "insn" domain
...
type (+'a, +'k, 's, 'r) state =
  ...
  ; hit:
      (('a, 'k, 's, 'r) state -> mem -> (asm, kinds) insn -> 's -> 'r) option
      sexp_opaque }
[@@deriving sexp_of]

let create_state ?(backlog = 8) ?(stop_on = []) ?stopped ?invalid ?hit dis mem
    ~return =
  { backlog
  ; dis
  ; return
...
let with_preds s (ps : pred list) =
  let add p =
    ...
    if C.is_supported !!(s.dis) p then C.predicates_push !!(s.dis) p in
  ...
  let rec loop s data =
    ...
    let insns =
      Array.init n ~f:(fun insn ->
          let is_valid = not (C.insn_satisfies !!(s.dis) ~insn C.Is_invalid) in
          ( insn_mem s ~insn
    ...
    else if C.insn_satisfies !!(s.dis) ~insn C.Is_invalid then
...
let jump s mem data : 'r =
  ...
  set_memory s.dis s.current ; step s data

let stop s data = s.return data

let back s data =
  let current, history =
    match s.history with [] -> (s.current, s.history) | x :: xs -> (x, xs) in
...
let create ?(debug_level = 0) ?(cpu = "") ?(backend = "llvm") triple =
  ...
  | None ->
      let dd =
        match C.create ~backend ~triple ~cpu ~debug_level with
        | n when n >= 0 -> Ok n
...
let with_disasm ?debug_level ?cpu ?backend triple ~f =
  create ?debug_level ?cpu ?backend triple
  >>= fun dis -> f dis >>| fun res -> close dis ; res
...
let available_backends () = C.backends_size () |> List.init ~f:C.backend_name

...
module Parents = struct
  ...
  include Binable.Of_binable (struct
            ...
            (struct
              ...
              let of_binable xs =
                let init =
                  ok_exn @@ Map.of_increasing_sequence (module Word) xs in
...
let callgraph_of_disasm disasm =
  Driver.explore disasm ~init:Callgraph.init
    ...
    ~node:(fun n g ->
      ...
      Theory.Label.for_addr (Word.to_bitvec n)
      >>= fun code ->
      KB.collect Theory.Label.is_subroutine code
      >>| function Some true -> Callgraph.mark_as_root n g | _ -> g)
    ~edge:(fun src dst g ->

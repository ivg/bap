...
type dsts = {call: bool; barrier: bool; indirect: bool; resolved: Addr.Set.t}
[@@deriving bin_io]
...
module Machine : sig
  ...
  val view :
    state -> mem -> empty:(state -> 'a) -> ready:(state -> mem -> 'a) -> 'a
  ...
  let mark_data s addr =
    ...
    ; jmps=
        Map.filter_map (Map.remove s.jmps addr) ~f:(fun dsts ->
  ...
  let pp_task ppf = function
    ...
    | Jump {src; age} -> Format.fprintf ppf "Delay%d %a" age Addr.pp src
  ...
  let rec step s =
    ...
    | [] ->
        if Set.is_empty s.usat then {s with stop= true}
          ...
          step {s with work= [Dest {dst= Set.min_elt_exn s.usat; parent= None}]}
    ...
    | (Fall {dst= next} as curr) :: work ->
        if is_code s next then step {s with begs= Set.add s.begs next; work}
    ...
    | (Jump {src; dsts} as jump) :: ([] as work)
     |(Jump {src; dsts; age= 0} as jump) :: work ->
        if Set.mem s.data src then step @@ cancel jump {s with work}
          ...
          let resolved =
            Set.filter dsts.resolved ~f:(fun dst -> not (Set.mem s.data dst))
          in
    ...
    | (Jump jmp as self) :: Fall ({dst= next} as slot) :: work ->
        ...
        step
          { s with
            work=
              Fall {slot with delay}
              :: Jump {jmp with age= jmp.age - 1; src= next}
              :: work }
    | Jump jmp :: work -> step {s with work= Jump {jmp with age= 0} :: work}

  let decoded s mem =
    let addr = Memory.min_addr mem in
    {s with code= Set.add s.code addr; usat= Set.remove s.usat addr}
  ...
  let jumped s mem dsts delay =
    ...
    let next =
      if dsts.barrier && delay = 0 then Dest {dst= next; parent= None}
  ...
  let moved s mem =
    ...
    let next =
      match parent with
      | Jump {dsts= {barrier= true}} -> Dest {dst= next; parent= None}
      | parent -> Fall {dst= next; parent; delay= Ready None} in
    let work =
      match s.curr with
  ...
  let failed s addr = step @@ cancel s.curr @@ mark_data s addr
...
let new_insn arch mem insn =
  ...
  >>= fun () -> KB.provide Dis.Insn.slot code (Some insn) >>| fun () -> code

let collect_dests arch mem insn =
  let width = Size.in_bits (Arch.addr_size arch) in
  new_insn arch mem insn
  >>= fun code ->
  KB.collect Theory.Program.Semantics.slot code
  >>= fun insn ->
  let init =
    { call= Insn.(is call insn)
    ; barrier= Insn.(is barrier insn)
    ; indirect= false
    ; resolved= Set.empty (module Addr) } in
  KB.Value.get Insn.Slot.dests insn
  |> function
  | None -> KB.return init
  | Some dests ->
      Set.to_sequence dests
      |> KB.Seq.fold ~init ~f:(fun {call; barrier; indirect; resolved} label ->
             KB.collect Theory.Label.addr label
             >>| function
             | Some d ->
                 { call
                 ; barrier
             ...
             | None -> {call; barrier; indirect= true; resolved})
      >>= fun res -> KB.return res
...
let classify_mem mem =
  ...
  |> KB.Seq.fold ~init:(empty, empty, empty) ~f:(fun (code, data, root) off ->
         ...
         KB.Object.scoped Theory.Program.cls
         @@ fun label ->
         KB.provide Theory.Label.addr label slot
         >>= fun () ->
         KB.collect Theory.Label.is_valid label
         >>= function
         | Some false -> KB.return (code, Set.add data addr, root)
         | r -> (
             let code = if Option.is_none r then code else Set.add code addr in
             KB.collect Theory.Label.is_subroutine label
             >>| function
...
let query_arch addr =
  KB.Object.scoped Theory.Program.cls
  @@ fun obj ->
  KB.provide Theory.Label.addr obj (Some addr)
  >>= fun () -> KB.collect Arch.slot obj

let already_scanned addr s =
  List.exists s.mems ~f:(fun mem -> Memory.contains mem addr)
  ...
  |> KB.Seq.iter ~f:(fun (_, dsts) ->
         if dsts.call then
           ...
           |> KB.Seq.iter ~f:(fun addr ->
                  ...
                  >>= fun dst ->
                  KB.provide Theory.Label.is_valid dst (Some true)
                  >>= fun () ->
...
let scan mem s =
  ...
  if already_scanned start s then KB.return s
  else
    query_arch (Word.to_bitvec start)
    >>= fun arch ->
    match Dis.create (Arch.to_string arch) with
    | Error _ -> KB.return s
    | Ok dis ->
        scan_mem arch dis s.debt mem
        >>= fun {Machine.begs; jmps; data; debt} ->
        let jmps =
          Map.merge s.jmps jmps ~f:(fun ~key:_ ->
            function `Left dsts | `Right dsts | `Both (_, dsts) -> Some dsts)
        in
        let begs = Set.union s.begs begs in
        let data = Set.union s.data data in
        let s = {begs; data; jmps; mems= mem :: s.mems; debt} in
        commit_calls s >>| fun () -> s
...
let merge t1 t2 =
  ...
  ; jmps=
      Map.merge t1.jmps t2.jmps ~f:(fun ~key:_ ->
        function
...
let execution_order stack =
  KB.List.fold stack ~init:[] ~f:(fun insns insn ->
      KB.collect Theory.Program.Semantics.slot insn
      >>| fun s ->
...
let may_fall insn =
  KB.collect Theory.Program.Semantics.slot insn
  >>| fun insn -> not Insn.(is barrier insn)

let explore ?entry:start ?(follow = always) ~block ~node ~edge ~init
    {begs; jmps; data; mems} =
  let find_base addr =
  ...
  let edge_insert cfg src dst =
    match dst with None -> KB.return cfg | Some dst -> edge src dst cfg in
            ...
            | Some base -> (
                Dis.run dis (view beg base) ~stop_on:[`Valid]
                  ...
                  ~hit:(fun s mem insn (curr, len, insns, _) ->
                    ...
                    if Set.mem begs next || Map.mem jmps curr then
                      may_fall insn
                      >>| fun may -> (curr, len, insn :: insns, may)
                    else
                      Dis.jump s (view next base)
                ...
                | None when may_fall ->
                    build cfg fall
                    >>= fun (cfg, dst) ->
                    edge_insert cfg block dst
                    >>= fun cfg -> KB.return (cfg, Some block)
                | None -> KB.return (cfg, Some block)
                | Some {resolved= dsts} ->
                    let dsts = if may_fall then Set.add dsts fall else dsts in
                    Set.to_sequence dsts
                    |> KB.Seq.fold ~init:cfg ~f:(fun cfg dst ->
                           build cfg dst
                           >>= fun (cfg, dst) -> edge_insert cfg block dst)
                    >>= fun cfg -> KB.return (cfg, Some block) ) ) ) in
  match start with
  | None ->
      Set.to_sequence begs
      |> KB.Seq.fold ~init ~f:(fun cfg beg -> build cfg beg >>| fst)

...
module Machine : sig
    ...
    | [] ->
        if Set.is_empty s.usat then { s with stop = true }
          ...
          step
            {
              s with
              work = [ Dest { dst = Set.min_elt_exn s.usat; parent = None } ];
            }
        ...
        if Set.mem s.data src then step @@ cancel jump { s with work }
          ...
          @@ Set.fold resolved ~init ~f:(fun s next ->
                 if not (is_visited s next) then
                   {
                     s with
                     work = Dest { dst = next; parent = Some jump } :: s.work;
                   }
  ...
  let jumped s mem dsts delay =
    ...
    let next =
      ...
      else Fall { dst = next; parent = jump; delay = Ready None }
    in
  ...
  let moved s mem =
    ...
    let next =
      ...
      | parent -> Fall { dst = next; parent; delay = Ready None }
    in
    let work =
      match s.curr with
...
let collect_dests arch mem insn =
  ...
  | Some dests ->
      ...
      |> KB.Seq.fold ~init
           ~f:(fun { call; barrier; indirect; resolved } label ->
...
let explore ?entry:start ?(follow = always) ~block ~node ~edge ~init
  ...
  let rec build cfg beg =
    if Set.mem data beg then KB.return (cfg, None)
      ...
      | true -> (
          with_disasm beg cfg @@ fun arch dis ->

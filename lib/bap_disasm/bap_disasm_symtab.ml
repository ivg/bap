...
let ( <-- ) g f = match g with None -> None | Some (e, g) -> Some (e, f g)

let build_cfg disasm calls entry =
  Disasm.explore disasm ~entry ~init:None
    ~follow:(fun dst -> KB.return (Callgraph.belongs calls ~entry dst))
    ~block:(fun mem insns ->
      Disasm.execution_order insns
      >>= fun insns ->
      KB.List.filter_map insns ~f:(fun label ->
          KB.collect Theory.Program.Semantics.slot label
          >>= fun s ->
          KB.collect Memory.slot label
          >>| function None -> None | Some mem -> Some (mem, s))
      >>| fun insns -> Block.create mem insns)
    ~node:(fun n g ->
      KB.return
      @@
      if Addr.equal (Block.addr n) entry then
        Some (n, Cfg.Node.insert n Cfg.empty)
      else g <-- Cfg.Node.insert n)
    ~edge:(fun src dst g ->
      let msrc = Block.memory src and mdst = Block.memory dst in
      let next = Addr.succ (Memory.max_addr msrc) in
      let kind =
        if Addr.equal next (Memory.min_addr mdst) then `Fall else `Jump in
      let edge = Cfg.Edge.create src dst kind in
      KB.return (g <-- Cfg.Edge.insert edge))

let build_symbol disasm calls start =
  build_cfg disasm calls start
  >>= function
  | None ->
      Format.eprintf "Failed to obtain a CFG from %a@\n%!" Addr.pp start ;
      assert false
  | Some (entry, graph) ->
      Symbolizer.get_name start >>| fun name -> (name, entry, graph)

let create_intra disasm calls =
  Callgraph.entries calls |> Set.to_sequence
  |> KB.Seq.fold ~init:empty ~f:(fun symtab entry ->
         build_symbol disasm calls entry >>| fun fn -> add_symbol symtab fn)

let create_inter disasm calls init =
  Disasm.explore disasm ~init
    ...
    ~edge:(fun src dst s ->
      ...
      if Callgraph.siblings calls src dst then KB.return s
      else
        Symbolizer.get_name dst
        >>| fun name ->
        if Addr.equal next dst then {s with icalls= Map.set s.icalls src name}
        else {s with ecalls= Map.set s.ecalls src name})

let create disasm calls =
  create_intra disasm calls >>= create_inter disasm calls

...
let insn arch mem insn =
  ...
  let rec loop acc addr =
    let stmts, next = disasm_instr mode mem addr in
    if Addr.(next > last) then stmts::acc
    else loop (stmts::acc) next in
  Or_error.try_with (fun () -> Memory.min_addr mem |> loop [] |> List.concat)

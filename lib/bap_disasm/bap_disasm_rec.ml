  ...
  Driver.explore disasm ~init:Cfg.empty
    ~block:(fun mem insns ->
      ...
      >>= KB.List.filter_map ~f:(fun label ->
              KB.collect Basic.Insn.slot label
              >>= fun basic ->
              KB.collect Theory.Program.Semantics.slot label
              >>= fun s ->
              KB.collect Memory.slot label
              >>| function
              | None -> None | Some mem -> Some (mem, create_insn basic s))
      >>| Block.create mem)
...
let provide_arch arch mem =
  ...
  | Some p ->
      ...
      if Memory.contains mem p then arch else `unknown

let scan arch mem state = provide_arch arch mem ; Driver.scan mem state

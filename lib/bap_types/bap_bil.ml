...
module Exp = struct
  type exp =
    ...
    | UnOp of unop * exp
    | Var of var
    | Int of word  (** Cast to a new type *)
    | Cast of cast * nat1 * exp
    | Let of var * exp * exp
    | Unknown of string * typ
    | Ite of exp * exp * exp  (** Extract hbits to lbits of e (Reg type) *)
    | Extract of nat1 * nat1 * exp  (** Concat two reg expressions together *)
    | Concat of exp * exp
  [@@deriving bin_io, compare, sexp]
end
...
module Stmt = struct
  type stmt =
    ...
    | Move of var * exp  (** Jump to a address *)
    | Jmp of exp  (** Statement with semantics not expressible in BIL *)
    | Special of string

...
type edge = {src: blk term; dst: blk term; pos: int} [@@deriving bin_io, sexp]
...
module Node = struct
  ...
  let update b blk' t =
    ...
    | Some blk -> (
        ...
        | [] -> {t with sub}
        | ds ->
            List.fold ~init:{t with sub} ds ~f:(fun t ->
              function
  ...
  let has_edge src dst t : bool =
    Map.find t.preds (Term.tid dst)
    |> function None -> false | Some ps -> Set.mem ps (Term.tid src)
...
module Edge = struct
  ...
  let jmps dir e g =
    ...
    | None -> Seq.empty
    | Some src -> (
      match dir with `after -> jmps_after e src | `before -> jmps_before e src )
...
let preds_of_sub sub : Tid.Set.t Tid.Map.t =
  ...
  |> Seq.fold ~init:Tid.Map.empty ~f:(fun ins src ->
         ...
         let ins =
           Map.change ins src_id (function

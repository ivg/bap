...
let eval slot exp =
  try_eval slot exp
  |> function
  | Ok v -> v | Error conflict -> raise (Internal_runtime_error conflict)
...
let exec stmt =
  try_exec stmt
  |> function Ok () -> () | Error err -> raise (Internal_runtime_error err)

let put slot exp =
  exec @@ (exp >>= fun v -> this >>= fun x -> Knowledge.provide slot x (Some v))

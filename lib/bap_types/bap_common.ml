...
module Type = struct
  type t =
    ...
    | Imm of nat1  (** [Mem (a,t)]memory with a specified addr_size *)
...
module Arch = struct
  ...
  type sparc = [`sparc | `sparcv9] [@@deriving bin_io, compare, enumerate, sexp]
  type nvptx = [`nvptx | `nvptx64] [@@deriving bin_io, compare, enumerate, sexp]

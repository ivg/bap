...
let pp_generic ?(case : [`lower | `upper] = `upper)
  ...
  let () =
    ...
    | `base -> pp_prefix ppf
    | `auto -> if Z.(geq x (min (word 10) base)) then pp_prefix ppf in
  let fmt =
    format_of_string
    @@
...
let of_binary ?width endian num =
  let num =
    match endian with LittleEndian -> num | BigEndian -> String.rev num in
  ...
  if is_zero t then take (repeat '\x00') n
    ...
    let s =
      unfold ~init:start ~f:(fun i ->
          if i = finish then None else Some (bytes.[i], next i)) in
...
module Stable = struct
  module V1 = struct
    ...
    let to_legacy x =
      V1.{z= Bitvec.to_bigint (payload x); w= bitwidth x; signed= is_signed x}

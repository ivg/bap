...
let rec pp fmt s =
  ...
  | Jmp ((Exp.Var _ | Exp.Int _) as exp) -> fprintf fmt "jmp %a" Bap_exp.pp exp
  | Jmp exp -> fprintf fmt "jmp (%a)" Bap_exp.pp exp
  | Special s -> fprintf fmt "special (%s)" s
  | While (cond, body) ->
      fprintf fmt "@[<v0>@[<v2>while (%a) {@;%a@]@;}@]" Bap_exp.pp cond pp_list
        body
  | If (cond, ts, []) ->
      fprintf fmt "@[<v0>@[<v2>if (%a) {@;%a@]@,}@]" Bap_exp.pp cond pp_list ts
  | If (cond, ts, fs) ->
      fprintf fmt "@[<v0>@[<v2>if (%a) {@;%a@]@,}@;%a@]" Bap_exp.pp cond pp_list
        ts pp_else fs
...
let pp_stmts fmt ss = fprintf fmt "@[<v0>@[<v2>{@\n%a@]@\n}@]" pp_list ss
...
module Infix = struct let ( := ) v x = Bap_bil.Stmt.Move (v, x) end
...
let persistent =
  Knowledge.Persistent.of_binable
    (module struct type t = stmt list [@@deriving bin_io] end)

let slot =
  Knowledge.Class.property ~package:"bap.std" ~persistent
    Theory.Program.Semantics.cls "bil" domain ~public:true

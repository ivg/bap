...
module Make (Interval : Interval) = struct
  ...
  *)
  ...
  let rec mapi m ~f =
    Option.map m ~f:(fun m ->
        {m with lhs= mapi m.lhs ~f; data= f m.key m.data; rhs= mapi m.rhs ~f})
  ...
  let remove_if ~leave_if ~remove_if map mem =
    let rec remove = function
      ...
      | Some t -> bal (remove t.lhs) t.key t.data (remove t.rhs) in
    remove map
  ...
  let filter_mapi map ~f =
    let rec fmap = function
      ...
      | Some t -> (
        ...
        | Some data -> bal (fmap t.lhs) t.key data (fmap t.rhs) ) in
    fmap map
  ...
  module C = Container.Make (struct
    ...
    let rec iter m ~f =
      Option.iter m ~f:(fun m -> iter m.lhs ~f ; f m.data ; iter m.rhs ~f)

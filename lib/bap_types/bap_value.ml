...
type 'a tag =
  {key: 'a Type_equal.Id.t; slot: (Theory.program, 'a option) KB.slot}
...
let register_slot (type a) slot (module S : S with type t = a) : a tag =
  ...
  let collect obj dict =
    ...
    KB.collect slot obj
    >>| function None -> dict | Some x -> Univ_map.set dict key x in
...
let register (type a) ~name ~uuid (module S : S with type t = a) =
  let persistent =
    KB.Persistent.of_binable
      (module struct type t = S.t option [@@deriving bin_io] end) in
  let equal x y = S.compare x y = 0 in
  let domain = KB.Domain.optional ~equal name in
  let slot =
    KB.Class.property ~persistent ~package:uuid Theory.Program.cls name domain
  in
...
let info typeid =
  Hashtbl.find_and_call types typeid ~if_found:ident
...
module Tag = struct
  ...
  let register (type a) ~name ~uuid (typ : (module S with type t = a)) : a tag =

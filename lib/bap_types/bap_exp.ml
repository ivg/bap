...
module PP = struct
  ...
  let rec pp fmt exp =
    ...
    | Ite (ce, te, fe) -> pr "@[<2>if %a@;then %a@;else %a@]" pp ce pp te pp fe
    | Extract (hi, lo, exp) -> pr "extract:%d:%d[%a]" hi lo pp exp
...
let persistent =
  Knowledge.Persistent.of_binable
    (module struct type t = Bap_bil.exp [@@deriving bin_io] end)

let slot =
  Knowledge.Class.property ~package:"bap.std" ~persistent Theory.Value.cls "exp"

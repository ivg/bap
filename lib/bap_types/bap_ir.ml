...
module Var = Bap_var
module Bil = struct include Bap_visitor include Bap_helpers end
...
module Stmt = struct include Bap_stmt include Stmt include Bap_helpers.Stmt end
...
module Tid = struct
  ...
  let for_addr s = generate Theory.Label.for_addr @@ Bap_bitvector.to_bitvec s
  ...
  let intern n =
    Toplevel.put name
      ( KB.Symbol.intern n Theory.Program.cls
      >>= fun t -> KB.provide Theory.Label.name t (Some n) >>| fun () -> t ) ;
    Toplevel.get name
...
module Def : sig
  type t = {var: unit Theory.var; rhs: Rhs.t} [@@deriving bin_io, compare, sexp]
...
module Program : sig
  ...
  let mangle_name addr tid name =
    match addr with
    | Some a ->
        sprintf "%s@%s" name @@ Bap_bitvector.string_of_value ~hex:true a
...
module Array = struct
  ...
  *)
  ...
  let pp ppx ppf xs = Array.iter xs ~f:(fun x -> Format.fprintf ppf "%a" ppx x)
end
...
let nil_blk : blk term = make_term Tid.nil {phis= [||]; defs= [||]; jmps= [||]}
...
let sub_t : (program, sub) cls =
  {par= Top; typ= Sub; nil= nil_sub; set= Program.update; get= Program.subs}

let term_pp pp_self ppf t =
  let open Format in
  let attrs = Dict.data t.dict in
  Seq.iter attrs ~f:(fun attr -> pp_open_tag ppf (asprintf "%a" pp_attr attr)) ;
  fprintf ppf "@[%08Lx: %a@]@."
...
module Ir_arg = struct
  ...
  module Intent = struct
    module T = struct type t = intent option [@@deriving bin_io] end
  ...
  let with_rhs ({self= {Def.var}} as t) rhs = {t with self= Def.{var; rhs}}
  ...
  let warn_unused =
    Bap_value.Tag.register
      (module Unit)
      ~name:"warn-unused" ~uuid:"7aa17c89-cc9b-4ed2-8700-620cb9e09491"

  let format =
    Bap_value.Tag.register
      (module String)
      ~name:"format" ~uuid:"d864c411-73eb-48b2-a7e9-33b51fa540c9"

  let alloc_size =
    Bap_value.Tag.register
      (module Unit)
      ~name:"alloc-size" ~uuid:"b29905b3-4fb5-486e-8064-9b63cadc6174"

  let restricted =
    Bap_value.Tag.register
      (module Unit)
      ~name:"restricted" ~uuid:"5ee30262-aed9-4aa8-a8e3-34a061104420"

  let nonnull =
    Bap_value.Tag.register
      (module Unit)
      ~name:"nonnull" ~uuid:"3c0a6181-9a9c-4cf4-aa37-8ceebd773952"
...
module Ir_def = struct
  ...
  let pp_self ppf {Def.var; rhs} =
    Format.fprintf ppf "%s := %a" (Theory.Var.name var) Bap_exp.pp (Rhs.exp rhs)
...
module Ir_phi = struct
  ...
  let with_lhs ({self} as t) lhs = {t with self= Phi.{self with var= to_var lhs}}

  let reify ?(tid = Tid.create ()) var bs =
    let bs = List.map bs ~f:(fun (t, x) -> (t, Rhs.of_value x)) in
    make_term tid
      Phi.{var= Theory.Var.forget var; map= Map.of_alist_exn (module Tid) bs}

  let of_list ?(tid = Tid.create ()) var bs : phi term =
    let bs = List.map bs ~f:(fun (t, x) -> (t, Rhs.of_exp x)) in
    make_term tid Phi.{var= to_var var; map= Map.of_alist_exn (module Tid) bs}
  ...
  let update ({self= {Phi.map; var}} as t) tid exp : phi term =
    {t with self= Phi.{var; map= Map.set map ~key:tid ~data:(Rhs.of_exp exp)}}
...
module Ir_jmp = struct
  ...
  let exps (jmp : jmp term) : exp Sequence.t =
    ...
    let label label =
      match label with Indirect exp -> yield exp | Direct _ -> return () in
    let call call =
      Option.value_map ~default:(return ()) (Call.return call) ~f:label
      >>= fun () -> label (Call.target call) in
    let r =
      match kind jmp with
      | Call t -> call t
...
module Term = struct
  ...
  let tid x = x.tid
  let with_field field term x = {term with self= Field.fset field term.self x}
  ...
  let next t p tid =
    ...
    findi t p tid
    >>= fun (i, _) -> if i + 1 >= Array.length ts then None else Some ts.(i + 1)
  ...
  let live =
    Bap_value.Tag.register
      (module Unit)
      ~name:"live" ~uuid:"4d3871ab-9481-4d41-97a3-cd3136acfa90"

  let dead =
    Bap_value.Tag.register
      (module Unit)
      ~name:"dead" ~uuid:"6009fb21-2a6c-4511-9aa4-92b2894debc7"

  let visited =
    Bap_value.Tag.register
      (module Unit)
      ~name:"visited" ~uuid:"0e162aa3-153f-44a3-88e7-6e42d876e760"
  ...
  let invariant =
    Bap_value.Tag.register
      (module Exp)
      ~name:"invariant" ~uuid:"743d712b-7ee4-46da-b3b7-98d3ca5e618b"
  ...
  let persistent =
    Knowledge.Persistent.of_binable
      (module struct type t = blk term list [@@deriving bin_io] end)

  let slot =
    Knowledge.Class.property Theory.Program.Semantics.cls "bir" domain ~package
      ~persistent ~public:true ~desc:"BIL semantics in a graphical IR"

  let change t p tid f =
    Array.findi (t.get p.self) ~f:(fun _ x -> Tid.(x.tid = tid))
    |> function
  ...
  let map_term (type t) (cls : (_, t) cls) ?(program : program map = ident)
      ?(sub : sub map = ident) ?(arg : arg map = ident) ?(blk : blk map = ident)
      ?(phi : phi map = ident) ?(def : def map = ident) ?(jmp : jmp map = ident)
      (t : t term) : t term =
    match cls.typ with
    | Nil -> assert false
    | Top -> program t
...
module Ir_blk = struct
  ...
  module Builder = struct
    ...
    let init ?(same_tid = true) ?(copy_phis = false) ?(copy_defs = false)
      ...
      let b =
        create ~tid () ~phis:(Term.length phi_t blk)
          ~defs:(Term.length def_t blk) ~jmps:(Term.length jmp_t blk) in
    ...
    let result (b : t) : blk term =
      ...
      ; dict= Dict.empty
      ; self=
          {defs= of_vec b.b_defs; phis= of_vec b.b_phis; jmps= of_vec b.b_jmps}
      }
  end

  let create ?(tid = Tid.create ()) () : blk term =
    {tid; dict= Dict.empty; self= {phis= [||]; defs= [||]; jmps= [||]}}
  ...
  let map_elts ?phi ?def ?jmp blk =
    ...
    { blk with
      self= {phis= map phis ~f:phi; defs= map defs ~f:def; jmps= map jmps ~f:jmp}
    }
  ...
  let free_vars blk =
    ...
    fst
    @@ Seq.fold (elts blk) ~init ~f:(fun (vars, kill) ->
         function
  ...
  let find_var blk var =
    ...
    | None -> (
        ...
        |> function Some phi -> Some (`Phi phi) | None -> None )
  ...
  let pp_self ppf self =
    Format.fprintf ppf "@[@.%a%a%a@]" (Array.pp Ir_phi.pp) self.phis
      (Array.pp Ir_def.pp) self.defs (Array.pp Ir_jmp.pp) self.jmps
...
module Ir_sub = struct
  ...
  let pure =
    Bap_value.Tag.register
      (module Unit)
      ~name:"pure" ~uuid:"2c477ecb-0de8-4e8e-8cba-052eb67c628a"
  ...
  let extern =
    Bap_value.Tag.register
      (module Unit)
      ~name:"extern" ~uuid:"7965a2ee-ae72-4eb6-88c1-6f2b6108915f"

  let leaf =
    Bap_value.Tag.register
      (module Unit)
      ~name:"leaf" ~uuid:"0688a9ac-9e55-4e37-a9e5-ac0a4e4af59e"
  ...
  let nothrow =
    Bap_value.Tag.register
      (module Unit)
      ~name:"nothrow" ~uuid:"32058b19-95ca-46e3-bee0-d0a3694fd5b1"
  ...
  let pp_self ppf self =
    Format.fprintf ppf "@[<v>sub %s(%s)@.%a%a@]" self.name
      ( String.concat ~sep:", " @@ Array.to_list
      @@ Array.map self.args ~f:Ir_arg.name )
      (Array.pp Ir_arg.pp) self.args (Array.pp Ir_blk.pp) self.blks
...
module Ir_program = struct
  ...
  let create ?(tid = Tid.create ()) () : t = make_term tid (Program.empty ())
        ...
        Array.iteri self.subs ~f:(fun i {self} ->
            Array.iteri self.blks ~f:(fun j blk ->
                Array.iteri (get blk.self) ~f:(fun k ent ->
                    if Tid.(ent.tid = tid) then return (Some ([|i; j; k|], ent))))) ;
        None)
  ...
  let pp_self ppf self =
    Format.fprintf ppf "@[<v>program@.%a@]" (Array.pp Ir_sub.pp) self.subs

  let pp_self_slots ds ppf self =
    Format.fprintf ppf "@[<v>program@.%a@]"
      (Array.pp (Ir_sub.pp_slots ds))
      self.subs

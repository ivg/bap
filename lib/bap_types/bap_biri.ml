  ...
  class ['a] t =
    object (self)
      ...
      method private do_enter_term : 't 'p. ('p, 't) cls -> 't term -> 'a u =
        fun cls t ->
          update_context (fun c -> c#enter_term (Term.tid t))
          >>= fun () -> self#enter_term cls t
      ...
      method eval_sub sub : 'a u =
        set_next (Term.first blk_t sub)
        >>= fun () ->
        eval_args `enter sub (self#eval arg_t)
        >>= fun () ->
      ...
      method eval_blk (blk : blk term) : 'a u =
        set_next None
        >>= fun () ->
        eval_terms self#eval phi_t (Some def_t) (Some jmp_t) blk
        >>= fun () ->
        eval_terms self#eval def_t (Some jmp_t) None blk
        >>= fun () -> eval_terms self#eval jmp_t None None blk
      ...
      method eval_phi phi : 'a u =
        self#do_enter_term phi_t phi
        >>= fun () ->
        SM.get ()
        >>= fun c ->
        self#eval_exp (resolve_phi phi c#trace)
        >>= fun v ->
      ...
      method eval_jmp jmp =
        ...
        self#do_enter_term jmp_t jmp
        >>= fun () ->
        self#eval_exp (Ir_jmp.cond jmp)
        >>| value
        >>= function
        | Bot -> stop ()

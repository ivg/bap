...
module Make (Key : Key) = struct
  ...
  let length trie =
    let rec count trie =
      ...
      Tokens.fold ~init:n trie.subs ~f:(fun ~key:_ ~data:trie n ->
          n + count trie) in
    count trie

  let longest_match root k =
    walk root k ~init:(0, 0, None) ~f:(fun (n, i, best) ->
      function None -> (n + 1, i, best) | better -> (n + 1, n, better))
    |> function _, i, Some thing -> Some (i, thing) | _ -> None

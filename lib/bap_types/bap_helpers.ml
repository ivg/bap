...
class substitution x y =
  object (self)
    ...
    method! map_let z ~exp ~body =
      ...
      else super#map_let z ~exp:(self#map_exp exp) ~body:(self#map_exp body)
...
module Apply = struct
  ...
  let binop op u v =
    let open Word in
    if Int.(bitwidth u <> bitwidth v) && not (is_shift op) then
      failwithf "binop type error - %s %s %s" (to_string u)
...
module Type = struct
  ...
  and binop op x y =
    ...
    | _ -> (
      ...
      | Type.Imm _ as t -> (
        match op with LT | LE | EQ | NEQ | SLT | SLE -> Type.Imm 1 | _ -> t ) )
...
let fix compare f x =
  let rec loop slow fast =
    if compare slow fast = 0 then fast
      ...
      let fast' = f fast in
      if compare fast' fast = 0 then fast else loop (f slow) (f fast') in
    ...
    map
      (object
         ...
         method! map_sym var =
           ...
           String.Table.find_or_add vars name ~default:(fun () ->
...
module Normalize = struct
  ...
  *)
  let expand_load m a e s =
    ...
    let rec expand a i =
      if i > 1 then cat (load a) (expand (a ++ 1) (i - 1)) else load a in
  ...
  *)
  let hoist_non_generative_expressions =
    ...
    let rec exp : exp -> exp hoister = function
      ...
      | BinOp (op, x, y) ->
          exp x >>= fun x -> exp y >>= fun y -> return (BinOp (op, x, y))
      | Load (m, a, e, s) ->
          hoist m >>= fun m -> hoist a >>= fun a -> return (Load (m, a, e, s))
      | Store (m, a, v, e, s) ->
          exp m
          >>= fun m ->
          exp a
          ...
          >>= fun v ->
          hoist a
          >>= fun a -> hoist v >>= fun v -> return (Store (m, a, v, e, s))
      | Cast (t, s, x) -> exp x >>= fun x -> return (Cast (t, s, x))
      | Let (v, x, y) ->
          exp x
          >>= fun x ->
          exp y >>= fun y -> hoist x >>= fun x -> return (Let (v, x, y))
      | Ite (x, y, z) ->
          exp x
          >>= fun x ->
          exp y
          >>= fun y ->
          exp z >>= fun z -> hoist x >>= fun x -> return (Ite (x, y, z))
      | Extract (n, m, x) -> exp x >>= fun x -> return (Extract (n, m, x))
      | Concat (x, y) ->
          exp x >>= fun x -> exp y >>= fun y -> return (Concat (x, y)) in
...
module Stmt = struct
  ...
  let bil_free_vars bil =
    let update news vars kill = VS.union vars (VS.diff news kill) in
    fst
    @@ List.fold bil ~init:(VS.empty, VS.empty) ~f:(fun (vars, kill) ->
         function
         | Stmt.Move (v, e) ->

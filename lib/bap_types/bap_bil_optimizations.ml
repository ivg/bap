  ...
  class exp_propagator =
    object (self)
      method update const v : unit r =
        SM.get () >>= fun ctxt -> SM.put (update ctxt v const)
      ...
      method eval_binop op x y =
        self#eval_exp x
        >>= fun x -> self#eval_exp y >>= fun y -> SM.return (binop op x y)
      ...
      method eval_ite ~cond ~yes ~no =
        ...
        self#eval_exp yes
        >>= fun yes ->
        self#eval_exp no >>= fun no -> SM.return (ite cond yes no)
      ...
      method eval_concat x y =
        self#eval_exp x
        >>= fun x -> self#eval_exp y >>= fun y -> SM.return (concat x y)
  ...
  class bil_propagator =
    object (self)
      ...
      method eval_move v e : stmt r =
        self#eval_exp e
        >>= fun e -> self#update_var v e >>= fun () -> SM.return (move v e)
      ...
      method eval_while cond body =
        SM.get ()
        >>= fun ctxt ->
        SM.List.iter (defs body) ~f:(self#update Top)
        >>= fun () ->
        self#eval_exp cond
        >>= fun cond ->
        self#eval body
        ...
        >>= fun ctxt' ->
        let ctxt =
          if has_unconditional_jmps body then ctxt else join ctxt ctxt' in
        ...
        | While (cond, body) ->
            let live' =
              List.fold body ~init:live ~f:(fun live s ->
                  live ++ Stmt_helpers.free_vars s) in

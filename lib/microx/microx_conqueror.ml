...
let merge_visited =
  Map.merge ~f:(fun ~key ->
    function `Left x | `Right x -> Some x | `Both (x, y) -> Some (Int.max x y))
...
class context ?(max_steps = Int.max_value) ?(max_loop = min 10 (max_steps / 10))
  ...
  object (self : 's)
    ...
    val vis : int Tid.Map.t = Tid.Map.empty
...
class ['a] main ?(deterministic = false) p =
  object (self)
    ...
    method! enter_term cls t =
      ...
      SM.update (fun ctxt -> ctxt#visit_term tid)
      >>= fun () -> super#enter_term cls t

    method! eval_blk blk =
      SM.update (fun ctxt -> ctxt#enter_blk blk)
      >>= fun () -> super#eval_blk blk

    method! eval_sub sub =
      SM.update (fun ctxt -> ctxt#enter_sub sub)
      >>= fun () ->
      super#eval_sub sub
      >>= fun () -> SM.update (fun ctxt -> ctxt#leave_sub sub)
    ...
    method private next_of_jmp jmp =
      let goto dst =
        self#eval_goto dst >>= fun () -> SM.get () >>| fun ctxt -> ctxt#next
      in
      SM.get ()
      >>= fun ctxt ->
      let next =
    ...
    method! eval_call call =
      ...
      | Some (Direct ret) ->
          SM.update (fun ctxt -> ctxt#store_return ret)
          >>= fun () -> super#eval_call call
  end

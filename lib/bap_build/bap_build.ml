module Plugin_rules = struct
  ...
  let default_predicates = ["custom_ppx"; "ppxlib"]

  let default_tags =
    ["thread"; "debug"; "custom"; "pp(ppx-jane -dump-ast -inline-test-drop)"]
  ...
  let symlink env =
    if Options.make_links.contents then
      Cmd
        (S
           [ A "ln"; A "-sf"; P (env (!Options.build_dir / "%.plugin"))
  ...
  let link_shared_native ~src ~dst =
    Cmd
      (S
         [ !Options.ocamlopt; A "-shared"; A "-linkall"; A "-ccopt"; A "-L"
         ; A "-ccopt"; A (Filename.dirname src); P src; A "-o"; Px dst ])
  ...
  let bundle env =
    let requires =
      ...
      |> concat_map ~f:(fun pkg ->
             ...
             |> List.map ~f:(fun path ->
                    let name =
                      path |> Filename.chop_extension |> Filename.basename in
    ...
    Cmd
      (S
         [ A "bapbundle"; A "pack"
         ; T (Tags.of_list ["bundle"; "library"; "plugin"]); A "-name"
         ; A (env "%"); A "-main"; A (env "%.cmxs"); A "-main"; A (env "%.cma")
         ; requires; provides; Px (env "%.plugin") ])

  let register_cmxs_of_cmxa_rule () =
    rule "bap: cmxa & a -> cmxs" ~prods:["%.cmxs"]
      ~deps:["%.cmxa"; "%" -.- !Options.ext_lib] (fun env _ ->
...
module Std = struct
  ...
  module Plugin_options = struct let set = Plugin_rules.set_default_options end
end

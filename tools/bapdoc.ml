...
let run cmd =
  ...
  if res <> 0 then failwith ("Command: '" ^ cmd ^ "' failed")
...
let program = {man= ident; help= sprintf "%s --help=groff"}
let plugin = {man= sprintf "bap-plugin-%s"; help= sprintf "bap --%s-help=groff"}
...
let build_manual {man; help} tool =
  ...
  | 0 ->
      ...
      run @@ dump_to_file out
      @@ connect_with_pipes [help tool; repair_links; man2html; redirect_index]
  | _ ->
...
let generate_manual () =
  ...
  let _plugins =
    Plugins.list ()
    |> List.iter ~f:(fun p -> build_manual plugin (Plugin.name p)) in
...
let install_handwritten_manpages () = mkdir "man1" ; run "cp ../man/* man1/"
